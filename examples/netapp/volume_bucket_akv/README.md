...
Bucket credentials are generated by the `azurerm_netapp_volume_bucket_credentials` action. The action is invoked once via a `terraform_data` lifecycle `action_trigger` after the bucket is created. Azure NetApp Files writes the generated `access_key_id` / `secret_access_key` as a JSON secret into the credentials Key Vault configured on the bucket. Terraform never sees the key material.
...
To rotate the credentials, taint the `terraform_data.bucket_credentials` trigger (or change its `input`) and re-apply. The action runs again, generates a new key pair, immediately invalidates the previous one, and overwrites the secret in the credentials Key Vault.
