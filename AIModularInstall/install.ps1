...
.DESCRIPTION
    ...
    of the Terraform AzureRM Provider. This modular version replaces the monolithic
    installer with a clean, maintainable module system.
...
.EXAMPLE
    .\install.ps1 -RepositoryPath "C:\Projects\terraform-provider-azurerm"
    Installs AI setup using the specified repository path.

.EXAMPLE
    .\install.ps1 -AutoApprove
...
if ($Help) {
    ...
    Write-Host "  .\install.ps1                                  # Auto-discover repository"
    Write-Host "  .\install.ps1 -RepositoryPath C:\path\to\repo  # Use specific path"
    Write-Host "  .\install.ps1 -AutoApprove                     # Non-interactive install"
    Write-Host "  .\install.ps1 -Clean                           # Remove installation"
    Write-Host ""
