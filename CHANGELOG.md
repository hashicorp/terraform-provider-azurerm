...
* `azurerm_cdn_frontdoor_origin` - support `managedEnvironments` value for `private_link.target_type` [GH-28239]
* `azurerm_cdn_frontdoor_origin` - add support for the `web_secondary` `Gateway` values in the `private_link.target_type` property [GH-29380]
* `azurerm_cognitive_deployment` - add support for the `Cohere` value in the `model.format` property [GH-29143]
...
* `azurerm_mysql_flexible_server` - support for the `log_on_disk_enabled` property [GH-28929]
...
* `azurerm_postgresql_flexible_server` - downgrading `storage_mb` forces a new resource to be created [GH-29309]
* `azurerm_private_endpoint` - `private_dns_zone_group.private_dns_zone_ids` can now be updated correctly [GH-29329]
