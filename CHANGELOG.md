...
* dependencies: `go-azure-helpers` - update version to `0.82.0` [GH-33142]
...
BUG FIXES:
* `azurerm_eventhub_namespace` -  prevent `network_rulesets.x.default_action` being set to `Deny` if `ip_rule` or `virtual_network_rule` is not specified [GH-33216]
