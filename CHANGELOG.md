...
* New Resource: `azurerm_cosmosdb_sql_role_assignment` [GH-15038]
...
* dependencies: updating `sentinel` to `2021-09-01-preview` [GH-14983]
* Data Source: `azurerm_kubernetes_cluster` - deprecated the `addon_profile` block in favour of `aci_connector_linux`, `azure_policy_enabled`, `http_application_routing_enabled`, `ingress_application_gateway`, `key_vault_secrets_provider`, `oms_agent` and `open_service_mesh_enabled` properties [GH-15584]
* Data Source: `azurerm_kubernetes_cluster` - deprecated the `role_based_access_control` block in favour of `azure_active_directory_role_based_access_control` and `role_based_access_control_enabled` properties [GH-15584]
* Data Source: `azurerm_servicebus_namespace_authorization_rule` - support for the `namespace_id` property [GH-15671]
* Data Source: `azurerm_servicebus_namespace_disaster_recovery_config` - support for the `namespace_id` property [GH-15671]
* Data Source: `azurerm_servicebus_queue` - support for the `namespace_id` property [GH-15671]
* Data Source: `azurerm_servicebus_queue_authorization_rule` - support for the `queue_id` property [GH-15671]
* Data Source: `azurerm_servicebus_subscription` - support for the `topic_id` property [GH-15671]
* Data Source: `azurerm_servicebus_topic` - support for the `namespace_id` property [GH-15671]
* Data Source: `azurerm_servicebus_topic_authorization_rule` - support for the `topic_id` property [GH-15671]
* Data Source: `azurerm_virtual_network` - support for the `tags` property [GH-14882]
...
* `azurerm_backup_protected_vm` - the `source_vm_id` property is now case insensitive [GH-15656]
* `azurerm_batch_job` - will not longer fail during creation if multiple `common_environment_properties` are set [GH-15686]
* `azurerm_container_group` - correctly parse empty or omitted `dns_config.options` and `dns_config.search_domains` properties [GH-15618]
* `azurerm_key_vault_key` - correctly set the vault id on import [GH-15670]
* `azurerm_monitor_diagnostic_setting` - will now correctly parse the `eventhub_authorization_rule_id` property [GH-15582]
* `azurerm_mssql_managed_instance_active_directory_administrator` - prevent a perpetual diff with the instance ID [GH-15725]
* `azurerm_orchestrated_virtual_machine_scale_set` - prevent a crash when the 3.0 beta was enabled [GH-15637]
* `azurerm_storage_data_lake_gen2_filesystem` - support configuring the `group` and `owner` properties [GH-15598]
* `azurerm_virtual_network_gateway` - prevent a panic with `bgp_settings.0.peering_address` [GH-15689]
