...
* **New Data Source:** `azurerm_servicebus_namespace` [GH-2841]
...
* Data Source `azurerm_key_vault_key` - deprecating the `vault_uri` property in favour of `key_vault_id` [GH-2820]
* Data Source `azurerm_key_vault_secret` - deprecating the `vault_uri` property in favour of `key_vault_id` [GH-2820]
...
* `azurerm_autoscale_setting` - support values from `0` to `1000` for the `minimum`, `maximum` and `default` properties [GH-2815]
* `azurerm_batch_pool` - support for the `max_tasks_per_node` property [GH-2805]
* `azurerm_cognitive_account` - exporting `primary_access_key` and `secondary_access_key` [GH-2825]
