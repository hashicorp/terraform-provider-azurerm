...
* snapshot - refactoring to use `github.com/hashicorp/go-azure-sdk` [GH-18957]
...
*  `azurerm_app_configuration_keys` - will now correctly retrieve result if more than 100 entries are returned [GH-19020]
* `azurerm_data_factory_dataset_parquet` - `azure_blob_storage_location.path` and `http_server_location.path` are now Optional [GH-19009]
