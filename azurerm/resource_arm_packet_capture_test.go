...
func testAccAzureRMPacketCapture_requiresImport(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config:      testAzureRMPacketCapture_requiresImportConfig(ri, location),
...
func testAzureRMPacketCapture_requiresImportConfig(rInt int, location string) string {
	template := testAzureRMPacketCapture_localDiskConfig(rInt, location)
	return fmt.Sprintf(`
%s
