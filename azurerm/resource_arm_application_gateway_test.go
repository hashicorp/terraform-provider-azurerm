...
func TestAccAzureRMApplicationGateway_webApplicationFirewall_exclusions(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAzureRMApplicationGateway_webApplicationFirewall_exclusions(ri, testLocation()),
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMApplicationGatewayExists(resourceName),
...
func testAccAzureRMApplicationGateway_webApplicationFirewall_exclusions(rInt int, location string) string {
	template := testAccAzureRMApplicationGateway_template(rInt, location)
	return fmt.Sprintf(`
%s
