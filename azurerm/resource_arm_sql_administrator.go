	...
	if d.IsNewResource() {
		...
		if err != nil {
			if !utils.ResponseWasNotFound(resp.Response) {
				return fmt.Errorf("Error checking for the existence of SQL AD Administrator %q (Resource Group %q): %+v", serverName, resGroup, err)
			}
		}

		if resp.ID != nil {
			return tf.ImportAsExistsError("azurerm_sql_active_directory_administrator", *resp.ID)
		}
	}
...
func resourceArmSqlActiveDirectoryAdministratorDelete(d *schema.ResourceData, meta interface{}) error {
	...
	defer cancel()
	_, err = client.Delete(waitCtx, resourceGroup, serverName)
	if err != nil {
