...
func resourceArmDnsTxtRecordCreateOrUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.IsNewResource() {
		...
		if err != nil {
			if !utils.ResponseWasNotFound(resp.Response) {
				return fmt.Errorf("Error checking for the existence of DNS TXT Record %q (Zone %q / Resource Group %q): %+v", name, zoneName, resGroup, err)
			}
		}

		if resp.ID != nil {
			return tf.ImportAsExistsError("azurerm_dns_txt_record", *resp.ID)
		}
	}
	...
	defer cancel()
	resp, err := client.CreateOrUpdate(waitCtx, resGroup, zoneName, name, dns.TXT, parameters, eTag, ifNoneMatch)
	if err != nil {
		return err
	}
...
func resourceArmDnsTxtRecordDelete(d *schema.ResourceData, meta interface{}) error {
	...
	defer cancel()
	resp, err := client.Delete(waitCtx, resGroup, zoneName, name, dns.TXT, "")
