...
func TestAccAzureRMIotHub_enrichments(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_iothub", "test")
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAzureRMIotHub_enrichments(data),
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMIotHubExists(data.ResourceName),
					resource.TestCheckResourceAttr(data.ResourceName, "endpoint.#", "2"),
					resource.TestCheckResourceAttr(data.ResourceName, "endpoint.0.type", "AzureIotHub.StorageContainer"),
					...
					resource.TestCheckResourceAttr(data.ResourceName, "enrichment.#", "2"),
				),
			},
			data.ImportStep(),
		},
...
func testAccAzureRMIotHub_enrichments(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
