...
func (LogAnalyticsSavedSearchResource) Exists(ctx context.Context, clients *clients.Client, state *terraform.InstanceState) (*bool, error) {
	id, err := parse.LogAnalyticsSavedSearchID(state.ID)
	if err != nil {
		return nil, err
	}

	resp, err := clients.LogAnalytics.SavedSearchesClient.Get(ctx, id.ResourceGroup, id.WorkspaceName, id.Name)
	if err != nil {
		return nil, fmt.Errorf("retrieving Log Analytics Saved Search %q (Workspace: %q / Resource Group: %q): %v", id.Name, id.WorkspaceName, id.ResourceGroup, err)
	}

	return utils.Bool(resp.SavedSearchProperties != nil), nil
}
...
func (LogAnalyticsSavedSearchResource) requiresImport(data acceptance.TestData) string {
	template := LogAnalyticsSavedSearchResource{}.basic(data)
	return fmt.Sprintf(`
%s
