...
import (
	...
	dataFactoryValidate "github.com/terraform-providers/terraform-provider-azurerm/azurerm/internal/services/datafactory/validate"
...
var identity SystemAssignedUserAssigned

func resourceDataFactory() *pluginsdk.Resource {
	return &pluginsdk.Resource{
		Create: resourceDataFactoryCreate,
		...
		Schema: map[string]*pluginsdk.Schema{
			...
			"data_factory_id": {
				...
				Required:     true,
				ForceNew:     true,
				ValidateFunc: dataFactoryValidate.DataFactoryID,
			},
...
func resourceDataFactoryCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if d.IsNewResource() {
		...
		if !utils.ResponseWasNotFound(existing.Response) {
			return tf.ImportAsExistsError("azurerm_machine_learning_data_factory", id.ID())
		}
	}

	identity, err := identity.Expand(d.Get("identity").([]interface{}))
	if err != nil {
		return err
	}
	computeParameters := machinelearningservices.ComputeResource{
		Properties: machinelearningservices.DataFactory{
			ResourceID:      utils.String(d.Get("data_factory_id").(string)),
...
func resourceDataFactoryRead(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	identity, err := identity.Flatten(resp.Identity)
	if err != nil {
		return err
	}
	d.Set("identity", identity)

	if props, ok := resp.Properties.AsDataFactory(); ok && props != nil {
		d.Set("data_factory_id", props.ResourceID)
		d.Set("description", props.Description)
	} else {
		return fmt.Errorf("compute resource %s is not a Data Factory Compute", id)
	}
