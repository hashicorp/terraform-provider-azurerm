...
func (s SystemAssignedUserAssigned) Expand(input []interface{}) (*machinelearningservices.Identity, error) {
	if len(input) == 0 || input[0] == nil {
		return &machinelearningservices.Identity{
			Type: machinelearningservices.None,
		}, nil
	}
	...
	if len(identityIds) != 0 {
		if config.Type != machinelearningservices.UserAssigned && config.Type != machinelearningservices.SystemAssignedUserAssigned {
			return nil, fmt.Errorf("`identity_ids` can only be specified when `type` includes `UserAssigned`")
		}
		config.UserAssignedIdentities = map[string]*machinelearningservices.IdentityUserAssignedIdentitiesValue{}
		for _, id := range identityIds {
			config.UserAssignedIdentities[id.(string)] = &machinelearningservices.IdentityUserAssignedIdentitiesValue{}
		}
	}
...
func (s SystemAssignedUserAssigned) Flatten(input *machinelearningservices.Identity) ([]interface{}, error) {
	if input == nil || input.Type == machinelearningservices.None {
			...
			Schema: map[string]*pluginsdk.Schema{
				"type": {
					...
					ValidateFunc: validation.StringInSlice([]string{
						string(machinelearningservices.UserAssigned),
						string(machinelearningservices.SystemAssigned),
						string(machinelearningservices.SystemAssignedUserAssigned),
					}, false),
				},
				"identity_ids": {
