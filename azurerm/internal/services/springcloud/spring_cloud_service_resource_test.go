	...
	data.ResourceTest(t, r, []resource.TestStep{
		{
			...
			Check: resource.ComposeTestCheckFunc(
				...
				check.That(data.ResourceName).Key("required_traffic.0.protocol").Exists(),
				check.That(data.ResourceName).Key("required_traffic.0.port").Exists(),
				check.That(data.ResourceName).Key("required_traffic.0.ip_addresses.#").Exists(),
				check.That(data.ResourceName).Key("required_traffic.0.fqdns.#").Exists(),
				check.That(data.ResourceName).Key("required_traffic.0.direction").Exists(),
			),
		},
		data.ImportStep(
			// those field returned by api are "*"
