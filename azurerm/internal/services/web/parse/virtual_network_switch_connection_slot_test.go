...
func TestVirtualNetworkSwitchConnectionSlotID(t *testing.T) {
	testData := []struct {
		Name     string
		Input    string
		Expected *VirtualNetworkSwitchConnectionSlotId
	}{
		{
			Name:     "Empty",
			Input:    "",
	...
	for _, v := range testData {
		...
		actual, err := VirtualNetworkSwitchConnectionSlotID(v.Input)
		if err != nil {
			if v.Expected == nil {
				continue
			}
