...
func TestParseAppServiceSlotCustomHostnameBinding(t *testing.T) {
	testData := []struct {
		Name     string
		Input    string
		...
		{
			Name:     "Missing Host Name Bindings Value",
			Input:    "/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/mygroup1/providers/Microsoft.Web/sites/site1/slots/staging/hostNameBindings/",
			Expected: nil,
		},
		{
	...
	for _, v := range testData {
		...
		if actual.AppServiceSlot != v.Expected.AppServiceSlot {
			t.Fatalf("Expected %q but got %q for Slot", v.Expected.AppServiceSlot, actual.AppServiceSlot)
		}
