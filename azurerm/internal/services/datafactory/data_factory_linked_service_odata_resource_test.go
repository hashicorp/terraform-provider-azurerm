	...
	data.ResourceTest(t, r, []resource.TestStep{
		{
			...
			Check: resource.ComposeTestCheckFunc(
				...
				check.That(data.ResourceName).Key("url").HasValue("https://services.odata.org/v4/TripPinServiceRW/People"),
			),
		},
		data.ImportStep(),
	})
	...
	data.ResourceTest(t, r, []resource.TestStep{
		{
			...
			Check: resource.ComposeTestCheckFunc(
				...
				check.That(data.ResourceName).Key("url").Exists(),
				check.That(data.ResourceName).Key("url").HasValue("https://services.odata.org/v4/TripPinServiceRW/People"),
				check.That(data.ResourceName).Key("basic_authentication.username").Exists(),
				check.That(data.ResourceName).Key("basic_authentication.username").HasValue("emma"),
				check.That(data.ResourceName).Key("basic_authentication.password").Exists(),
				check.That(data.ResourceName).Key("basic_authentication.password").HasValue("Ch4ngeM3!"),
			),
		},
		data.ImportStep(),
	})
	...
	data.ResourceTest(t, r, []resource.TestStep{
		{
			...
			Check: resource.ComposeTestCheckFunc(
				...
				check.That(data.ResourceName).Key("url").HasValue("https://services.odata.org/v4/TripPinServiceRW/"),
			),
		},
		data.ImportStep(),
		{
...
func (t LinkedServiceODataResource) Exists(ctx context.Context, clients *clients.Client, state *terraform.InstanceState) (*bool, error) {
