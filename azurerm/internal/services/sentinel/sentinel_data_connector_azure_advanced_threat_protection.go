...
func resourceSentinelDataConnectorAzureAdvancedThreatProtection() *schema.Resource {
	return &schema.Resource{
		Create: resourceSentinelDataConnectorAzureAdvancedThreatProtectionCreateUpdate,
...
func resourceSentinelDataConnectorAzureAdvancedThreatProtectionCreateUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.IsNewResource() {
		...
		if err != nil {
			if !utils.ResponseWasNotFound(resp.Response) {
				return fmt.Errorf("checking for existing Sentinel Data Connector Azure Advanced Threat Protection %q: %+v", id, err)
			}
		}

		id := dataConnectorID(resp.Value)
		if id != nil && *id != "" {
			return tf.ImportAsExistsError("azurerm_sentinel_data_connector_azure_advanced_threat_protection", *id)
		}
	}
	...
	if err != nil {
		return fmt.Errorf("creating Sentinel Data Connector Azure Advanced Threat Protection %q: %+v", id, err)
	}
...
func resourceSentinelDataConnectorAzureAdvancedThreatProtectionRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Sentinel.DataConnectorsClient
	...
	if err != nil {
		if utils.ResponseWasNotFound(resp.Response) {
			log.Printf("[DEBUG] Sentinel Data Connector Azure Advanced Threat Protection %q was not found - removing from state!", id)
			d.SetId("")
			return nil
		}

		return fmt.Errorf("retrieving Sentinel Data Connector Azure Advanced Threat Protection %q: %+v", id, err)
	}

	if err := assertDataConnectorKind(resp.Value, securityinsight.DataConnectorKindAzureAdvancedThreatProtection); err != nil {
		return fmt.Errorf("asserting Sentinel Data Connector Azure Advanced Threat Protection of %q: %+v", id, err)
	}
...
func resourceSentinelDataConnectorAzureAdvancedThreatProtectionDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Sentinel.DataConnectorsClient
	...
	if err != nil {
		return fmt.Errorf("deleting Sentinel Data Connector Azure Advanced Threat Protection %q: %+v", id, err)
	}
