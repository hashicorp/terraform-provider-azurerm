	...
	if d.IsNewResource() {
		...
		if err != nil {
			if !utils.ResponseWasNotFound(resp.Response) {
				return fmt.Errorf("checking for existing Sentinel Data Connector Office ATP %q: %+v", id, err)
			}
		}

		id := dataConnectorID(resp.Value)
		if id != nil && *id != "" {
			return tf.ImportAsExistsError("azurerm_sentinel_data_connector_office_atp", *id)
		}
	}
...
func resourceSentinelDataConnectorOfficeATPRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Sentinel.DataConnectorsClient
	...
	if err != nil {
		if utils.ResponseWasNotFound(resp.Response) {
			log.Printf("[DEBUG] Sentinel Data Connector Office ATP %q was not found - removing from state!", id)
			d.SetId("")
			return nil
		}

		return fmt.Errorf("retrieving Sentinel Data Connector Office ATP %q: %+v", id, err)
	}

	if err := assertDataConnectorKind(resp.Value, securityinsight.DataConnectorKindOfficeATP); err != nil {
		return fmt.Errorf("asserting Sentinel Data Connector Office ATP of %q: %+v", id, err)
	}
	dc := resp.Value.(securityinsight.OfficeATPDataConnector)
...
func resourceSentinelDataConnectorOfficeATPDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Sentinel.DataConnectorsClient
	...
	if err != nil {
		return fmt.Errorf("deleting Sentinel Data Connector Office ATP %q: %+v", id, err)
	}
