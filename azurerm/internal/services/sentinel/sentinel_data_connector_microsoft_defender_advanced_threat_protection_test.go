...
func (r SentinelDataConnectorMicrosoftDefenderAdvancedThreatProtectionResource) Exists(ctx context.Context, clients *clients.Client, state *terraform.InstanceState) (*bool, error) {
	...
	if resp, err := client.Get(ctx, id.ResourceGroup, "Microsoft.OperationalInsights", id.WorkspaceName, id.Name); err != nil {
		...
		return nil, fmt.Errorf("retrieving Sentinel Data Connector Microsoft Defender Advanced Threat Protection %q: %+v", id, err)
	}
