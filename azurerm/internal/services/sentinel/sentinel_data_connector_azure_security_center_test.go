...
func (r SentinelDataConnectorAzureSecurityCenterResource) Exists(ctx context.Context, clients *clients.Client, state *terraform.InstanceState) (*bool, error) {
	...
	if resp, err := client.Get(ctx, id.ResourceGroup, "Microsoft.OperationalInsights", id.WorkspaceName, id.Name); err != nil {
		...
		return nil, fmt.Errorf("retrieving Sentinel Data Connector Azure Security Center %q: %+v", id, err)
	}
