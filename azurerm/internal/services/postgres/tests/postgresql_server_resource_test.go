...
func TestAccAzureRMPostgreSQLServer_updatedDeprecated(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			data.ImportStep("administrator_login_password"),
			{
				Config: testAccAzureRMPostgreSQLServer_updateDeprecated(data, "9.6"),
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMPostgreSQLServerExists(data.ResourceName),
				),
			},
...
func TestAccAzureRMPostgreSQLServer_updated(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			data.ImportStep("administrator_login_password"),
			{
				Config: testAccAzureRMPostgreSQLServer_complete3(data, "9.6"),
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMPostgreSQLServerExists(data.ResourceName),
				),
			},
...
func testAccAzureRMPostgreSQLServer_updateDeprecated(data acceptance.TestData, version string) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
...
func testAccAzureRMPostgreSQLServer_complete3(data acceptance.TestData, version string) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
