...
func resourceArmLoadBalancerNatPoolCreateUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		...
		return fmt.Errorf("failed to retrieve Load Balancer %q (resource group %q) for Nat Pool %q: %+v", loadBalancerId.Name, loadBalancerId.ResourceGroup, id.InboundNatPoolName, err)
	}
