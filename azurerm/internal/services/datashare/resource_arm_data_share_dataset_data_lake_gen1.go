...
func resourceArmDataShareDataSetDataLakeGen1() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"data_lake_store_resource_group_name": azure.SchemaResourceGroupName(),
	...
	if fileName, ok := d.GetOk("file_name"); ok {
		dataSet = datashare.ADLSGen1FileDataSet{
			...
			ADLSGen1FileProperties: &datashare.ADLSGen1FileProperties{
				...
				ResourceGroup:  utils.String(d.Get("data_lake_store_resource_group_name").(string)),
				SubscriptionID: utils.String(d.Get("data_lake_store_subscription_id").(string)),
