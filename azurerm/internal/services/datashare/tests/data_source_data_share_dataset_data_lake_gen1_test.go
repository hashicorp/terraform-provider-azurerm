		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet(data.ResourceName, "data_lake_store_name"),
					resource.TestCheckResourceAttrSet(data.ResourceName, "data_lake_store_resource_group_name"),
					resource.TestCheckResourceAttrSet(data.ResourceName, "data_lake_store_subscription_id"),
...
data "azurerm_data_share_dataset_data_lake_gen1" "test" {
  ...
  share_id = azurerm_data_share_dataset_data_lake_gen1.test.share_id
}
`, config)
}
