...
func resourceArmMonitorScheduledQueryRulesAction() *schema.Resource {
	return &schema.Resource{
		Create: resourceArmMonitorScheduledQueryRulesActionCreateUpdate,
		Read:   resourceArmMonitorScheduledQueryRulesActionRead,
		Update: resourceArmMonitorScheduledQueryRulesActionCreateUpdate,
...
func resourceArmMonitorScheduledQueryRulesActionCreateUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if features.ShouldResourcesBeImported() && d.IsNewResource() {
		...
		if existing.ID != nil && *existing.ID != "" {
			return tf.ImportAsExistsError("azurerm_monitor_scheduled_query_rules_action", *existing.ID)
		}
	}
	...
	return resourceArmMonitorScheduledQueryRulesActionRead(d, meta)
}

func resourceArmMonitorScheduledQueryRulesActionRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Monitor.ScheduledQueryRulesClient
...
func resourceArmMonitorScheduledQueryRulesActionDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Monitor.ScheduledQueryRulesClient
...
func expandMonitorScheduledQueryRulesAlertingAction(d *schema.ResourceData) *insights.AlertingAction {
	...
	aznsAction := expandMonitorScheduledQueryRulesActionAznsAction(aznsActionRaw)
	...
	trigger := expandMonitorScheduledQueryRulesActionTrigger(triggerRaw)
...
func expandMonitorScheduledQueryRulesActionAznsAction(input []interface{}) *insights.AzNsActionGroup {
...
func expandMonitorScheduledQueryRulesActionMetricTrigger(input []interface{}) *insights.LogMetricTrigger {
	if len(input) == 0 {
		return nil
	}
...
func expandMonitorScheduledQueryRulesActionTrigger(input []interface{}) *insights.TriggerCondition {
	...
	for _, item := range input {
		...
		result.ThresholdOperator = insights.ConditionalOperator(v["operator"].(string))
		result.Threshold = utils.Float(v["threshold"].(float64))
		result.MetricTrigger = expandMonitorScheduledQueryRulesActionMetricTrigger(metricTriggerRaw)
	}
...
func flattenAzureRmScheduledQueryRulesActionMetricTrigger(input *insights.LogMetricTrigger) []interface{} {
...
func flattenAzureRmScheduledQueryRulesActionTrigger(input *insights.TriggerCondition) []interface{} {
	result := make(map[string]interface{})
	...
	if input.MetricTrigger != nil {
		result["metric_trigger"] = flattenAzureRmScheduledQueryRulesActionMetricTrigger(input.MetricTrigger)
	}
