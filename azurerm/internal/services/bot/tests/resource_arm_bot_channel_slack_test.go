...
func testCheckAzureRMBotChannelSlackExists(name string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		id, err := parse.BotChannelSlackID(rs.Primary.ID)
		if err != nil {
			return err
		}

		resp, err := client.Get(ctx, id.ResourceGroup, id.BotName, string(botservice.ChannelNameSlackChannel))
		if err != nil {
			return fmt.Errorf("Bad: Get on botChannelClient: %+v", err)
		}

		if utils.ResponseWasNotFound(resp.Response) {
			return fmt.Errorf("Bad: Bot Channel Slack %q (resource group: %q / bot: %q) does not exist", name, id.ResourceGroup, id.BotName)
		}
