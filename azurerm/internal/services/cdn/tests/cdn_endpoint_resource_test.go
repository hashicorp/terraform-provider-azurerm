...
func TestAccAzureRMCdnEndpoint_updateHostHeader(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAzureRMCdnEndpoint_hostHeader(data, "www.google.com"),
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMCdnEndpointExists(data.ResourceName),
					resource.TestCheckResourceAttr(data.ResourceName, "origin_host_header", "www.google.com"),
				),
			},
			{
			...
			{
				Config: testAccAzureRMCdnEndpoint_hostHeader(data, "www.terraform.io"),
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMCdnEndpointExists(data.ResourceName),
					resource.TestCheckResourceAttr(data.ResourceName, "origin_host_header", "www.terraform.io"),
				),
			},
		},
	})
...
resource "azurerm_cdn_endpoint" "test" {
  ...
  origin_host_header            = "www.google.com"
  optimization_type             = "GeneralWebDelivery"
  origin_path                   = "/origin-path"
  probe_path                    = "/origin-path/probe"
