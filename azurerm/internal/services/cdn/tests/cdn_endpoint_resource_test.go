...
func TestAccAzureRMCdnEndpoint_deliveryRuleOptionalMatchValue(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_cdn_endpoint", "test")
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAzureRMCdnEndpoint_deliveryRuleOptionalMatchValue(data),
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMCdnEndpointExists(data.ResourceName),
				),
			},
...
func testAccAzureRMCdnEndpoint_deliveryRuleOptionalMatchValue(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
