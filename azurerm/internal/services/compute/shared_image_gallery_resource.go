...
func resourceArmSharedImageGalleryRead(d *schema.ResourceData, meta interface{}) error {
	...
	resp, err := client.Get(ctx, id.ResourceGroup, id.GalleriesName)
	if err != nil {
		if utils.ResponseWasNotFound(resp.Response) {
			log.Printf("[DEBUG] Shared Image Gallery %q (Resource Group %q) was not found - removing from state", id.GalleriesName, id.ResourceGroup)
			d.SetId("")
			return nil
		}

		return fmt.Errorf("Error making Read request on Shared Image Gallery %q (Resource Group %q): %+v", id.GalleriesName, id.ResourceGroup, err)
	}

	d.Set("name", id.GalleriesName)
...
func resourceArmSharedImageGalleryDelete(d *schema.ResourceData, meta interface{}) error {
	...
	future, err := client.Delete(ctx, id.ResourceGroup, id.GalleriesName)
	if err != nil {
		...
		return fmt.Errorf("Error deleting Shared Image Gallery %q (Resource Group %q): %+v", id.GalleriesName, id.ResourceGroup, err)
	}

	if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
		if !response.WasNotFound(future.Response()) {
			return fmt.Errorf("Error waiting for the deletion of Shared Image Gallery %q (Resource Group %q): %+v", id.GalleriesName, id.ResourceGroup, err)
		}
	}
