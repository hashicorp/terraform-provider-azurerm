...
`, CosmosDBAccountMongoResource{}.basic(data, documentdb.Strong), data.RandomInteger)
}
...
`, CosmosDBAccountMongoResource{}.capabilities(data, []string{"EnableMongo"}), data.RandomInteger, maxThroughput)
}
...
`, CosmosDBAccountMongoResource{}.mongo36WithCapabilities(data, []string{"EnableServerless", "mongoEnableDocLevelTTL"}), data.RandomInteger)
}
