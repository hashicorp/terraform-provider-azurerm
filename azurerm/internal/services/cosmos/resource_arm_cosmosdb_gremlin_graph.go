...
func resourceArmCosmosDbGremlinGraphCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if resp.ID == nil {
		return fmt.Errorf("Error getting ID from Cosmos Gremlin Graph %s (Account %s, Database:%s)", name, account, database)
	}
...
func resourceArmCosmosDbGremlinGraphUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error issuing create/update request for Cosmos Gremlin Graph %s (Account: %s, Database:%s): %+v", id.Name, id.Account, id.Database, err)
	}

	if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
		return fmt.Errorf("Error waiting on create/update future for Cosmos Gremlin Graph %s (Account: %s, Database:%s): %+v", id.Name, id.Account, id.Database, err)
	}
	...
	if d.HasChange("throughput") {
		...
		if err = throughputFuture.WaitForCompletionRef(ctx, client.Client); err != nil {
			return fmt.Errorf("Error waiting on ThroughputUpdate future for Cosmos Gremlin Graph %s (Account: %s, Database:%s): %+v", id.Name, id.Account, id.Database, err)
		}
	}
...
func resourceArmCosmosDbGremlinGraphRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		if utils.ResponseWasNotFound(resp.Response) {
			log.Printf("[INFO] Error reading Cosmos Gremlin Graph %s (Account %s) - removing from state", id.Name, id.Account)
			d.SetId("")
			return nil
		}

		return fmt.Errorf("Error reading Cosmos Gremlin Graph %s (Account %s): %+v", id.Name, id.Account, err)
	}
	...
	if err != nil {
		if !utils.ResponseWasNotFound(throughputResp.Response) {
			return fmt.Errorf("Error reading Throughput on Gremlin Graph '%s' (Account: %s, Database:%s) ID: %v", id.Name, id.Account, id.Database, err)
		} else {
...
func resourceArmCosmosDbGremlinGraphDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		if !response.WasNotFound(future.Response()) {
			return fmt.Errorf("Error deleting Cosmos Gremlin Graph %s (Account %s): %+v", id.Database, id.Name, err)
		}
	}
