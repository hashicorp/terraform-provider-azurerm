...
func TestAccAzureRMFirewall_withSku(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_firewall", "test")
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAzureRMFirewall_withSku(data),
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMFirewallExists(data.ResourceName),
				),
			},
...
func testAccAzureRMFirewall_withSku(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
