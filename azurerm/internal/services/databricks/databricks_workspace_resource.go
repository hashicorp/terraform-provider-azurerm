...
func resourceDatabricksWorkspace() *pluginsdk.Resource {
	return &pluginsdk.Resource{
		...
		Schema: map[string]*pluginsdk.Schema{
			...
			"customer_managed_keys_for_managed_services_key_vault_key_id": {
				Type:         pluginsdk.TypeString,
				Optional:     true,
				ForceNew:     true,
		...
		CustomizeDiff: pluginsdk.CustomizeDiffShim(func(ctx context.Context, d *pluginsdk.ResourceDiff, v interface{}) error {
			...
			_, notebookCMK := d.GetChange("customer_managed_keys_for_managed_services_key_vault_key_id")
			...
			if (customerEncryptionEnabled.(bool) || infrastructureEncryptionEnabled.(bool) || notebookCMK.(string) != "") && !strings.EqualFold("premium", newSku.(string)) {
				return fmt.Errorf("'customer_managed_key_enabled', 'infrastructure_encryption_enabled' and 'customer_managed_keys_for_managed_services_key_vault_key_id' are only available with a 'premium' workspace 'sku', got %q", newSku)
			}
...
func resourceDatabricksWorkspaceCreateUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	keyIdRaw := d.Get("customer_managed_keys_for_managed_services_key_vault_key_id").(string)
	if keyIdRaw != "" {
	...
	if encrypt != nil && keyIdRaw != "" {
		d.Set("customer_managed_keys_for_managed_services_key_vault_key_id", keyIdRaw)
	}
...
func resourceDatabricksWorkspaceRead(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if encryptKeyVaultURI != "" {
		...
		if err == nil {
			d.Set("customer_managed_keys_for_managed_services_key_vault_key_id", key.ID())
		}
	}
