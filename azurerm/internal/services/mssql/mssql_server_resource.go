...
func resourceArmMsSqlServerRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err := d.Set("restore_dropped_database_id", flattenAzureRmSqlServerRestorableDatabases(restorableResp)); err != nil {
		return fmt.Errorf("setting `restore_dropped_database_id`: %+v", err)
	}
