			...
			/*
				{
					...
					Check: resource.ComposeTestCheckFunc(
						testCheckAzureRMVirtualMachineExists(resourceName, &afterVm),
					),
				},
			*/
		},
...
func testAccAzureRMVirtualMachine_reorderedDisksBefore(rInt int, location string) string {
	return fmt.Sprintf(`
resource "azurerm_resource_group" "test" {
    name = "acctestRG-%d"
    location = "%s"
...
resource "azurerm_managed_disk" "first" {
  name = "acctestdisk-%d"
  location = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
  storage_account_type = "Standard_LRS"
...
resource "azurerm_virtual_machine" "test" {
	...
	storage_data_disk {
		name            = "${azurerm_managed_disk.first.name}"
		managed_disk_id = "${azurerm_managed_disk.first.id}"
		create_option   = "Attach"
		lun             = 1
		disk_size_gb    = "${azurerm_managed_disk.first.disk_size_gb}"
	}

	storage_data_disk {
		name            = "${azurerm_managed_disk.second.name}"
		managed_disk_id = "${azurerm_managed_disk.second.id}"
		create_option   = "Attach"
		lun             = 10
		disk_size_gb    = "${azurerm_managed_disk.second.disk_size_gb}"
	}
...
func testAccAzureRMVirtualMachine_reorderedDisksAfter(rInt int, location string) string {
	return fmt.Sprintf(`
resource "azurerm_resource_group" "test" {
    name = "acctestRG-%d"
    location = "%s"
