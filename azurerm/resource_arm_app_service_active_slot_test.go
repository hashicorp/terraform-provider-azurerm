...
func TestAccAzureRMAppServiceActiveSlot_multiple(t *testing.T) {
	resourceName := "azurerm_app_service_active_slot.test"
	ri := acctest.RandInt()
	config := testAccAzureRMAppServiceActiveSlot_multiple(ri, testLocation())
	config2 := testAccAzureRMAppServiceActiveSlot_multipleUpdated(ri, testLocation())
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				Config: config2,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "source_slot_name", fmt.Sprintf("acctestASSlot2-%d", ri)),
				),
			},
		},
	})
...
func testAccAzureRMAppServiceActiveSlot_multiple(rInt int, location string) string {
	return fmt.Sprintf(`
	resource "azurerm_resource_group" "test" {
		name = "acctestRG-%d"
		location = "%s"
...
func testAccAzureRMAppServiceActiveSlot_multipleUpdated(rInt int, location string) string {
	return fmt.Sprintf(`
	resource "azurerm_resource_group" "test" {
		name = "acctestRG-%d"
		location = "%s"
	...
	resource "azurerm_app_service_active_slot" "test" {
		...
		app_service_name    = "${azurerm_app_service.test.name}"
		preserve_vnet       = true
		source_slot_name    = "${azurerm_app_service_slot.test2.name}"
	}
	`, rInt, location, rInt, rInt, rInt, rInt)
}
