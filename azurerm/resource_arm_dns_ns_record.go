...
func resourceArmDnsNsRecordCreateOrUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.IsNewResource() {
		...
		if err != nil {
			if !utils.ResponseWasNotFound(resp.Response) {
				return fmt.Errorf("Error checking for the existence of DNS NS Record %q (Zone %q / Resource Group %q): %+v", name, zoneName, resGroup, err)
			}
		}

		if resp.ID != nil {
			return tf.ImportAsExistsError("azurerm_dns_ns_record", *resp.ID)
		}
	}
	...
	defer cancel()
	resp, err := client.CreateOrUpdate(waitCtx, resGroup, zoneName, name, dns.NS, parameters, eTag, ifNoneMatch)
	if err != nil {
		return err
	}
...
func resourceArmDnsNsRecordDelete(d *schema.ResourceData, meta interface{}) error {
	...
	defer cancel()
	resp, error := dnsClient.Delete(waitCtx, resGroup, zoneName, name, dns.NS, "")
