...
func TestAccAzureRMKubernetesCluster_basicWithLinuxProfile(t *testing.T) {
	resourceName := "azurerm_kubernetes_cluster.test"
	ri := acctest.RandInt()
	clientId := os.Getenv("ARM_CLIENT_ID")
	clientSecret := os.Getenv("ARM_CLIENT_SECRET")
	config := testAccAzureRMKubernetesCluster_basic(ri, clientId, clientSecret, testLocation(), fmt.Sprintf(`
linux_profile {
...
func testAccAzureRMKubernetesCluster_basic(rInt int, clientId string, clientSecret string, location string, linuxProfile string) string {
	return fmt.Sprintf(`
