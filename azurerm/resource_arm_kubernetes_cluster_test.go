...
func TestAccAzureRMKubernetesCluster_orchestratorVersion(t *testing.T) {
	resourceName := "azurerm_kubernetes_cluster.test"
	ri := tf.AccRandTimeInt()
	clientId := os.Getenv("ARM_CLIENT_ID")
	clientSecret := os.Getenv("ARM_CLIENT_SECRET")
	config := testAccAzureRMKubernetesCluster_orchestratorVersion(ri, clientId, clientSecret, testLocation())
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMKubernetesClusterExists(resourceName),
					resource.TestCheckResourceAttr(resourceName, "agent_pool_profile.0.orchestrator_version", "1.14.6"),
				),
			},
			{
				ResourceName:      resourceName,
...
func testAccAzureRMKubernetesCluster_orchestratorVersion(rInt int, clientId string, clientSecret string, location string) string {
	return fmt.Sprintf(`
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-%d"
  location = "%s"
