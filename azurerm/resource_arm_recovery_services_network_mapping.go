...
// So the casing of the IDs are outputed diffreently from different services in Azure, try to handle that
func unifyIdCasing(id *string) *string {
	if id == nil {
		return id
	}
	//s1 := strings.Replace(*id, "/subscriptions/", "/Subscriptions/", 1)
	s := *id
