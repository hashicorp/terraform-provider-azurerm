...
# The single source of truth for the copyplop version - the copyright CI jobs read this line.
COPYPLOP_VERSION := v0.10.0
...
tools: ## Install the tools required to develop the provider
	...
	go install github.com/YakDriver/copyplop@$(COPYPLOP_VERSION)
...
copyright: ## Check copyright headers with copyplop (config in .copyplop.yaml)
	@command -v copyplop >/dev/null || (echo "copyplop not installed. Install via 'make tools' or: go install github.com/YakDriver/copyplop@$(COPYPLOP_VERSION)" && exit 1)
	@echo "==> Checking copyright headers with copyplop..."
	@copyplop check

copyright-fix: ## Add or update copyright headers with copyplop
	@command -v copyplop >/dev/null || (echo "copyplop not installed. Install via 'make tools' or: go install github.com/YakDriver/copyplop@$(COPYPLOP_VERSION)" && exit 1)
	@echo "==> Fixing copyright headers with copyplop..."
	@copyplop fix
