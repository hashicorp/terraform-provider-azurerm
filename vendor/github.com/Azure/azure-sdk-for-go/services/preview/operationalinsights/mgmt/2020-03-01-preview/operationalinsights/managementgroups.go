...
func NewManagementGroupsClient(subscriptionID string) ManagementGroupsClient {
	return NewManagementGroupsClientWithBaseURI(DefaultBaseURI, subscriptionID)
}

// NewManagementGroupsClientWithBaseURI creates an instance of the ManagementGroupsClient client using a custom
...
func NewManagementGroupsClientWithBaseURI(baseURI string, subscriptionID string) ManagementGroupsClient {
	return ManagementGroupsClient{NewWithBaseURI(baseURI, subscriptionID)}
}
...
func (client ManagementGroupsClient) List(ctx context.Context, resourceGroupName string, workspaceName string) (result WorkspaceListManagementGroupsResult, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/ManagementGroupsClient.List")
		defer func() {
			sc := -1
	...
	if err := validation.Validate([]validation.Validation{
		...
		return result, validation.NewError("operationalinsights.ManagementGroupsClient", "List", err.Error())
	}

	req, err := client.ListPreparer(ctx, resourceGroupName, workspaceName)
	if err != nil {
		err = autorest.NewErrorWithError(err, "operationalinsights.ManagementGroupsClient", "List", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		...
		err = autorest.NewErrorWithError(err, "operationalinsights.ManagementGroupsClient", "List", resp, "Failure sending request")
		return
	}
