...
// before performing this operation. For Windows, please refer to [Create a managed image of a generalized VM in
// Azure](https://docs.microsoft.com/azure/virtual-machines/windows/capture-image-resource). For Linux, please refer to
// [How to create an image of a virtual machine or
...
// expand - the expand expression to apply on operation. 'instanceView' enables fetching run time status of all
// Virtual Machines, this can only be specified if a valid $filter option is specified
func (client VirtualMachinesClient) List(ctx context.Context, resourceGroupName string, filter string, expand ExpandTypeForListVMs) (result VirtualMachineListResultPage, err error) {
	if tracing.IsEnabled() {
	...
	req, err := client.ListPreparer(ctx, resourceGroupName, filter, expand)
	if err != nil {
...
func (client VirtualMachinesClient) ListPreparer(ctx context.Context, resourceGroupName string, filter string, expand ExpandTypeForListVMs) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"resourceGroupName": autorest.Encode("path", resourceGroupName),
		"subscriptionId":    autorest.Encode("path", client.SubscriptionID),
	}
...
func (client VirtualMachinesClient) ListComplete(ctx context.Context, resourceGroupName string, filter string, expand ExpandTypeForListVMs) (result VirtualMachineListResultIterator, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/VirtualMachinesClient.List")
		defer func() {
			sc := -1
	...
	result.page, err = client.List(ctx, resourceGroupName, filter, expand)
	return
}
...
// expand - the expand expression to apply on operation. 'instanceView' enables fetching run time status of all
// Virtual Machines, this can only be specified if a valid $filter option is specified
func (client VirtualMachinesClient) ListAll(ctx context.Context, statusOnly string, filter string, expand ExpandTypesForListVMs) (result VirtualMachineListResultPage, err error) {
	if tracing.IsEnabled() {
	...
	req, err := client.ListAllPreparer(ctx, statusOnly, filter, expand)
	if err != nil {
...
func (client VirtualMachinesClient) ListAllPreparer(ctx context.Context, statusOnly string, filter string, expand ExpandTypesForListVMs) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"subscriptionId": autorest.Encode("path", client.SubscriptionID),
	}
...
func (client VirtualMachinesClient) ListAllComplete(ctx context.Context, statusOnly string, filter string, expand ExpandTypesForListVMs) (result VirtualMachineListResultIterator, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/VirtualMachinesClient.ListAll")
		defer func() {
			sc := -1
	...
	result.page, err = client.ListAll(ctx, statusOnly, filter, expand)
	return
}
...
// Reimage reimages (upgrade the operating system) a virtual machine which don't have a ephemeral OS disk, for virtual
// machines who have a ephemeral OS disk the virtual machine is reset to initial state. NOTE: The retaining of old OS
// disk depends on the value of deleteOption of OS disk. If deleteOption is detach, the old OS disk will be preserved
// after reimage. If deleteOption is delete, the old OS disk will be deleted after reimage. The deleteOption of the OS
