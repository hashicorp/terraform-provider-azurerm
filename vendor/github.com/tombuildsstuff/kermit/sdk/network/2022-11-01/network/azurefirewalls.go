...
// PacketCaptureMethod runs a packet capture on AzureFirewall.
// Parameters:
// resourceGroupName - the name of the resource group.
// azureFirewallName - the name of the Azure Firewall.
// parameters - parameters supplied to run packet capture on azure firewall.
func (client AzureFirewallsClient) PacketCaptureMethod(ctx context.Context, resourceGroupName string, azureFirewallName string, parameters FirewallPacketCaptureParameters) (result AzureFirewallsPacketCaptureMethodFuture, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/AzureFirewallsClient.PacketCaptureMethod")
		defer func() {
			sc := -1
			if result.FutureAPI != nil && result.FutureAPI.Response() != nil {
				sc = result.FutureAPI.Response().StatusCode
	...
	if err := validation.Validate([]validation.Validation{
		{TargetValue: azureFirewallName,
			Constraints: []validation.Constraint{{Target: "azureFirewallName", Name: validation.MaxLength, Rule: 56, Chain: nil},
				{Target: "azureFirewallName", Name: validation.MinLength, Rule: 1, Chain: nil},
				{Target: "azureFirewallName", Name: validation.Pattern, Rule: `^[a-zA-Z0-9]`, Chain: nil}}},
		{TargetValue: parameters,
			Constraints: []validation.Constraint{{Target: "parameters.FirewallPacketCaptureParametersFormat", Name: validation.Null, Rule: false,
				Chain: []validation.Constraint{{Target: "parameters.FirewallPacketCaptureParametersFormat.DurationInSeconds", Name: validation.Null, Rule: false,
					Chain: []validation.Constraint{{Target: "parameters.FirewallPacketCaptureParametersFormat.DurationInSeconds", Name: validation.InclusiveMaximum, Rule: int64(1800), Chain: nil},
						{Target: "parameters.FirewallPacketCaptureParametersFormat.DurationInSeconds", Name: validation.InclusiveMinimum, Rule: int64(30), Chain: nil},
					}},
					{Target: "parameters.FirewallPacketCaptureParametersFormat.NumberOfPacketsToCapture", Name: validation.Null, Rule: false,
						Chain: []validation.Constraint{{Target: "parameters.FirewallPacketCaptureParametersFormat.NumberOfPacketsToCapture", Name: validation.InclusiveMaximum, Rule: int64(90000), Chain: nil},
							{Target: "parameters.FirewallPacketCaptureParametersFormat.NumberOfPacketsToCapture", Name: validation.InclusiveMinimum, Rule: int64(100), Chain: nil},
						}},
				}}}}}); err != nil {
		return result, validation.NewError("network.AzureFirewallsClient", "PacketCaptureMethod", err.Error())
	}

	req, err := client.PacketCaptureMethodPreparer(ctx, resourceGroupName, azureFirewallName, parameters)
	if err != nil {
		err = autorest.NewErrorWithError(err, "network.AzureFirewallsClient", "PacketCaptureMethod", nil, "Failure preparing request")
		return
	}

	result, err = client.PacketCaptureMethodSender(req)
	if err != nil {
		err = autorest.NewErrorWithError(err, "network.AzureFirewallsClient", "PacketCaptureMethod", result.Response(), "Failure sending request")
		return
	}
...
// PacketCaptureMethodPreparer prepares the PacketCaptureMethod request.
func (client AzureFirewallsClient) PacketCaptureMethodPreparer(ctx context.Context, resourceGroupName string, azureFirewallName string, parameters FirewallPacketCaptureParameters) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"azureFirewallName": autorest.Encode("path", azureFirewallName),
		"resourceGroupName": autorest.Encode("path", resourceGroupName),
		"subscriptionId":    autorest.Encode("path", client.SubscriptionID),
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.AsPost(),
		autorest.WithBaseURL(client.BaseURI),
		autorest.WithPathParameters("/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Network/azureFirewalls/{azureFirewallName}/packetCapture", pathParameters),
		autorest.WithJSON(parameters),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// PacketCaptureMethodSender sends the PacketCaptureMethod request. The method will close the
// http.Response Body if it receives an error.
func (client AzureFirewallsClient) PacketCaptureMethodSender(req *http.Request) (future AzureFirewallsPacketCaptureMethodFuture, err error) {
	var resp *http.Response
	future.FutureAPI = &azure.Future{}
	resp, err = client.Send(req, azure.DoRetryWithRegistration(client.Client))
	if err != nil {
...
// PacketCaptureMethodResponder handles the response to the PacketCaptureMethod request. The method always
// closes the http.Response Body.
func (client AzureFirewallsClient) PacketCaptureMethodResponder(resp *http.Response) (result autorest.Response, err error) {
	err = autorest.Respond(
		resp,
		azure.WithErrorUnlessStatusCode(http.StatusOK, http.StatusAccepted),
		autorest.ByClosing())
