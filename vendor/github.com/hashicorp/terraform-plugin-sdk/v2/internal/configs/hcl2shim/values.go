...
func ConfigValueFromHCL2Block(v cty.Value, schema *configschema.Block) map[string]interface{} {
	...
	for name := range schema.Attributes {
		...
		if name == "route" {
			ret[name] = ConfigValueFromHCL2ForRoute(av)
		} else {
...
func ConfigValueFromHCL2ForRoute(v cty.Value) interface{} {
	if !v.IsKnown() {
		//panic(fmt.Sprintf("Type? %+v", v.Type().FriendlyName()))
		return UnknownVariableValue
	}
	if v.IsNull() {
		...
		//panic(fmt.Sprintf("is V NIl? %+v", v))
		return nil
	}
	switch v.Type() {
	case cty.Bool:
	...
	if v.Type().IsListType() || v.Type().IsSetType() || v.Type().IsTupleType() {
		...
		for it.Next() {
			_, ev := it.Element()
			l = append(l, ConfigValueFromHCL2ForRoute(ev))
		}
		return l
	}
	...
	if v.Type().IsMapType() || v.Type().IsObjectType() {
		...
		for it.Next() {
			ek, ev := it.Element()
			cv := ConfigValueFromHCL2ForRoute(ev)
			if cv != nil {
				l[ek.AsString()] = cv
			}
		}
