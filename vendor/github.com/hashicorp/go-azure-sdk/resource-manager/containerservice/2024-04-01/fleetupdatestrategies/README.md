...
// alternatively `client.ListByFleet(ctx, id, fleetupdatestrategies.DefaultListByFleetOperationOptions())` can be used to do batched pagination
items, err := client.ListByFleetComplete(ctx, id, fleetupdatestrategies.DefaultListByFleetOperationOptions())
if err != nil {
	// handle the error
}
