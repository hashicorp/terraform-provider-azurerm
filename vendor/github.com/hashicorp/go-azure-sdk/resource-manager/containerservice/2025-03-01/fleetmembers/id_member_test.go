...
func TestParseMemberID(t *testing.T) {
	testData := []struct {
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.ContainerService/fleets/fleetName/members",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseMemberID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseMemberIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MemberId
	...
	for _, v := range testData {
		...
		actual, err := ParseMemberIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForMemberId(t *testing.T) {
	segments := MemberId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("MemberId has no segments")
	}
