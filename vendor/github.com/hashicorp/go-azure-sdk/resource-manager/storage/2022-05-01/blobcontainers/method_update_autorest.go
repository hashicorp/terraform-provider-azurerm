...
func (c BlobContainersClient) Update(ctx context.Context, id commonids.StorageContainerId, input BlobContainer) (result UpdateOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "blobcontainers.BlobContainersClient", "Update", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForUpdate(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "blobcontainers.BlobContainersClient", "Update", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c BlobContainersClient) responderForUpdate(resp *http.Response) (result UpdateOperationResponse, err error) {
