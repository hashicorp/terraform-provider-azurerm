...
func PossibleValuesForZonePlacementPolicy() []string {
	return []string{
		string(ZonePlacementPolicyAny),
		string(ZonePlacementPolicyNone),
	}
}

func (s *ZonePlacementPolicy) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseZonePlacementPolicy(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseZonePlacementPolicy(input string) (*ZonePlacementPolicy, error) {
	vals := map[string]ZonePlacementPolicy{
		"any":  ZonePlacementPolicyAny,
		"none": ZonePlacementPolicyNone,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
