...
func (c AppPlatformClient) ConfigurationServicesValidateResource(ctx context.Context, id ConfigurationServiceId, input ConfigurationServiceResource) (result ConfigurationServicesValidateResourceOperationResponse, err error) {
	opts := client.RequestOptions{
		...
		Path:       fmt.Sprintf("%s/validateResource", id.ID()),
	}
...
func (c AppPlatformClient) ConfigurationServicesValidateResourceCallbackThenPoll(ctx context.Context, id ConfigurationServiceId, input ConfigurationServiceResource, callback func() error) error {
	...
	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after ConfigurationServicesValidateResource: %+v", err)
	}
