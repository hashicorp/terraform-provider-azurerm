...
func ParseApplicationDefinitionID(input string) (*ApplicationDefinitionId, error) {
	...
	if id.ApplicationDefinitionName, ok = parsed.Parsed["applicationDefinitionName"]; !ok {
		return nil, fmt.Errorf("the segment 'applicationDefinitionName' was not found in the resource id %q", input)
	}
