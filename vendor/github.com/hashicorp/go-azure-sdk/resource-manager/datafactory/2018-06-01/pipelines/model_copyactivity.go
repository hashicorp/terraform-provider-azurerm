...
var _ json.Marshaler = CopyActivity{}

func (s CopyActivity) MarshalJSON() ([]byte, error) {
	type wrapper CopyActivity
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling CopyActivity: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling CopyActivity: %+v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("re-marshaling CopyActivity: %+v", err)
	}
