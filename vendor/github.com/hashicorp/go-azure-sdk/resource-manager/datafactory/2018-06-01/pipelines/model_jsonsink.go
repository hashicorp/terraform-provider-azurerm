...
var _ json.Marshaler = JsonSink{}

func (s JsonSink) MarshalJSON() ([]byte, error) {
	type wrapper JsonSink
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling JsonSink: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling JsonSink: %+v", err)
	}

	decoded["type"] = "JsonSink"

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling JsonSink: %+v", err)
	}
