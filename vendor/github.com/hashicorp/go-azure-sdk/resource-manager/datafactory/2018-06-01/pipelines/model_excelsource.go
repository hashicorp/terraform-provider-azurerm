...
func (s ExcelSource) MarshalJSON() ([]byte, error) {
	type wrapper ExcelSource
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling ExcelSource: %+v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("re-marshaling ExcelSource: %+v", err)
	}
