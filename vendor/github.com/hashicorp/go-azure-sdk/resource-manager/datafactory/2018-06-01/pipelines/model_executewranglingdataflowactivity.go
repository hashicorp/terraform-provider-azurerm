...
var _ Activity = ExecuteWranglingDataflowActivity{}
...
func (s ExecuteWranglingDataflowActivity) MarshalJSON() ([]byte, error) {
	type wrapper ExecuteWranglingDataflowActivity
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling ExecuteWranglingDataflowActivity: %+v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("re-marshaling ExecuteWranglingDataflowActivity: %+v", err)
	}
