...
var _ CopySink = LakeHouseTableSink{}
...
var _ json.Marshaler = LakeHouseTableSink{}

func (s LakeHouseTableSink) MarshalJSON() ([]byte, error) {
	type wrapper LakeHouseTableSink
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling LakeHouseTableSink: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling LakeHouseTableSink: %+v", err)
	}

	decoded["type"] = "LakeHouseTableSink"

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling LakeHouseTableSink: %+v", err)
	}
