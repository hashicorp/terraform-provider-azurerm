...
var _ json.Marshaler = SalesforceSink{}

func (s SalesforceSink) MarshalJSON() ([]byte, error) {
	type wrapper SalesforceSink
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SalesforceSink: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SalesforceSink: %+v", err)
	}

	decoded["type"] = "SalesforceSink"

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SalesforceSink: %+v", err)
	}
