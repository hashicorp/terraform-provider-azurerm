...
var _ json.Marshaler = InformixSink{}

func (s InformixSink) MarshalJSON() ([]byte, error) {
	type wrapper InformixSink
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling InformixSink: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling InformixSink: %+v", err)
	}

	decoded["type"] = "InformixSink"

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling InformixSink: %+v", err)
	}
