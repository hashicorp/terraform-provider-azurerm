...
var _ json.Marshaler = WaitActivity{}

func (s WaitActivity) MarshalJSON() ([]byte, error) {
	type wrapper WaitActivity
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling WaitActivity: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling WaitActivity: %+v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("re-marshaling WaitActivity: %+v", err)
	}
