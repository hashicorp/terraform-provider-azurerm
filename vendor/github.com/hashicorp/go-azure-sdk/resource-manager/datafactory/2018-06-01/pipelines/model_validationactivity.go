...
var _ Activity = ValidationActivity{}
...
var _ json.Marshaler = ValidationActivity{}

func (s ValidationActivity) MarshalJSON() ([]byte, error) {
	type wrapper ValidationActivity
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling ValidationActivity: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling ValidationActivity: %+v", err)
	}

	decoded["type"] = "Validation"

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling ValidationActivity: %+v", err)
	}
