...
var _ Activity = ForEachActivity{}
...
var _ json.Marshaler = ForEachActivity{}

func (s ForEachActivity) MarshalJSON() ([]byte, error) {
	type wrapper ForEachActivity
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling ForEachActivity: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling ForEachActivity: %+v", err)
	}

	decoded["type"] = "ForEach"

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling ForEachActivity: %+v", err)
	}
