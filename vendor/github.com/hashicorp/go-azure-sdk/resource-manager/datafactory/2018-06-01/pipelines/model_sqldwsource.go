...
var _ json.Marshaler = SqlDWSource{}

func (s SqlDWSource) MarshalJSON() ([]byte, error) {
	type wrapper SqlDWSource
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SqlDWSource: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SqlDWSource: %+v", err)
	}

	decoded["type"] = "SqlDWSource"

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SqlDWSource: %+v", err)
	}
