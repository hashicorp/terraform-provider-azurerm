...
func (s SqlMISink) MarshalJSON() ([]byte, error) {
	type wrapper SqlMISink
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SqlMISink: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SqlMISink: %+v", err)
	}

	decoded["type"] = "SqlMISink"

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SqlMISink: %+v", err)
	}
