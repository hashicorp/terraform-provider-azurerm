...
var _ json.Marshaler = BlobSink{}

func (s BlobSink) MarshalJSON() ([]byte, error) {
	type wrapper BlobSink
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling BlobSink: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling BlobSink: %+v", err)
	}

	decoded["type"] = "BlobSink"

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling BlobSink: %+v", err)
	}
