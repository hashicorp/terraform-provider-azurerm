...
func (s AmazonS3CompatibleReadSettings) MarshalJSON() ([]byte, error) {
	type wrapper AmazonS3CompatibleReadSettings
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling AmazonS3CompatibleReadSettings: %+v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("re-marshaling AmazonS3CompatibleReadSettings: %+v", err)
	}
