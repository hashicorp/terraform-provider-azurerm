...
var _ json.Marshaler = FilterActivity{}

func (s FilterActivity) MarshalJSON() ([]byte, error) {
	type wrapper FilterActivity
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling FilterActivity: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling FilterActivity: %+v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("re-marshaling FilterActivity: %+v", err)
	}
