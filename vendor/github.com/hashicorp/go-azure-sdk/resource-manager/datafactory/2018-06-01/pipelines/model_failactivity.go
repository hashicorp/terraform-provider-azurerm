...
var _ json.Marshaler = FailActivity{}

func (s FailActivity) MarshalJSON() ([]byte, error) {
	type wrapper FailActivity
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling FailActivity: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling FailActivity: %+v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("re-marshaling FailActivity: %+v", err)
	}
