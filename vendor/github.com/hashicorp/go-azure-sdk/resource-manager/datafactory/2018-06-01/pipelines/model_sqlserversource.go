...
var _ CopySource = SqlServerSource{}
...
var _ json.Marshaler = SqlServerSource{}

func (s SqlServerSource) MarshalJSON() ([]byte, error) {
	type wrapper SqlServerSource
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling SqlServerSource: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling SqlServerSource: %+v", err)
	}

	decoded["type"] = "SqlServerSource"

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling SqlServerSource: %+v", err)
	}
