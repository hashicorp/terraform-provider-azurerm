...
var _ json.Marshaler = UntilActivity{}

func (s UntilActivity) MarshalJSON() ([]byte, error) {
	type wrapper UntilActivity
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling UntilActivity: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling UntilActivity: %+v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("re-marshaling UntilActivity: %+v", err)
	}
