...
var _ json.Marshaler = OrcDataset{}

func (s OrcDataset) MarshalJSON() ([]byte, error) {
	type wrapper OrcDataset
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling OrcDataset: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling OrcDataset: %+v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("re-marshaling OrcDataset: %+v", err)
	}
