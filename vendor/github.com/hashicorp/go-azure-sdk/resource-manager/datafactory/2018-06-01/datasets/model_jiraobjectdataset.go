...
var _ json.Marshaler = JiraObjectDataset{}

func (s JiraObjectDataset) MarshalJSON() ([]byte, error) {
	type wrapper JiraObjectDataset
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling JiraObjectDataset: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling JiraObjectDataset: %+v", err)
	}

	decoded["type"] = "JiraObject"

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling JiraObjectDataset: %+v", err)
	}
