...
var _ json.Marshaler = XmlDataset{}

func (s XmlDataset) MarshalJSON() ([]byte, error) {
	type wrapper XmlDataset
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling XmlDataset: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling XmlDataset: %+v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("re-marshaling XmlDataset: %+v", err)
	}
