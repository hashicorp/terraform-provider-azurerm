...
func (c HdinsightsClient) ClusterPoolsUpdateTagsThenPoll(ctx context.Context, id ClusterPoolId, input TagsObject) error {
	result, err := c.ClusterPoolsUpdateTags(ctx, id, input)
	if err != nil {
		return fmt.Errorf("performing ClusterPoolsUpdateTags: %+v", err)
	}

	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after ClusterPoolsUpdateTags: %+v", err)
	}
