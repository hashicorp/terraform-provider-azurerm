...
func (c VirtualWANsClient) RoutingIntentDeleteThenPoll(ctx context.Context, id RoutingIntentId) error {
	result, err := c.RoutingIntentDelete(ctx, id)
	if err != nil {
		return fmt.Errorf("performing RoutingIntentDelete: %+v", err)
	}

	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after RoutingIntentDelete: %+v", err)
	}
