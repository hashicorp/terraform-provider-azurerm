...
const (
	...
	VirtualNetworkApplianceIPVersionTypeIPvFour   VirtualNetworkApplianceIPVersionType = "IPv4"
)

func PossibleValuesForVirtualNetworkApplianceIPVersionType() []string {
	return []string{
		string(VirtualNetworkApplianceIPVersionTypeDualStack),
		string(VirtualNetworkApplianceIPVersionTypeIPvFour),
	}
}

func (s *VirtualNetworkApplianceIPVersionType) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseVirtualNetworkApplianceIPVersionType(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseVirtualNetworkApplianceIPVersionType(input string) (*VirtualNetworkApplianceIPVersionType, error) {
	vals := map[string]VirtualNetworkApplianceIPVersionType{
		"dualstack": VirtualNetworkApplianceIPVersionTypeDualStack,
		"ipv4":      VirtualNetworkApplianceIPVersionTypeIPvFour,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
	...
	// otherwise presume it's an undefined value and best-effort it
	out := VirtualNetworkApplianceIPVersionType(input)
	return &out, nil
}
