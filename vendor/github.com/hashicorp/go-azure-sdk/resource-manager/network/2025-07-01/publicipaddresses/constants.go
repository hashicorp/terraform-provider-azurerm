...
func PossibleValuesForIsRollback() []string {
	return []string{
		string(IsRollbackFalse),
		string(IsRollbackTrue),
	}
}

func (s *IsRollback) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseIsRollback(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseIsRollback(input string) (*IsRollback, error) {
	vals := map[string]IsRollback{
		"false": IsRollbackFalse,
		"true":  IsRollbackTrue,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
