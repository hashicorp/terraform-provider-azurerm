...
func PossibleValuesForApplicationGatewayWafRuleSensitivityTypes() []string {
	return []string{
		string(ApplicationGatewayWafRuleSensitivityTypesHigh),
		string(ApplicationGatewayWafRuleSensitivityTypesLow),
		string(ApplicationGatewayWafRuleSensitivityTypesMedium),
	}
}

func (s *ApplicationGatewayWafRuleSensitivityTypes) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseApplicationGatewayWafRuleSensitivityTypes(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseApplicationGatewayWafRuleSensitivityTypes(input string) (*ApplicationGatewayWafRuleSensitivityTypes, error) {
	vals := map[string]ApplicationGatewayWafRuleSensitivityTypes{
		"high":   ApplicationGatewayWafRuleSensitivityTypesHigh,
		"low":    ApplicationGatewayWafRuleSensitivityTypesLow,
		"medium": ApplicationGatewayWafRuleSensitivityTypesMedium,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
	...
	// otherwise presume it's an undefined value and best-effort it
	out := ApplicationGatewayWafRuleSensitivityTypes(input)
	return &out, nil
}
