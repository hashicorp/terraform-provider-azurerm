...
func TestParseApplicationGatewayID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseApplicationGatewayID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseApplicationGatewayIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ApplicationGatewayId
	...
	for _, v := range testData {
		...
		actual, err := ParseApplicationGatewayIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForApplicationGatewayId(t *testing.T) {
	segments := ApplicationGatewayId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ApplicationGatewayId has no segments")
	}
