...
var _ json.Unmarshaler = &ConnectToSourceSqlServerSyncTaskProperties{}
...
func (s *ConnectToSourceSqlServerSyncTaskProperties) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling ConnectToSourceSqlServerSyncTaskProperties into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["commands"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Commands' for 'ConnectToSourceSqlServerSyncTaskProperties': %+v", i, err)
			}
	...
	if v, ok := temp["output"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Output' for 'ConnectToSourceSqlServerSyncTaskProperties': %+v", i, err)
			}
			output = append(output, impl)
		}
		s.Output = &output
