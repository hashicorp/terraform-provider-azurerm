...
var _ json.Unmarshaler = &ConnectToTargetSqlDbTaskProperties{}
...
func (s *ConnectToTargetSqlDbTaskProperties) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling ConnectToTargetSqlDbTaskProperties into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["commands"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Commands' for 'ConnectToTargetSqlDbTaskProperties': %+v", i, err)
			}
