...
var _ json.Unmarshaler = &MigrateSqlServerSqlMITaskProperties{}
...
func (s *MigrateSqlServerSqlMITaskProperties) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling MigrateSqlServerSqlMITaskProperties into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["commands"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Commands' for 'MigrateSqlServerSqlMITaskProperties': %+v", i, err)
			}
	...
	if v, ok := temp["output"]; ok {
		...
		output := make([]MigrateSqlServerSqlMITaskOutput, 0)
		for i, val := range listTemp {
			impl, err := UnmarshalMigrateSqlServerSqlMITaskOutputImplementation(val)
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Output' for 'MigrateSqlServerSqlMITaskProperties': %+v", i, err)
			}
			output = append(output, impl)
		}
		s.Output = &output
