...
var _ json.Unmarshaler = &MigrateOracleAzureDbForPostgreSqlSyncTaskProperties{}
...
func (s *MigrateOracleAzureDbForPostgreSqlSyncTaskProperties) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling MigrateOracleAzureDbForPostgreSqlSyncTaskProperties into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["commands"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Commands' for 'MigrateOracleAzureDbForPostgreSqlSyncTaskProperties': %+v", i, err)
			}
	...
	if v, ok := temp["output"]; ok {
		...
		output := make([]MigrateOracleAzureDbPostgreSqlSyncTaskOutput, 0)
		for i, val := range listTemp {
			impl, err := UnmarshalMigrateOracleAzureDbPostgreSqlSyncTaskOutputImplementation(val)
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Output' for 'MigrateOracleAzureDbForPostgreSqlSyncTaskProperties': %+v", i, err)
			}
			output = append(output, impl)
		}
		s.Output = &output
