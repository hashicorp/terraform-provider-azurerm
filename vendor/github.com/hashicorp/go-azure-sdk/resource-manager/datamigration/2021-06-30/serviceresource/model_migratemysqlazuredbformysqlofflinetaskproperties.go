...
var _ json.Unmarshaler = &MigrateMySqlAzureDbForMySqlOfflineTaskProperties{}
...
func (s *MigrateMySqlAzureDbForMySqlOfflineTaskProperties) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling MigrateMySqlAzureDbForMySqlOfflineTaskProperties into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["commands"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Commands' for 'MigrateMySqlAzureDbForMySqlOfflineTaskProperties': %+v", i, err)
			}
	...
	if v, ok := temp["output"]; ok {
		...
		output := make([]MigrateMySqlAzureDbForMySqlOfflineTaskOutput, 0)
		for i, val := range listTemp {
			impl, err := UnmarshalMigrateMySqlAzureDbForMySqlOfflineTaskOutputImplementation(val)
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Output' for 'MigrateMySqlAzureDbForMySqlOfflineTaskProperties': %+v", i, err)
			}
			output = append(output, impl)
		}
		s.Output = &output
