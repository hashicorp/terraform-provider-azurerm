...
var _ json.Unmarshaler = &MigrateSsisTaskProperties{}
...
func (s *MigrateSsisTaskProperties) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling MigrateSsisTaskProperties into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["commands"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Commands' for 'MigrateSsisTaskProperties': %+v", i, err)
			}
	...
	if v, ok := temp["output"]; ok {
		...
		output := make([]MigrateSsisTaskOutput, 0)
		for i, val := range listTemp {
			impl, err := UnmarshalMigrateSsisTaskOutputImplementation(val)
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Output' for 'MigrateSsisTaskProperties': %+v", i, err)
			}
			output = append(output, impl)
		}
		s.Output = &output
