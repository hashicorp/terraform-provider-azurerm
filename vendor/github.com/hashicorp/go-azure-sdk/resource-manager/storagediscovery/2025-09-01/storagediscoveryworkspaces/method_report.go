...
type ReportOperationResponse struct {
...
// Report ...
func (c StorageDiscoveryWorkspacesClient) Report(ctx context.Context, id ProviderStorageDiscoveryWorkspaceId, input GetReportContent) (result ReportOperationResponse, err error) {
	opts := client.RequestOptions{
		...
		Path:       fmt.Sprintf("%s/report", id.ID()),
	}
...
func (c StorageDiscoveryWorkspacesClient) ReportThenPoll(ctx context.Context, id ProviderStorageDiscoveryWorkspaceId, input GetReportContent) error {
	...
	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after Report: %+v", err)
	}
