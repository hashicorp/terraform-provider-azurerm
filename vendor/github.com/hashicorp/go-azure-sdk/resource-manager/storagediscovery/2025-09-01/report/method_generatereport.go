...
type GenerateReportOperationResponse struct {
...
// GenerateReport ...
func (c ReportClient) GenerateReport(ctx context.Context, id ReportId, input GetReportContent) (result GenerateReportOperationResponse, err error) {
	opts := client.RequestOptions{
		...
		Path:       fmt.Sprintf("%s/generateReport", id.ID()),
	}
...
func (c ReportClient) GenerateReportThenPoll(ctx context.Context, id ReportId, input GetReportContent) error {
	...
	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after GenerateReport: %+v", err)
	}
