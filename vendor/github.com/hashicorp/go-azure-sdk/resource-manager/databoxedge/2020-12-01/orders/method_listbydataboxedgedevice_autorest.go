...
func (c OrdersClient) ListByDataBoxEdgeDevice(ctx context.Context, id DataBoxEdgeDeviceId) (resp ListByDataBoxEdgeDeviceOperationResponse, err error) {
	req, err := c.preparerForListByDataBoxEdgeDevice(ctx, id)
	if err != nil {
		err = autorest.NewErrorWithError(err, "orders.OrdersClient", "ListByDataBoxEdgeDevice", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "orders.OrdersClient", "ListByDataBoxEdgeDevice", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForListByDataBoxEdgeDevice(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "orders.OrdersClient", "ListByDataBoxEdgeDevice", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c OrdersClient) preparerForListByDataBoxEdgeDevice(ctx context.Context, id DataBoxEdgeDeviceId) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/orders", id.ID())),
...
func (c OrdersClient) preparerForListByDataBoxEdgeDeviceWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c OrdersClient) responderForListByDataBoxEdgeDevice(resp *http.Response) (result ListByDataBoxEdgeDeviceOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListByDataBoxEdgeDeviceOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "orders.OrdersClient", "ListByDataBoxEdgeDevice", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForListByDataBoxEdgeDevice(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "orders.OrdersClient", "ListByDataBoxEdgeDevice", result.HttpResponse, "Failure responding to request")
				return
			}
...
func (c OrdersClient) ListByDataBoxEdgeDeviceCompleteMatchingPredicate(ctx context.Context, id DataBoxEdgeDeviceId, predicate OrderOperationPredicate) (resp ListByDataBoxEdgeDeviceCompleteResult, err error) {
	items := make([]Order, 0)
