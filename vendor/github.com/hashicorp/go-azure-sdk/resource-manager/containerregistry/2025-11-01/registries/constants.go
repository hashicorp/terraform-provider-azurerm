...
func PossibleValuesForRoleAssignmentMode() []string {
	return []string{
		string(RoleAssignmentModeAbacRepositoryPermissions),
		string(RoleAssignmentModeLegacyRegistryPermissions),
	}
}

func (s *RoleAssignmentMode) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseRoleAssignmentMode(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseRoleAssignmentMode(input string) (*RoleAssignmentMode, error) {
	vals := map[string]RoleAssignmentMode{
		"abacrepositorypermissions": RoleAssignmentModeAbacRepositoryPermissions,
		"legacyregistrypermissions": RoleAssignmentModeLegacyRegistryPermissions,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
