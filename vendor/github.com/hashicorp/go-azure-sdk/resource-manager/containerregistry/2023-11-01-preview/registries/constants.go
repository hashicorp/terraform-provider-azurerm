...
func PossibleValuesForMetadataSearch() []string {
	return []string{
		string(MetadataSearchDisabled),
		string(MetadataSearchEnabled),
	}
}

func (s *MetadataSearch) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseMetadataSearch(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseMetadataSearch(input string) (*MetadataSearch, error) {
	vals := map[string]MetadataSearch{
		"disabled": MetadataSearchDisabled,
		"enabled":  MetadataSearchEnabled,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
