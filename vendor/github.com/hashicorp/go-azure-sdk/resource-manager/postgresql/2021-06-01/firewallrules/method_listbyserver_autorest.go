...
func (c FirewallRulesClient) ListByServer(ctx context.Context, id FlexibleServerId) (resp ListByServerOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "firewallrules.FirewallRulesClient", "ListByServer", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "firewallrules.FirewallRulesClient", "ListByServer", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForListByServer(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "firewallrules.FirewallRulesClient", "ListByServer", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c FirewallRulesClient) preparerForListByServerWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c FirewallRulesClient) responderForListByServer(resp *http.Response) (result ListByServerOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListByServerOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "firewallrules.FirewallRulesClient", "ListByServer", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForListByServer(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "firewallrules.FirewallRulesClient", "ListByServer", result.HttpResponse, "Failure responding to request")
				return
			}
