...
type GenerateAkvCredentialsOperationResponse struct {
...
func (c BucketsClient) GenerateAkvCredentials(ctx context.Context, id BucketId, input BucketCredentialsExpiry) (result GenerateAkvCredentialsOperationResponse, err error) {
	opts := client.RequestOptions{
		...
		Path:       fmt.Sprintf("%s/generateAkvCredentials", id.ID()),
	}
...
func (c BucketsClient) GenerateAkvCredentialsThenPoll(ctx context.Context, id BucketId, input BucketCredentialsExpiry) error {
	...
	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after GenerateAkvCredentials: %+v", err)
	}
