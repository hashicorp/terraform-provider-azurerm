...
type PerformReplicationTransferOperationResponse struct {
...
func (c VolumesClient) PerformReplicationTransfer(ctx context.Context, id VolumeId) (result PerformReplicationTransferOperationResponse, err error) {
	opts := client.RequestOptions{
		...
		Path:       fmt.Sprintf("%s/performReplicationTransfer", id.ID()),
	}
...
func (c VolumesClient) PerformReplicationTransferCallbackThenPoll(ctx context.Context, id VolumeId, callback func() error) error {
	...
	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after PerformReplicationTransfer: %+v", err)
	}
