...
## `github.com/hashicorp/go-azure-sdk/resource-manager/netapp/2025-12-01/volumes` Documentation

The `volumes` SDK allows for interaction with Azure Resource Manager `netapp` (API Version `2025-12-01`).
...
```go
import "github.com/hashicorp/go-azure-sdk/resource-manager/netapp/2025-12-01/volumes"
```
...
### Example Usage: `VolumesClient.ListQuotaReport`

```go
ctx := context.TODO()
id := volumes.NewVolumeID("12345678-1234-9876-4563-123456789012", "example-resource-group", "netAppAccountName", "capacityPoolName", "volumeName")

payload := volumes.QuotaReportFilterRequest{
	// ...
}


if err := client.ListQuotaReportThenPoll(ctx, id, payload); err != nil {
	// handle the error
}
```
...
payload := volumes.ListReplicationsRequest{
	// ...
}


// alternatively `client.ListReplications(ctx, id, payload)` can be used to do batched pagination
items, err := client.ListReplicationsComplete(ctx, id, payload)
if err != nil {
	// handle the error
}
for _, item := range items {
