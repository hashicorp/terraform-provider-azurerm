...
type AFDCustomDomainsCreateOperationResponse struct {
...
func (c AFDDomainsClient) AFDCustomDomainsCreate(ctx context.Context, id CustomDomainId, input AFDDomain) (result AFDCustomDomainsCreateOperationResponse, err error) {
...
func (c AFDDomainsClient) AFDCustomDomainsCreateCallbackThenPoll(ctx context.Context, id CustomDomainId, input AFDDomain, callback func() error) error {
	...
	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after AFDCustomDomainsCreate: %+v", err)
	}
