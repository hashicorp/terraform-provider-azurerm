...
func PossibleValuesForGeoDRRoleType() []string {
	return []string{
		string(GeoDRRoleTypePrimary),
		string(GeoDRRoleTypeSecondary),
	}
}

func (s *GeoDRRoleType) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseGeoDRRoleType(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseGeoDRRoleType(input string) (*GeoDRRoleType, error) {
	vals := map[string]GeoDRRoleType{
		"primary":   GeoDRRoleTypePrimary,
		"secondary": GeoDRRoleTypeSecondary,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
...
func parseIPAddressType(input string) (*IPAddressType, error) {
	vals := map[string]IPAddressType{
		"dualstack": IPAddressTypeDualStack,
...
const (
	...
	TlsVersionOnePointThree TlsVersion = "1.3"
...
func PossibleValuesForTlsVersion() []string {
	return []string{
		...
		string(TlsVersionOnePointThree),
