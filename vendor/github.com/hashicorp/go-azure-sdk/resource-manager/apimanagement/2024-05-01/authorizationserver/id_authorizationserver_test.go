...
func TestParseAuthorizationServerID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseAuthorizationServerID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseAuthorizationServerIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *AuthorizationServerId
	...
	for _, v := range testData {
		...
		actual, err := ParseAuthorizationServerIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForAuthorizationServerId(t *testing.T) {
	segments := AuthorizationServerId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("AuthorizationServerId has no segments")
	}
