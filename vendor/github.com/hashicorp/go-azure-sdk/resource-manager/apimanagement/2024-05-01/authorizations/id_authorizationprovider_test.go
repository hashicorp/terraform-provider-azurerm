...
func TestNewAuthorizationProviderID(t *testing.T) {
	id := NewAuthorizationProviderID("12345678-1234-9876-4563-123456789012", "example-resource-group", "serviceName", "authorizationProviderId")
...
func TestFormatAuthorizationProviderID(t *testing.T) {
	actual := NewAuthorizationProviderID("12345678-1234-9876-4563-123456789012", "example-resource-group", "serviceName", "authorizationProviderId").ID()
	expected := "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.ApiManagement/service/serviceName/authorizationProviders/authorizationProviderId"
	if actual != expected {
		t.Fatalf("Expected the Formatted ID to be %q but got %q", expected, actual)
	}
}

func TestParseAuthorizationProviderID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *AuthorizationProviderId
	}{
		{
			// Incomplete URI
			Input: "",
		...
		},
		{
			...
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.ApiManagement/service/serviceName/authorizationProviders/authorizationProviderId/extra",
			Error: true,
		},
	}
	for _, v := range testData {
		...
		actual, err := ParseAuthorizationProviderID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseAuthorizationProviderIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *AuthorizationProviderId
	...
	for _, v := range testData {
		...
		actual, err := ParseAuthorizationProviderIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForAuthorizationProviderId(t *testing.T) {
	segments := AuthorizationProviderId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("AuthorizationProviderId has no segments")
	}
