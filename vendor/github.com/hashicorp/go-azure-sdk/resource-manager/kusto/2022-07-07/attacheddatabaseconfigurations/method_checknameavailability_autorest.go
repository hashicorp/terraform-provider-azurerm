...
func (c AttachedDatabaseConfigurationsClient) CheckNameAvailability(ctx context.Context, id ClusterId, input AttachedDatabaseConfigurationsCheckNameRequest) (result CheckNameAvailabilityOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "attacheddatabaseconfigurations.AttachedDatabaseConfigurationsClient", "CheckNameAvailability", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForCheckNameAvailability(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "attacheddatabaseconfigurations.AttachedDatabaseConfigurationsClient", "CheckNameAvailability", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c AttachedDatabaseConfigurationsClient) preparerForCheckNameAvailability(ctx context.Context, id ClusterId, input AttachedDatabaseConfigurationsCheckNameRequest) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/attachedDatabaseConfigurationCheckNameAvailability", id.ID())),
...
func (c AttachedDatabaseConfigurationsClient) responderForCheckNameAvailability(resp *http.Response) (result CheckNameAvailabilityOperationResponse, err error) {
