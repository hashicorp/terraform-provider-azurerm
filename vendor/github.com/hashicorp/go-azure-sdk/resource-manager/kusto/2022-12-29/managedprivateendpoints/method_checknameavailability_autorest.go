...
func (c ManagedPrivateEndpointsClient) CheckNameAvailability(ctx context.Context, id ClusterId, input ManagedPrivateEndpointsCheckNameRequest) (result CheckNameAvailabilityOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "managedprivateendpoints.ManagedPrivateEndpointsClient", "CheckNameAvailability", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForCheckNameAvailability(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "managedprivateendpoints.ManagedPrivateEndpointsClient", "CheckNameAvailability", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c ManagedPrivateEndpointsClient) preparerForCheckNameAvailability(ctx context.Context, id ClusterId, input ManagedPrivateEndpointsCheckNameRequest) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/managedPrivateEndpointsCheckNameAvailability", id.ID())),
...
func (c ManagedPrivateEndpointsClient) responderForCheckNameAvailability(resp *http.Response) (result CheckNameAvailabilityOperationResponse, err error) {
