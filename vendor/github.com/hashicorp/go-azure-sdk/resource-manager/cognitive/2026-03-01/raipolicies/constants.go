...
func PossibleValuesForRaiActionType() []string {
	return []string{
		string(RaiActionTypeANNOTATING),
		string(RaiActionTypeBLOCKING),
		string(RaiActionTypeHITL),
		string(RaiActionTypeNone),
		string(RaiActionTypeRETRY),
	}
}

func (s *RaiActionType) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseRaiActionType(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseRaiActionType(input string) (*RaiActionType, error) {
	vals := map[string]RaiActionType{
		"annotating": RaiActionTypeANNOTATING,
		"blocking":   RaiActionTypeBLOCKING,
		"hitl":       RaiActionTypeHITL,
		"none":       RaiActionTypeNone,
		"retry":      RaiActionTypeRETRY,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
