...
func UnmarshalBackupDatasourceParametersImplementation(input []byte) (BackupDatasourceParameters, error) {
	...
	if strings.EqualFold(value, "GenericBackupDatasourceParameters") {
		var out GenericBackupDatasourceParameters
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into GenericBackupDatasourceParameters: %+v", err)
		}
		return out, nil
	}
