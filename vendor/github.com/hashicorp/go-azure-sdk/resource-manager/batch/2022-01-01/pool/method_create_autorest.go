...
func (c PoolClient) Create(ctx context.Context, id PoolId, input Pool, options CreateOperationOptions) (result CreateOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "pool.PoolClient", "Create", nil, "Failure preparing request")
		return
	}

	result.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "pool.PoolClient", "Create", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForCreate(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "pool.PoolClient", "Create", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c PoolClient) preparerForCreate(ctx context.Context, id PoolId, input Pool, options CreateOperationOptions) (*http.Request, error) {
...
func (c PoolClient) responderForCreate(resp *http.Response) (result CreateOperationResponse, err error) {
