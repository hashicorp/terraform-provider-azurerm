...
// ListByBatchAccount ...
func (c PoolClient) ListByBatchAccount(ctx context.Context, id BatchAccountId, options ListByBatchAccountOperationOptions) (resp ListByBatchAccountOperationResponse, err error) {
	req, err := c.preparerForListByBatchAccount(ctx, id, options)
	if err != nil {
		err = autorest.NewErrorWithError(err, "pool.PoolClient", "ListByBatchAccount", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "pool.PoolClient", "ListByBatchAccount", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForListByBatchAccount(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "pool.PoolClient", "ListByBatchAccount", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c PoolClient) preparerForListByBatchAccount(ctx context.Context, id BatchAccountId, options ListByBatchAccountOperationOptions) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/pools", id.ID())),
...
func (c PoolClient) preparerForListByBatchAccountWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c PoolClient) responderForListByBatchAccount(resp *http.Response) (result ListByBatchAccountOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListByBatchAccountOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "pool.PoolClient", "ListByBatchAccount", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForListByBatchAccount(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "pool.PoolClient", "ListByBatchAccount", result.HttpResponse, "Failure responding to request")
				return
			}
...
func (c PoolClient) ListByBatchAccountCompleteMatchingPredicate(ctx context.Context, id BatchAccountId, options ListByBatchAccountOperationOptions, predicate PoolOperationPredicate) (resp ListByBatchAccountCompleteResult, err error) {
	items := make([]Pool, 0)
