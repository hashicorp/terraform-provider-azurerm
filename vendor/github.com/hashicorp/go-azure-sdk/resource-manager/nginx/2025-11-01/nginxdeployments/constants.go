...
func PossibleValuesForActivationState() []string {
	return []string{
		string(ActivationStateDisabled),
		string(ActivationStateEnabled),
	}
}
...
func parseActivationState(input string) (*ActivationState, error) {
	vals := map[string]ActivationState{
		"disabled": ActivationStateDisabled,
		"enabled":  ActivationStateEnabled,
	}
...
func (s *NginxPrivateIPAllocationMethod) UnmarshalJSON(bytes []byte) error {
	...
	out, err := parseNginxPrivateIPAllocationMethod(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
