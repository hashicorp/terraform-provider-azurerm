...
func (c GalleriesClient) List(ctx context.Context, id commonids.SubscriptionId) (resp ListOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "galleries.GalleriesClient", "List", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "galleries.GalleriesClient", "List", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForList(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "galleries.GalleriesClient", "List", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c GalleriesClient) preparerForList(ctx context.Context, id commonids.SubscriptionId) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/providers/Microsoft.Compute/galleries", id.ID())),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// preparerForListWithNextLink prepares the List request with the given nextLink token.
func (c GalleriesClient) preparerForListWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c GalleriesClient) responderForList(resp *http.Response) (result ListOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "galleries.GalleriesClient", "List", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForList(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "galleries.GalleriesClient", "List", result.HttpResponse, "Failure responding to request")
				return
			}
...
func (c GalleriesClient) ListComplete(ctx context.Context, id commonids.SubscriptionId) (ListCompleteResult, error) {
	return c.ListCompleteMatchingPredicate(ctx, id, GalleryOperationPredicate{})
}
