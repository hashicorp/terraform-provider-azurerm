...
func ParsePrivateEndpointConnectionID(input string) (*PrivateEndpointConnectionId, error) {
	...
	if id.DiskAccessName, ok = parsed.Parsed["diskAccessName"]; !ok {
		return nil, fmt.Errorf("the segment 'diskAccessName' was not found in the resource id %q", input)
	}
...
func (id PrivateEndpointConnectionId) ID() string {
	fmtString := "/subscriptions/%s/resourceGroups/%s/providers/Microsoft.Compute/diskAccesses/%s/privateEndpointConnections/%s"
	return fmt.Sprintf(fmtString, id.SubscriptionId, id.ResourceGroupName, id.DiskAccessName, id.PrivateEndpointConnectionName)
}
