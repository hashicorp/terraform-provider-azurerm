...
func (c ImagesClient) List(ctx context.Context, id commonids.SubscriptionId) (resp ListOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "images.ImagesClient", "List", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "images.ImagesClient", "List", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForList(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "images.ImagesClient", "List", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c ImagesClient) preparerForList(ctx context.Context, id commonids.SubscriptionId) (*http.Request, error) {
...
func (c ImagesClient) preparerForListWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c ImagesClient) responderForList(resp *http.Response) (result ListOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "images.ImagesClient", "List", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForList(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "images.ImagesClient", "List", result.HttpResponse, "Failure responding to request")
				return
			}
