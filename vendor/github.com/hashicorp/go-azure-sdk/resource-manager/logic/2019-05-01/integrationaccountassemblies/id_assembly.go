...
func init() {
	recaser.RegisterResourceId(&AssemblyId{})
}
...
func (id *AssemblyId) FromParseResult(input resourceids.ParseResult) error {
	...
	if id.AssemblyName, ok = input.Parsed["assemblyName"]; !ok {
		return resourceids.NewSegmentNotSpecifiedError(id, "assemblyName", input)
	}
