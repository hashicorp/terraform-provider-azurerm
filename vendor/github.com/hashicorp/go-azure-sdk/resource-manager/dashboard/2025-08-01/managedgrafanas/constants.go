...
func PossibleValuesForCreatorCanAdmin() []string {
	return []string{
		string(CreatorCanAdminDisabled),
		string(CreatorCanAdminEnabled),
	}
}

func (s *CreatorCanAdmin) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseCreatorCanAdmin(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseCreatorCanAdmin(input string) (*CreatorCanAdmin, error) {
	vals := map[string]CreatorCanAdmin{
		"disabled": CreatorCanAdminDisabled,
		"enabled":  CreatorCanAdminEnabled,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
...
func PossibleValuesForSize() []string {
	return []string{
		string(SizeXOne),
		string(SizeXTwo),
	}
