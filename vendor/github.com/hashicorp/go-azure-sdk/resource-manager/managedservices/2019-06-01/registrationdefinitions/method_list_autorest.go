...
func (c RegistrationDefinitionsClient) List(ctx context.Context, id commonids.ScopeId) (resp ListOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "registrationdefinitions.RegistrationDefinitionsClient", "List", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForList(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "registrationdefinitions.RegistrationDefinitionsClient", "List", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c RegistrationDefinitionsClient) preparerForListWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c RegistrationDefinitionsClient) responderForList(resp *http.Response) (result ListOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "registrationdefinitions.RegistrationDefinitionsClient", "List", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForList(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "registrationdefinitions.RegistrationDefinitionsClient", "List", result.HttpResponse, "Failure responding to request")
				return
			}
