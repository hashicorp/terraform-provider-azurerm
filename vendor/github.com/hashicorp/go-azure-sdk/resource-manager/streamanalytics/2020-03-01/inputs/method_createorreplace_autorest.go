...
func (c InputsClient) CreateOrReplace(ctx context.Context, id InputId, input Input, options CreateOrReplaceOperationOptions) (result CreateOrReplaceOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "inputs.InputsClient", "CreateOrReplace", nil, "Failure preparing request")
		return
	}

	result.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "inputs.InputsClient", "CreateOrReplace", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForCreateOrReplace(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "inputs.InputsClient", "CreateOrReplace", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c InputsClient) preparerForCreateOrReplace(ctx context.Context, id InputId, input Input, options CreateOrReplaceOperationOptions) (*http.Request, error) {
...
func (c InputsClient) responderForCreateOrReplace(resp *http.Response) (result CreateOrReplaceOperationResponse, err error) {
