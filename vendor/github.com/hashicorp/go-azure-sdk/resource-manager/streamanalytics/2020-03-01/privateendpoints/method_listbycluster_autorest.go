...
func (c PrivateEndpointsClient) ListByCluster(ctx context.Context, id ClusterId) (resp ListByClusterOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "privateendpoints.PrivateEndpointsClient", "ListByCluster", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "privateendpoints.PrivateEndpointsClient", "ListByCluster", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForListByCluster(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "privateendpoints.PrivateEndpointsClient", "ListByCluster", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c PrivateEndpointsClient) preparerForListByCluster(ctx context.Context, id ClusterId) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/privateEndpoints", id.ID())),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// preparerForListByClusterWithNextLink prepares the ListByCluster request with the given nextLink token.
func (c PrivateEndpointsClient) preparerForListByClusterWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c PrivateEndpointsClient) responderForListByCluster(resp *http.Response) (result ListByClusterOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListByClusterOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "privateendpoints.PrivateEndpointsClient", "ListByCluster", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForListByCluster(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "privateendpoints.PrivateEndpointsClient", "ListByCluster", result.HttpResponse, "Failure responding to request")
				return
			}
