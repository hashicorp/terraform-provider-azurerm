...
func (c OutputsClient) CreateOrReplace(ctx context.Context, id OutputId, input Output, options CreateOrReplaceOperationOptions) (result CreateOrReplaceOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "outputs.OutputsClient", "CreateOrReplace", nil, "Failure preparing request")
		return
	}

	result.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "outputs.OutputsClient", "CreateOrReplace", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForCreateOrReplace(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "outputs.OutputsClient", "CreateOrReplace", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c OutputsClient) preparerForCreateOrReplace(ctx context.Context, id OutputId, input Output, options CreateOrReplaceOperationOptions) (*http.Request, error) {
...
func (c OutputsClient) responderForCreateOrReplace(resp *http.Response) (result CreateOrReplaceOperationResponse, err error) {
