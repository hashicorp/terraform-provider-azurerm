...
func (c GroupQuotasSubscriptionsClient) GroupQuotaSubscriptionsUpdateThenPoll(ctx context.Context, id SubscriptionId) error {
	return c.GroupQuotaSubscriptionsUpdateCallbackThenPoll(ctx, id, nil)
}

// GroupQuotaSubscriptionsUpdateCallbackThenPoll performs GroupQuotaSubscriptionsUpdate, runs the optional callback function, then polls until it's completed
func (c GroupQuotasSubscriptionsClient) GroupQuotaSubscriptionsUpdateCallbackThenPoll(ctx context.Context, id SubscriptionId, callback func() error) error {
	result, err := c.GroupQuotaSubscriptionsUpdate(ctx, id)
	if err != nil {
		return fmt.Errorf("performing GroupQuotaSubscriptionsUpdate: %+v", err)
	}
	...
	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after GroupQuotaSubscriptionsUpdate: %+v", err)
	}
