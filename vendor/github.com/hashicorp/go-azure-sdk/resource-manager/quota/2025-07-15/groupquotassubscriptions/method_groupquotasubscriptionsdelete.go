...
func (c GroupQuotasSubscriptionsClient) GroupQuotaSubscriptionsDeleteThenPoll(ctx context.Context, id SubscriptionId) error {
	result, err := c.GroupQuotaSubscriptionsDelete(ctx, id)
	if err != nil {
		return fmt.Errorf("performing GroupQuotaSubscriptionsDelete: %+v", err)
	}

	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after GroupQuotaSubscriptionsDelete: %+v", err)
	}
