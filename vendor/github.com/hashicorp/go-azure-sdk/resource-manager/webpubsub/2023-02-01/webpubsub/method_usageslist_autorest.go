...
func (c WebPubSubClient) UsagesList(ctx context.Context, id LocationId) (resp UsagesListOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "webpubsub.WebPubSubClient", "UsagesList", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "webpubsub.WebPubSubClient", "UsagesList", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForUsagesList(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "webpubsub.WebPubSubClient", "UsagesList", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c WebPubSubClient) preparerForUsagesList(ctx context.Context, id LocationId) (*http.Request, error) {
...
func (c WebPubSubClient) preparerForUsagesListWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c WebPubSubClient) responderForUsagesList(resp *http.Response) (result UsagesListOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result UsagesListOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "webpubsub.WebPubSubClient", "UsagesList", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForUsagesList(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "webpubsub.WebPubSubClient", "UsagesList", result.HttpResponse, "Failure responding to request")
				return
			}
