...
func PossibleValuesForExadataVMClusterStorageManagementType() []string {
	return []string{
		string(ExadataVMClusterStorageManagementTypeASM),
		string(ExadataVMClusterStorageManagementTypeExascale),
	}
}

func (s *ExadataVMClusterStorageManagementType) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseExadataVMClusterStorageManagementType(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseExadataVMClusterStorageManagementType(input string) (*ExadataVMClusterStorageManagementType, error) {
	vals := map[string]ExadataVMClusterStorageManagementType{
		"asm":      ExadataVMClusterStorageManagementTypeASM,
		"exascale": ExadataVMClusterStorageManagementTypeExascale,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
	...
	// otherwise presume it's an undefined value and best-effort it
	out := ExadataVMClusterStorageManagementType(input)
	return &out, nil
}
