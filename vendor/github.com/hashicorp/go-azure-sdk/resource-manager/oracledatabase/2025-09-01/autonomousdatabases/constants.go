...
func PossibleValuesForAutonomousDatabaseLifecycleActionEnum() []string {
	return []string{
		string(AutonomousDatabaseLifecycleActionEnumRestart),
		string(AutonomousDatabaseLifecycleActionEnumStart),
		string(AutonomousDatabaseLifecycleActionEnumStop),
	}
}

func (s *AutonomousDatabaseLifecycleActionEnum) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseAutonomousDatabaseLifecycleActionEnum(decoded)
	if err != nil {
...
func parseAutonomousDatabaseLifecycleActionEnum(input string) (*AutonomousDatabaseLifecycleActionEnum, error) {
	vals := map[string]AutonomousDatabaseLifecycleActionEnum{
		"restart": AutonomousDatabaseLifecycleActionEnumRestart,
		"start":   AutonomousDatabaseLifecycleActionEnumStart,
		"stop":    AutonomousDatabaseLifecycleActionEnumStop,
	}
	...
	out := AutonomousDatabaseLifecycleActionEnum(input)
	return &out, nil
}
