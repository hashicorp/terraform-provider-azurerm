...
func PossibleValuesForInaccessibilityReason() []string {
	return []string{
		string(InaccessibilityReasonDatabaseReplication),
		string(InaccessibilityReasonTransparentDataEncryption),
		string(InaccessibilityReasonUnknown),
	}
}

func (s *InaccessibilityReason) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseInaccessibilityReason(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseInaccessibilityReason(input string) (*InaccessibilityReason, error) {
	vals := map[string]InaccessibilityReason{
		"databasereplication":       InaccessibilityReasonDatabaseReplication,
		"transparentdataencryption": InaccessibilityReasonTransparentDataEncryption,
		"unknown":                   InaccessibilityReasonUnknown,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
	...
	// otherwise presume it's an undefined value and best-effort it
	out := InaccessibilityReason(input)
	return &out, nil
}
