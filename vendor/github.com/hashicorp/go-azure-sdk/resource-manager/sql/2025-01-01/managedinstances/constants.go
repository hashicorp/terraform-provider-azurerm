...
const (
	...
	ManagedInstanceDatabaseFormatSQLServerTwoZeroTwoFive ManagedInstanceDatabaseFormat = "SQLServer2025"
...
func PossibleValuesForManagedInstanceDatabaseFormat() []string {
	return []string{
		...
		string(ManagedInstanceDatabaseFormatSQLServerTwoZeroTwoFive),
...
func parseManagedInstanceDatabaseFormat(input string) (*ManagedInstanceDatabaseFormat, error) {
	vals := map[string]ManagedInstanceDatabaseFormat{
		...
		"sqlserver2025":  ManagedInstanceDatabaseFormatSQLServerTwoZeroTwoFive,
...
func PossibleValuesForPricingModel() []string {
	return []string{
		string(PricingModelFreemium),
		string(PricingModelRegular),
	}
}

func (s *PricingModel) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parsePricingModel(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parsePricingModel(input string) (*PricingModel, error) {
	vals := map[string]PricingModel{
		"freemium": PricingModelFreemium,
		"regular":  PricingModelRegular,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
