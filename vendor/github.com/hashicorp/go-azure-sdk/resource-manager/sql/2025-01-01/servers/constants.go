...
func PossibleValuesForServerCreateMode() []string {
	return []string{
		string(ServerCreateModeNormal),
		string(ServerCreateModeRestore),
	}
}

func (s *ServerCreateMode) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseServerCreateMode(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseServerCreateMode(input string) (*ServerCreateMode, error) {
	vals := map[string]ServerCreateMode{
		"normal":  ServerCreateModeNormal,
		"restore": ServerCreateModeRestore,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
