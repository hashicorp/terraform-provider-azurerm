...
const (
	SecurityLevelSLOneFiveZero   SecurityLevel = "SL150"
	SecurityLevelSLThreeThousand SecurityLevel = "SL3000"
	SecurityLevelSLTwoThousand   SecurityLevel = "SL2000"
	SecurityLevelUnknown         SecurityLevel = "Unknown"
)

func PossibleValuesForSecurityLevel() []string {
	return []string{
		string(SecurityLevelSLOneFiveZero),
		string(SecurityLevelSLThreeThousand),
		string(SecurityLevelSLTwoThousand),
		string(SecurityLevelUnknown),
	}
}
...
func (s *SecurityLevel) UnmarshalJSON(bytes []byte) error {
	...
	out, err := parseSecurityLevel(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
