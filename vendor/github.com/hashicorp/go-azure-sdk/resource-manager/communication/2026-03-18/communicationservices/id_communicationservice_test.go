...
func TestParseCommunicationServiceID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseCommunicationServiceID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseCommunicationServiceIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *CommunicationServiceId
	...
	for _, v := range testData {
		...
		actual, err := ParseCommunicationServiceIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForCommunicationServiceId(t *testing.T) {
	segments := CommunicationServiceId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("CommunicationServiceId has no segments")
	}
