...
* `dns_forwarding_rule` - A `dns_forwarding_rule` block as defined below.

* `dns_listening_endpoint_allowed_cidr` - A list of CIDRs that are allowed to send requests to the DNS listening endpoint.
...
A `dns_forwarding_rule` block exports the following:
