...
Whilst the existing App Service and Function App resources allow provisioning of these Resources, due the App Service API's behaviour changing over time, we're looking to better represent these in Terraform.
...
-> **Note:** More Data Sources and Resources will be added during this Beta - and these Data Sources/Resources behaviours may change during the Beta as needed to represent these correctly in Terraform. 
...
You can opt into this Beta by setting the Environment Variable `ARM_THREEPOINTOH_BETA_RESOURCES` to `true`.

On a macOS/Linux system:

```bash
export ARM_THREEPOINTOH_BETA_RESOURCES=true
```

On a Windows System (in PowerShell):

```powershell
[System.Environment]::SetEnvironmentVariable('ARM_THREEPOINTOH_BETA_RESOURCES', 'true', [System.EnvironmentVariableTarget]::User)
```
...
Should you wish to no longer try out the App Service Beta resources you can remove the `ARM_THREEPOINTOH_BETA_RESOURCES` Environment Variable (or set this to `false`) which will disable support for this new functionality in Terraform.
