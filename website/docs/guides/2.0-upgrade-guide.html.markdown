...
The Data Source no longer supports the `enable_blob_encryption` and `enable_file_encryption` properties as encryption is now on by default and cannot be disabled ([More information can be found here](https://docs.microsoft.com/en-us/azure/storage/common/storage-service-encryption))
...
The `enable_blob_encryption` and `enable_file_encryption` fields will be removed. Encryption is now on by default and can no longer be disabled making these fields redundant and potentially confusing. ([More information can be found here](https://docs.microsoft.com/en-us/azure/storage/common/storage-service-encryption))
