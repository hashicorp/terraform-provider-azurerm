...
A new `api_server_access_profile` block has been introduced to better align the resource with the Azure API for access controls currently in preview.  This block replaces `api_server_authorized_ip_ranges` and `private_link_enabled` from the top level.
