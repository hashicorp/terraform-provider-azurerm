...
* The `managed_network` block is no longer `Computed`, users will need to add this block to `ignore_changes`.
...
### `azurerm_remediation_management`
...
### `azurerm_remediation_resource_group`

* The deprecated property `policy_definition_id` has been removed in favour of the `policy_definition_reference_id` property.

### `azurerm_remediation_subscription`
...
* The deprecated property `` has been removed in favour of the `` property.
