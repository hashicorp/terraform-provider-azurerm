...
* The property `operation_name_format` has had the default value `Name` removed.

### `azurerm_application_gateway`

* The property `ssl_profile.ssl_policy.policy_name` now has a default value of `AppGwSslPolicy20220101`.
* The property `ssl_profile.ssl_policy.min_protocol_version` now has a default value of `TLSv1_2`.
...
* The property `user_managed_https.key_vault_secret_id` is now Required.
...
* The property `minimal_tls_version` now defaults to `Tls12`.
...
* The property `roles.kafka_management_node.username` is now Computed only.
...
* The property `file_upload.sas_ttl` now defaults to `PT1H`.
* The property `file_upload.default_ttl` now defaults to `PT1H`.
* The property `file_upload.lock_duration` now defaults to `PT1M`.
...
* New property `vnet_image_pull_enabled` has been added, this property must be set to `true` if your App is running in an App Service Environment.
...
* The property `version` now defaults to `~4`.
...
* The block `ip_configurations` can have a maximum of one item.
...
* The property `redis_configuraton.data_persistence_authentication_method` no longer defaults to `SAS`.
...
* The deprecated property `resource_group_name` has been removed.
...
* The property `authentication_type` now defaults to `SystemAssignedIdentity`.
...
* The property `protocol` has become Required.
...
* The property `private_endpoint_network_policies` now defaults to `Disabled`.
* The property `private_link_service_network_policies_enabled` now defaults to `true`.
...
* The deprecated block `sql_aad_admin` has been removed in favour of the `azurerm_synapse_workspace_sql_aad_admin` resource.
...
* The property `managed_rules.managed_rule_set.rule_group_override.rule.enabled` now defaults to `false`.
