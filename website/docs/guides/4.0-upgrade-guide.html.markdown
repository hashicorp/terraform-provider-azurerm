...
The last major release for the AzureRM provider was in March 2022. Since then we've added support for nearly 340 Resources and 120 Data Sources, bringing the total supported features to 1101 Resources and 358 Data Sources at the time of writing this guide.
...
In version 4.0 of the AzureRM Provider, it's now required to specify the Azure Subscription ID when configuring a provider instance in your configuration. This can be done by specifying the `subscription_id` provider property, or by exporting the `ARM_SUBSCRIPTION_ID` environment variable. For example:
...
`normalise_resource_id` - Takes an Azure Resource ID and attempts to normalise the case-sensitive system segments as required by the AzureRM provider. User specified segments are not affected or corrected. 
...
An example of this is the `subnet` block within `azurerm_virtual_network_resource`, which can also be specified by the resource `azurerm_subnet`. Older resources containing this nature of dependency have been updated to conform to the present pattern of using `ignore_changes` on the appropriate properties.
...
* `endpoint` has had `Computed` and `ConfigModeAttr` removed, attribute notation can no longer be used. If you are managing `endpoint` through any of the IotHub Endpoint resources you will need to add this property to `ignore_changes`.
* `enrichment` has had `Computed` and `ConfigModeAttr` removed, attribute notation can no longer be used. If you are managing `enrichment` through the resource `azurerm_iothub_enrichment` you will need to add this property to `ignore_changes`.
* `route` has had `Computed` and `ConfigModeAttr` removed, attribute notation can no longer be used. If you are managing `route` through the resource `azurerm_iothub_route` you will need to add this property to `ignore_changes`.

### `azurerm_key_vault`

* `access_policy` has had `Computed` and `ConfigModeAttr` removed, attribute notation can no longer be used. If you are managing `access_policy` through the resource `azurerm_key_vault_access_policy` you will need to add this property to `ignore_changes`.
...
* `security_rule` has had `Computed` and `ConfigModeAttr` removed, attribute notation can no longer be used. If you are managing `security_rule` through the resource `azurerm_network_security_rule` you will need to add this property to `ignore_changes`.
...
* `route` has had `Computed` and `ConfigModeAttr` removed, attribute notation can no longer be used. If you are managing `route` through the resource `azurerm_route` you will need to add this property to `ignore_changes`.
...
* `subnet` has had `Computed` and `ConfigModeAttr` removed, attribute notation can no longer be used. If you are managing `subnet` through the resource `azurerm_subnet` you will need to add this property to `ignore_changes`.
...
* The property `extention_operations_enabled` now defaults to `true`.
