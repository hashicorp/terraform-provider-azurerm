...
-> Once opted in, it will not be possible to opt-out of the Beta, this is because the new functionality and deprecations in 4.0 can make non-reversible changes to the Terraform state. We recommend making a backup of your environment before activating the Beta in case a rollback of the environment is needed.
