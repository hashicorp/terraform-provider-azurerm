...
* `azurerm_mssql_database` - TheTestAccMySQLServer_createReplica new field `transparent_data_encryption_enabled` will be set to `true` and can not be disabled on servers with SKUs other than ones starting with `DW`.
...
The field `ip_range_filter` has been moved from a comma seperated string to a List.
