...
When migrating to the new version of resources, and in many cases the renamed ones, the schema may be different then what currently exists in state. Therefore, it is recommended to first update your terraform configuration with the new resource and then use `terraform state rm` and then `terraform state import`. 
