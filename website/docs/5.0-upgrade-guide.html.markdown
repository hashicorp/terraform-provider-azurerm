...
### `azuremr_security_center_automation`

* The property `type` in the `action` block lowercase inputs (`logicapp`, `eventhub`, `loganalytics`) have been changed to titlecase (`LogicApp`, `EventHub`, `LogAnalytics`).
