...
~> **Note:** The `managed_hsm_key_id` property inside the `customer_managed_key` block has been removed. Use the `azurerm_cognitive_account_customer_managed_key` resource for managed HSM key encryption instead. Additionally, `key_vault_key_id` inside the `customer_managed_key` block is now **Required** in `azurerm_cognitive_account`, whereas in `azurerm_ai_services` it was **Optional** (one of `key_vault_key_id` or `managed_hsm_key_id` was required).
