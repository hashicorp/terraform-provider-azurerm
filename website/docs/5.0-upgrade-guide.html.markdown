...
| `customer_managed_key.managed_hsm_key_id`  | `customer_managed_key.key_vault_key_id` | **Migrated**. Apply value of `customer_managed_key.managed_hsm_key_id` from `azurerm_ai_services` to `customer_managed_key.key_vault_key_id` in `azurerm_cognitive_account`. |
