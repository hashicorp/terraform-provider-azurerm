...
* `local_authentication_enabled` - (Optional) Whether local authentication is allowed, as opposed to only Managed Identity and Microsoft Entra (AAD) can be used exclusively for authentication. Defaults to `true`. Can be set only when using the SQL API.
