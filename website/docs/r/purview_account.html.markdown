...
* `type` - (Required) Specifies the type of Managed Service Identity that should be configured on this Purview Account. The only possible value is `UserAssigned` and `SystemAssigned`.
