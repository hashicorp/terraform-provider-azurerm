...
* `license_type` - (Optional) Specifies the BYOL Type for this Virtual Machine. Possible values for Windows Virtual Machines are `Windows_Client` and `Windows_Server`. Possible values for Linux Virtual Machines are `RHEL_BYOS` and `SLES_BYOS`.
