...
* `location` - (Optional) The Azure Region where the activity log alert rule should exist. Changing this forces a new resource to be created. Defaults to `global`. Supported locations are `global`, `westeurope` and `northeurope`.
