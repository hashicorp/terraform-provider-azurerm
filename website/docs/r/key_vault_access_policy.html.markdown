...
* `object_id` - (Required) The object ID of a user, service principal or security group in the Azure Active Directory tenant for the vault. To fetch the object ID of a service principal, please use `azuread_service_principal.object_id` not `azuread_application.object_id`.The object ID must be unique for the list of access policies. Changing this forces a new resource to be created.
