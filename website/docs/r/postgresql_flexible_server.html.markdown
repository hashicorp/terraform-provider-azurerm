...
~> **Note** As service backend would automatically switch `zone` and `standby_availability_zone` after automatic failover, so terraform would detect a config drift and attempt to apply the values in configuration. This can be prevented by using the `ignore_changes` attribute.
