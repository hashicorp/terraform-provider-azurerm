...
-> **Note:** Setting `active_directory_auth_enabled` to `true` needs to set up the Service Principal of Postgres Flexible Server first. Details could be found on [this document](https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/how-to-configure-sign-in-azure-ad-authentication).
