...
-> **NOTE** Since `network_acls` can be configured both inline and via the separate `azurerm_key_vault_network_acl` resource, we have to explicitly set it to empty slice (`[]`) to remove it.
