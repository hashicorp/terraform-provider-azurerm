...
* `requested_service_objective_id` - (Optional) A GUID/UUID corraponding to a configured Service Level Objective for the Azure SQL database which can be used to configure a performance level.
.
