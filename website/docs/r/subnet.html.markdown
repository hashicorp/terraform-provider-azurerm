...
* `sharing_scope` - (Optional) The sharing scope of the subnet. Possible values are `Tenant` and `DelegatedServices`. This property cannot be set if `default_outbound_access_enabled` is set to `true`.
