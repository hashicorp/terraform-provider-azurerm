...
* `storage_key_vault_secret_id` - (Optional) The Key Vault Secret ID, including version, that contains the Connection String to connect to the storage account for this Function App Slot.
...
~> **NOTE:** Not all intervals are supported on all SKU's. Please refer to the official documentation for appropriate values.
