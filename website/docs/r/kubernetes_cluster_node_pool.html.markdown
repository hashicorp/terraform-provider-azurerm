...
* `max_unavailable` - (Optional) The maximum number or percentage of nodes which can be unavailable during the upgrade. Cannot be used at the same time as `max_surge` using percentages that would violate upgrade constraints.
