...
* `max_surge` - (Optional) The maximum number or percentage of nodes which will be added to the Node Pool size during an upgrade. The `max_unavailable` must be set to 0 for `max_surge` to be set. The two values can't both be active at the same time.

* `max_unavailable` - (Optional) The maximum number or percentage of nodes which can be unavailable during the upgrade. The `max_surge` must be set to 0 for `max_unavailable` to be set. The two values can't both be active at the same time. Defaults to `"0"`.
