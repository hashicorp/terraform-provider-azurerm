...
* `pod_ip_allocation_mode` - (Optional) The IP allocation mode for pods in the agent pool. Possible values are `DynamicIndividual` and `StaticBlock`. Must be used with `pod_subnet_id`. Changing this, or removing it from the configuration when a value is present in state, forces a new Kubernetes Cluster Node Pool to be created.
