...
* `conflict_resolution_mode` - (Optional) The conflict resolution mode for the Auto Import Job. Possible values are `Fail`, `OverwriteAlways`, `OverwriteIfDirty`, and `Skip`. Defaults to `Fail`. Changing this forces a new resource to be created.

* `deletions_enabled` - (Optional) Whether to enable deletions for the Auto Import Job. Defaults to `false`. Changing this forces a new resource to be created.

* `maximum_errors` - (Optional) The maximum number of errors allowed for the Auto Import Job. Defaults to `0`. Changing this forces a new resource to be created.
