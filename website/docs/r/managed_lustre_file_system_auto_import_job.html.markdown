...
resource "azurerm_managed_lustre_file_system_auto_import_job" "example" {
  name                          = "acctest-amlfs-auto-import-job"
...
* `managed_lustre_file_system_id` - (Required) The ID of the Azure Managed Lustre File System to which this Auto Import Job belongs. Changing this forces a new resource to be created.
...
* `auto_import_prefixes` - (Required) A list of prefixes that get auto imported to the cluster namespace. Changing this forces a new resource to be created.

* `conflict_resolution_mode` - (Optional) Specifies how to resolve conflicts when importing data. Possible values are `Fail`, `OverwriteAlways`, `OverwriteIfDirty`, and `Skip`. Defaults to `Fail`. Changing this forces a new resource to be created.

* `enable_deletions` - (Optional) Whether to enable deletions during auto-import. Defaults to `false`. Changing this forces a new resource to be created.

* `maximum_errors` - (Optional) The maximum number of errors allowed before the auto-import job stops. Defaults to `0`. Changing this forces a new resource to be created.

* `admin_status_enabled` - (Optional) Whether the administrative status of the Auto Import Job is enabled. Defaults to `true`.
...
* `id` - The Azure Managed Lustre File System Auto Import Job ID.

## Timeouts

The `timeouts` block allows you to specify [timeouts](https://developer.hashicorp.com/terraform/language/resources/configure#define-operation-timeouts) for certain actions:

* `create` - (Defaults to 1 hour) Used when creating the Azure Managed Lustre File System Auto Import Job.
* `read` - (Defaults to 5 minutes) Used when retrieving the Azure Managed Lustre File System Auto Import Job.
* `update` - (Defaults to 1 hour) Used when updating the Azure Managed Lustre File System Auto Import Job.
* `delete` - (Defaults to 90 minutes) Used when deleting the Azure Managed Lustre File System Auto Import Job.

## Import

Azure Managed Lustre File Systems Auto Import Job can be imported using the `resource id`, e.g.
