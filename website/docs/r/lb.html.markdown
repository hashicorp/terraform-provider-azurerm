...
* `sku` - (Optional) The SKU of the Azure Load Balancer. Possible values are `Basic`, `Standard`, and `Gateway`. Defaults to `Standard`. Changing this forces a new resource to be created.

!> **Note:** `sku` can no longer be set to `Basic` for new resources following its retirement. Please see the Microsoft [upgrade guidance](https://learn.microsoft.com/azure/load-balancer/load-balancer-basic-upgrade-guidance) for more information.
