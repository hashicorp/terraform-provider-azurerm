...
~> **NOTE:** If `source_image_id` is set to a shared image with `specialized = true`, or a shared image version created by a specialized shared image, `admin_username`, `admin_password`, `custom_data`, `admin_ssh_key` and `secret` must not be specified.
