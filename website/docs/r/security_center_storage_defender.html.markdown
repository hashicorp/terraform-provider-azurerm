...
* `malware_scanning_blob_scan_results_options` - (Optional) Configure how malware scan results are saved. Possible values are `BlobIndexTags` (save scan results as blob index tags) and `None` (do not save scan results). Defaults to `BlobIndexTags`.
