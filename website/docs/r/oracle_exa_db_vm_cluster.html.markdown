...
* `virtual_machine_file_system_storage` - (Required) A `virtual_machine_file_system_storage` block as defined below. Changing this forces a new Exadata VM Cluster to be created.
...
* `total_size_in_gb` - (Required) Total Capacity. Changing this forces a new Exadata VM Cluster to be created.
