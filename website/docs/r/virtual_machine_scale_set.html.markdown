...
* `computer_name_prefix` - (Required) Specifies the computer name prefix for all of the virtual machines in the scale set. Computer name prefixes must be `1` to `9` characters long for windows images and `1` to `58` characters for linux. Changing this forces a new resource to be created.
...
* `public_ip_address_configuration` - (Optional) A `public_ip_address_configuration` block as documented below.
...
* `os_type` - (Optional) Specifies the operating system Type, valid values are `windows` or `linux`.
...
* `caching` - (Optional) Specifies the caching requirements. Possible values include: `None`, `ReadOnly` and `ReadWrite`. Defaults to `None`.
...
`boot_diagnostics` block supports the following:
