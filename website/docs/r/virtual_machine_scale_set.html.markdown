...
sidebar_current: "docs-azurerm-resource-compute-virtual-machine-scale-set"
description: |-
...
Complete examples of how to use the `azurerm_virtual_machine_scale_set` resource can be found [in the `./examples/vm-scale-sets` folder within the Github Repository](https://github.com/terraform-providers/terraform-provider-azurerm/tree/master/examples/vm-scale-sets)
...
resource "azurerm_virtual_machine_scale_set" "example" {
  ...
  network_profile {
    ...
    ip_configuration {
      ...
      load_balancer_inbound_nat_rules_ids    = ["${element(azurerm_lb_nat_pool.example.*.id, count.index)}"]
    }
  }
...
* `name` - (Required) Specifies the name of the Virtual Machine Scale Set. Changing this forces a new resource to be created.
...
* `location` - (Required) Specifies the supported Azure location where the Virtual Machine Scale Set should exist. Changing this forces a new resource to be created.

* `network_profile` - (Required) One or more `network_profile` blocks as documented below.

* `os_profile` - (Required) A `os_profile` block as documented below.

* `os_type` - (Optional) Specifies the Type of Operating System that should be used. Possible values are `Linux` and `Windows`.

* `storage_profile_os_disk` - (Required) A `storage_profile_os_disk` block as documented below.

* `sku` - (Required) A `sku` block as documented below.

* `upgrade_policy_mode` - (Required) Specifies the mode of an upgrade to Virtual Machines in the Scale Set. Possible values are `Manual` or `Automatic`.

---

When creating a VM Scale Set running Linux - the following fields are available:

* `os_profile_linux_config` - (Required) A `os_profile_linux_config` as documented below.

---

When creating a VM Scale Set running Windows - the following fields are available:

* `os_profile_windows_config` - (Required) A `os_profile_windows_config` as documented below.
...
* `extension` - (Optional) A list of `extension` blocks as defined below.
...
* `priority` - (Optional) Specifies the priority for the virtual machines in the scale set. Possible values are `Low` and `Regular`.

* `single_placement_group` - (Optional) Specifies whether the scale set is limited to a single placement group with a maximum size of 100 virtual machines. If set to `false`, managed disks must be used. Defaults to `true`. Changing this forces a new resource to be created. See [documentation](http://docs.microsoft.com/en-us/azure/virtual-machine-scale-sets/virtual-machine-scale-sets-placement-groups) for more information.

* `storage_profile_data_disk` - (Optional) A list of `storage_profile_data_disk` blocks as documented below
...
-> **NOTE:** Keys within the `settings` and `protected_settings` JSON are case sensitive
...
* `tier` - (Optional) Specifies the tier of virtual machines in a scale set. Possible values are `Basic` or `Standard`.
* `capacity` - (Required) Specifies the number of virtual machines in the scale set.

---

A `storage_profile_data_disk` block supports the following:
...
A `storage_profile_image_reference` block supports the following:
...
A `storage_profile_os_disk` block supports the following:
