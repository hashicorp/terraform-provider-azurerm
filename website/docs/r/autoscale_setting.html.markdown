...
* `target_resource_uri` - (Required) Specifies the resource ID of the resource that the autoscale setting should be added to.
...
* `metric_resource_uri` - (Required) Specifies the resource identifier of the resource the rule monitors.

* `time_grain` - (Required) Specifies the granularity of metrics the rule monitors. Must be one of the predefined 
values returned from metric definitions for the metric. Must be between 12 hours and 1 minute. ISO 8601 duration format.
...
greater than the delay in metric collection, which can vary from resource-to-resource. Must be between 12 hours 
and 5 minutes. ISO 8601 duration format.
...
be between 1 week and 1 minute. ISO 8601 duration format.
