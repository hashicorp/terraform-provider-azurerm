...
* `auto_mitigate_enabled` - (Optional) Should the alerts in this Metric Alert be auto resolved? Defaults to `true`.
-> **NOTE** The `auto_mitigate_enabled` and `throttling` are mutually exclusive and cannot both be set.
