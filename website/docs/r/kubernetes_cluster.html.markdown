...
* `node_resource_group_restriction_level` - (Optional) The restriction level applied to the Node Resource Group. Possible values are `ReadOnly` and `Unrestricted`. Unsetting this after configuring it will force a new resource to be created.
