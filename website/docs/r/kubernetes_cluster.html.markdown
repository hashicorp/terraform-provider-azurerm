...
-> **NOTE:** If you use BYO DNS Zone, AKS cluster should either use a service principal, or an identity type `UserAssigned` with `Private DNS Zone Contributor` role assigned to this identity for the zone. If `UserAssigned` identity is used - to prevent improper resource order destruction - cluster should depend on the role assignment, like in this example:
