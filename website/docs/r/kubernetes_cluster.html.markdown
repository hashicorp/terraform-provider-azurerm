...
-> **Note:** Changing `os_sku`, `os_disk_type` or `os_disk_size_gb` of the `default_node_pool` is done by cycling the system node pool of the cluster. `temporary_name_for_rotation` must be specified when attempting a resize.
...
-> **Note:** Changing the `zones` Virtual Machine is done by cycling the system node pool of the cluster. `temporary_name_for_rotation` must be specified when attempting a resize.
