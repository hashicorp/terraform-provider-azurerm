...
-> **Note:** Changing the `default_node_pool` Virtual Machine disk type is done by cycling the system node pool of the cluster. `temporary_name_for_rotation` must be specified when attempting a resize.
