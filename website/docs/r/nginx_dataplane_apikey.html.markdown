...
subcategory: "NGINX"
layout: "azurerm"
page_title: "Azure Resource Manager: azurerm_nginx_dataplane_apikey"
description: |-
  Manages a Dataplane API Key for an NGINX Deployment.
---

# azurerm_nginx_dataplane_apikey
...
resource "azurerm_nginx_dataplane_apikey" "example" {
  name                = "example-dataplane-apikey"
...
- `secret_text` - (Required) The value used as the Dataplane API Key. A valid value is:

  - at least 12 characters
  - no more than 256 characters
  - only consists of the characters: A-Z, a-z, 0-9, white space, and @#$%^&\*-\_!+=[]{}|\:',.?/`~"();<>
  - contains at least 3 of the 4 subgroups: A-Z, a-z, 0-9, @#$%^&\*-\_!+=[]{}|\:',.?/`~"();<>
...
- `end_date_time` - The RFC3339 formatted date-time after which this Dataplane API Key is no longer valid. The default value is now+6mo. The maximum value is now+2y.
...
```shell
terraform import azurerm_nginx_dataplane_apikey.example /subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/group1/providers/Nginx.NginxPlus/nginxDeployments/deploy1/apiKeys/key1
```
