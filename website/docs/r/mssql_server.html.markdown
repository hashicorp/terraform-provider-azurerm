...
* `primary_user_assigned_identity_id` - (Optional) Specifies the primary user managed identity id. Required if `type` contains `UserAssigned` and should be set at same time as setting `identity_ids`.
