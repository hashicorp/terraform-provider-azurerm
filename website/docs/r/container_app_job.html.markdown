...
* `identity` - (Optional) The Managed Identity to use to authenticate with Azure Container Registry. `System` and `system-environment` refer to the Container App Job's and Container App Environment's System Assigned Managed Identities, respectively. User Assigned Managed Identities are referenced by Resource ID.
