...
~> **Note:**  If the server is created with VNet Integration (i.e. values are provided for `delegated_subnet_id` and `private_dns_zone_id`"), `public_network_access` can only be set to `Disabled`.
