...
~> **Note:**  `public_network_access` is not available as an argument if the server is created with VNet Integration, i.e. values are provided for `delegated_subnet_id` and `private_dns_zone_id`."
