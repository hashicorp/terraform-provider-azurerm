...
* `backup_retention_days` - (Optional) The backup retention days for the MySQL Flexible Server. Possible values are between `7` and `35` days.
...
* `zone` - (Optional) The availability zone information of the MySQL Flexible Server. Possible values are `1`, `2` and `3`. Changing this forces a new MySQL Flexible Server to be created.
...
* `replication_role` - The replication role. Possible values are `None`, `Replica`, and `Source`.
