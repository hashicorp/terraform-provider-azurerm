...
~> **NOTE:** When creating a `Linux` App Service Plan, the `reserved` field must be set to `true`, and when creating a `Windows`/`app` App Service Plan the `reserved` field must be set to `false`. Otherwise, the service api would create a `Linux` App Service Plan when the `kind` field sets to `Windows` and the `reserved` field sets to `true`.
