...
* `kind` - (Optional) The kind of the App Service Plan to create. Possible values are `App`, `elastic`, `FunctionApp`, `Linux`, `Windows` and `xenon`. Defaults to `Windows`. Changing this forces a new resource to be created.
