...
* `platform_fault_domain` - (Optional) Specifies the virtual machine scale set logical fault domain into which this Windows Virtual Machine will be created. Defaults to `-1`. By specifying the default value, this Windows Virtual Machine will be automatically assigned to a fault domain that best maintains balance across available fault domains.
