...
* `allowed_ips` - (Optional) The list of ips in the format of CIDR allowed to connect to the cluster.
...
* `fqdns` - (Optional) List of allowed FQDNs(Fully Qualified Domain Name) for egress from Cluster.
