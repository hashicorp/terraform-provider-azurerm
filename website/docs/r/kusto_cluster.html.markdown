...
* `enable_disk_encryption` - (Optional) Specifies if the cluster's disks are encrypted. Defaults to `false`

* `enable_streaming_ingest` - (Optional) Specifies if the streaming ingest is enabled. Defaults to `false`
