...
* `public_network_access` - (Optional) Whether public network access is allowed for the Virtual Desktop Host Pool. Defaults to `Enabled`. Possible values are `Enabled`, `Disabled`, `EnabledForClientsOnly` and `EnabledForSessionHostsOnly`.
