...
* `data_format` - (Optional) Specifies the data format of the IotHub messages. Allowed values: `AVRO`, `CSV`, `JSON`, `MULTIJSON`, `PSV`, `RAW`, `SCSV`, `SINGLEJSON`, `SOHSV`, `TSV` and `TXT`
...
* `update` - (Defaults to 60 minutes) Used when updating the Kusto IotHub Data Connection.
