...
* `intent_vm_sizes` - (Optional) Specifies the supported sizes of Virtual Machines that can be created in the Proximity Placement Group.
...
~> **NOTE:** `intent_vm_sizes` must be set when `zone` is specified.
