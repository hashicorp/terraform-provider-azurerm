...
* `database_edition` - (Optional) The Oracle Database Edition that applies to the Autonomous databases. Possible values are `EnterpriseEdition` and `StandardEdition`. Changing this forces a new Exadata VM Cluster to be created.
