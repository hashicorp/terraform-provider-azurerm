...
* `custom_resource_group_name` - (Optional) The name of a Resource Group which will be created and used to deploy the infrastructure of Azure Red Hat OpenShift Cluster. The value should not contain uppercase characters. Defaults to `aro-{domain}`. Changing this forces a new resource to be created.
