...
~> **Note:** Associating, changing, or removing `capacity_reservation_group_id` on a Virtual Machine Scale Set with `zones` configured is applied in-place. Any change on a Virtual Machine Scale Set without `zones` configured forces a new resource to be created.
