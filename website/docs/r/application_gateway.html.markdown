...
* `rule_set_type` - (Optional) The Type of the Rule Set used for this Web Application Firewall. Currently, only `OWASP` is supported. Possible values are `OWASP` and `Microsoft_BotManagerRuleSet`.
