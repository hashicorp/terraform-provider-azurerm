...
This example provisions a basic Managed Kubernetes Cluster. Other examples of the `azurerm_kubernetes_automatic_cluster` resource can be found in [the `./examples/kubernetes` directory within the GitHub Repository](https://github.com/hashicorp/terraform-provider-azurerm/tree/main/examples/kubernetes).
...
* `cost_analysis_enabled` - (Optional) Should cost analysis be enabled for this Kubernetes Cluster? Defaults to `false`.

* `custom_ca_trust_certificates_base64` - (Optional) A list of between `1` and `10` base64 encoded CA certificates that will be added to the trust store on nodes.
...
* `http_application_routing_enabled` - (Optional) Should HTTP Application Routing be enabled? Defaults to `false`.
...
* `image_cleaner_interval_hours` - (Optional) Specifies the interval in hours when images should be cleaned up. Defaults to `168`.
...
* `open_service_mesh_enabled` - (Optional) Is Open Service Mesh enabled? For more details, please visit [Open Service Mesh for AKS](https://docs.microsoft.com/azure/aks/open-service-mesh-about). Defaults to `false`.

* `private_cluster_enabled` - (Optional) Should this Kubernetes Cluster have its API server only exposed on internal IP addresses? This provides a Private IP Address for the Kubernetes API on the Virtual Network where the Kubernetes Cluster is located. Defaults to `true`. Changing this forces a new resource to be created.
...
* `tenant_id` - (Optional) The Tenant ID used for Azure Active Directory integration. If this isn't specified, the Tenant ID of the current Subscription is used.
...
* `os_sku` - (Optional) Specifies the OS SKU used by the agent pool. Possible values are `AzureLinux`, `AzureLinux3`, `Ubuntu`, `Ubuntu2204`, `Windows2019` and `Windows2022`. `temporary_name_for_rotation` must be specified when attempting a change.
...
* `secret_rotation_interval` - (Optional) The interval to poll for secret rotation. Defaults to `2m`.
...
* `outbound_type` - (Optional) The outbound (egress) routing method which should be used for this Kubernetes Cluster. Possible values are `loadBalancer`, `userDefinedRouting`, `managedNATGateway`, `userAssignedNATGateway` and `none`. Defaults to `managedNATGateway`.
...
* `load_balancer_profile` - (Optional) A `load_balancer_profile` block as defined below. Changing this forces a new resource to be created.

* `nat_gateway_profile` - (Optional) A `nat_gateway_profile` block as defined below. Changing this forces a new resource to be created.
...
* `backend_pool_type` - (Optional) The type of the managed inbound Load Balancer Backend Pool. Possible values are `NodeIP` and `NodeIPConfiguration`. Defaults to `NodeIPConfiguration`.
...
* `managed_outbound_ipv6_count` - (Optional) The desired number of IPv6 outbound IPs created and managed by Azure for the cluster load balancer. Must be between `1` and `100` inclusive.
...
~> **Note:** At least one of `gateway_id`, `subnet_id`, or `subnet_cidr` must be specified. `gateway_id` conflicts with `subnet_id` and `subnet_cidr`, and `subnet_id` conflicts with `gateway_id` and `subnet_cidr`. The `gateway_name` field is optional.

~> **Note:** If specifying `ingress_application_gateway` in conjunction with `only_critical_addons_enabled`, the AGIC pod will fail to start. A separate `azurerm_kubernetes_automatic_cluster_node_pool` is required to run the AGIC pod successfully. This is because AGIC is classed as a "non-critical addon".
...
* `admin_password` - (Required) The Admin Password for Windows VMs. Length must be between `8` and `123` characters.
...
* `gmsa_profile_enabled` - (Optional) Specifies whether the gMSA profile should be enabled. Defaults to `true`.
...
* `max_surge` - (Optional) The maximum number or percentage of nodes which will be added to the Node Pool size during an upgrade. Defaults to `10%`.
