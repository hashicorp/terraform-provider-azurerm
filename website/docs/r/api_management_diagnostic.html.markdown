...
* `identifier` - (Required) The diagnostic identifier for the API Management Service. At this time the only supported value is `applicationinsights` and `azuremonitor`. Changing this forces a new resource to be created.
