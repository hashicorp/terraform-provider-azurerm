...
* `settings_json` - (Optional) JSON formatted public settings for the extension, its value should encode with [`jsonencode`](https://developer.hashicorp.com/terraform/language/functions/jsonencode) function.

* `protected_settings` - (Optional) JSON formatted protected settings for the extension, its value should encode with [`jsonencode`](https://developer.hashicorp.com/terraform/language/functions/jsonencode) function. The extension can contain either `protected_settings` or `provision_after_extensions` or no protected settings at all.
