...
* `allow_permanent_delete` - (Optional) Indicates whether deletion of the soft deleted blob versions and snapshots is allowed. Defaults to `false`.

~> **NOTE:** `allow_permanent_delete` cannot be set to true if a `restore_policy` block is defined.
