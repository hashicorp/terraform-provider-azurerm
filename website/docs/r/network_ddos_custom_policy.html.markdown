...
-> **Note:** A DDoS Custom Policy is applied to a Load Balancer Frontend IP Configuration by setting `ddos_custom_policy_id` on the Load Balancer, not on this resource. A Frontend IP Configuration can only be associated with one DDoS Custom Policy at a time.
...
* `load_balancer_frontend_ip_configuration_ids` - A list of Load Balancer Frontend IP Configuration IDs which are associated with the Network DDoS Custom Policy.

~> **Note:** `load_balancer_frontend_ip_configuration_ids` is currently always empty due to [a bug in the Azure API definition](https://github.com/Azure/azure-rest-api-specs/issues) where this property is declared under the wrong name. This will be resolved in a future release once the fix is available in the Azure SDK.
...
Network DDoS Custom Policies can be imported using the `resource id`, e.g.

```shell
terraform import azurerm_network_ddos_custom_policy.example /subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/group1/providers/Microsoft.Network/ddosCustomPolicies/policy1
```
