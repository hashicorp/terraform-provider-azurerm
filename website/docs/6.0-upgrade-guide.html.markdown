...
* The `auxiliary_mode` no longer accepts the `None` value.
* The `auxiliary_sku` no longer accepts the `None` value.
...
* The deprecated `storage_container_type` property has been fully removed and now defaults to `blobContainer`.
* The deprecated `storage_container_endpoint` property has been removed in favour of the `deployment_storage.container_endpoint`.
* The deprecated `storage_access_key` property has been removed in favour of the `deployment_storage.access_key`.
* The deprecated `storage_user_assigned_identity_id` property has been removed in favour of the `deployment_storage.user_assigned_identity_id`.
* In version 5.0, backend storage–related app_settings such as `AzureWebJobsStorage` reference `storage_container_endpoint` and `storage_access_key`. Starting from version 6.0, these will instead be managed via the `backend_storage` block.
* In version 5.0, `backend_storage` are required with `deployment_storage`
