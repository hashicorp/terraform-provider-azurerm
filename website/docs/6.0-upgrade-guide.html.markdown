...
* The deprecated `site_config.vnet_route_all_enabled` property has been removed in favour of the `virtual_network_application_traffic_enabled` property.
...
* The `site_config.vnet_route_all_enabled` property is no longer referred from the `WEBSITE_VNET_ROUTE_ALL` app setting, and vnet application traffic will be referred from the `vnet_application_traffic_enabled` property instead.
