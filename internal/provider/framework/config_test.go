...
func TestProviderConfig_LoadDefault(t *testing.T) {
	...
	if features.RecoveryService.VMWorkloadSAPAseDatabaseBackupStopProtectionAndRetainDataOnDestroy {
		t.Errorf("expected recovery_service.vm_workload_sap_ase_database_backup_stop_protection_and_retain_data_on_destroy to be false")
	}
...
func defaultFeaturesList() types.List {
	...
	recoveryServices, _ := basetypes.NewObjectValueFrom(context.Background(), RecoveryServiceAttributes, map[string]attr.Value{
		...
		"vm_workload_sap_ase_database_backup_stop_protection_and_retain_data_on_destroy": basetypes.NewBoolNull(),
