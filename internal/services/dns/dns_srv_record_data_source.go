...
func dataSourceDnsSrvRecordRead(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	resp, err := recordSetsClient.Get(ctx, resourceGroup, zoneName, name, dns.SRV)
	if err != nil {
		if utils.ResponseWasNotFound(resp.Response) {
			return fmt.Errorf("Error: DNS SRV record %s: (zone %s) was not found", name, zoneName)
		}
		return fmt.Errorf("reading DNS SRV record %s (zone %s): %+v", name, zoneName, err)
	}

	resourceId := parse.NewSrvRecordID(subscriptionId, resourceGroup, zoneName, name)
