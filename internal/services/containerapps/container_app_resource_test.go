...
func TestAccContainerAppResource_completeWithNoAppPort(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_container_app", "test")
	r := ContainerAppResource{}

	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.completeWithNoAppPort(data, "rev1"),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
...
func (r ContainerAppResource) completeWithNoAppPort(data acceptance.TestData, revisionSuffix string) string {
	return fmt.Sprintf(`
%s
