...
func TestAccContainerAppEnvironment_updateWorkloadProfileWithoutLogAnalyticsWorkspaceId(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_container_app_environment", "test")
	r := ContainerAppEnvironmentResource{}
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		...
		data.ImportStep(),
		{
			Config: r.completeUpdateWorkloadProfile(data),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
...
func (r ContainerAppEnvironmentResource) completeUpdateWorkloadProfile(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
