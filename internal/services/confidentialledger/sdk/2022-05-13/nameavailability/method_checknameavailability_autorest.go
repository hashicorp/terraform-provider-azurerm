...
func (c NameAvailabilityClient) CheckNameAvailability(ctx context.Context, id commonids.SubscriptionId, input CheckNameAvailabilityRequest) (result CheckNameAvailabilityOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "nameavailability.NameAvailabilityClient", "CheckNameAvailability", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForCheckNameAvailability(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "nameavailability.NameAvailabilityClient", "CheckNameAvailability", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c NameAvailabilityClient) preparerForCheckNameAvailability(ctx context.Context, id commonids.SubscriptionId, input CheckNameAvailabilityRequest) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/providers/Microsoft.ConfidentialLedger/checkNameAvailability", id.ID())),
