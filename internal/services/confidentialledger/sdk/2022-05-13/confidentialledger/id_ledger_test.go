...
func TestParseLedgerID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *LedgerId
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.ConfidentialLedger",
			Error: true,
		},
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.ConfidentialLedger/ledgers",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseLedgerID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseLedgerIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *LedgerId
	...
	for _, v := range testData {
		...
		actual, err := ParseLedgerIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForLedgerId(t *testing.T) {
	segments := LedgerId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("LedgerId has no segments")
	}
