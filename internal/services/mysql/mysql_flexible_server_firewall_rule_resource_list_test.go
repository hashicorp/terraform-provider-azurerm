...
func TestAccMySqlFlexibleFirewallRule_list_by_resource_group(t *testing.T) {
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				QueryResultChecks: []querycheck.QueryResultCheck{
					...
					querycheck.ExpectLength("azurerm_mysql_flexible_server_firewall_rule.list", 1),
					querycheck.ExpectIdentity(
						"azurerm_mysql_flexible_server_firewall_rule.list",
...
func (r MysqlFlexibleServerFirewallRuleResource) basicListQueryByResourceGroupName(data acceptance.TestData) string {
	return fmt.Sprintf(`
...
list "azurerm_mysql_flexible_server_firewall_rule" "list" {
  provider = azurerm
  config {
