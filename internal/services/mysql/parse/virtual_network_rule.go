...
func (id VirtualNetworkRuleId) ID() string {
	fmtString := "/subscriptions/%s/resourceGroups/%s/providers/Microsoft.DBforMySQL/servers/%s/virtualNetworkRules/%s"
...
func VirtualNetworkRuleID(input string) (*VirtualNetworkRuleId, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("parsing %q as a VirtualNetworkRule ID: %+v", input, err)
	}
