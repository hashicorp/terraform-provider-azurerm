...
func (r CommunicationsGatewayResource) CustomizeDiff() sdk.ResourceFunc {
	return sdk.ResourceFunc{
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			for _, sls := range model.ServiceLocations {
				if model.E911Type == communicationsgateways.E911TypeStandard {
					if len(sls.EsrpAddresses) > 0 {
						return fmt.Errorf("the esrp_addresses of %s must not be provided for each service_locations when e911_type is set to Standard", model.Name)
					}
				} else {
					if len(sls.EsrpAddresses) == 0 {
						return fmt.Errorf("the esrp_addresses of %s must be provided for each service_locations when e911_type is set to DirectToEsrp", model.Name)
					}
				}
			}
			return nil
