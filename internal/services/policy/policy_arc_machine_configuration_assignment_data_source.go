...
func dataSourcePolicyArcMachineConfigurationAssignment() *pluginsdk.Resource {
	return &pluginsdk.Resource{
		Read: dataSourcePolicyArcMachineConfigurationAssignmentRead,
...
func dataSourcePolicyArcMachineConfigurationAssignmentRead(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	vmName := d.Get("machine_name").(string)
	name := d.Get("name").(string)

	id := guestconfigurationhcrpassignments.NewProviders2GuestConfigurationAssignmentID(subscriptionId, resourceGroup, vmName, name)
	...
	if model := resp.Model; model != nil {
		if props := model.Properties; props != nil {
			...
			contentHash, contentUri := dataSourceFlattenGuestConfigurationHCRPAssignment(props.GuestConfiguration)
...
func dataSourceFlattenGuestConfigurationHCRPAssignment(input *guestconfigurationhcrpassignments.GuestConfigurationNavigation) (*string, *string) {
	if input == nil {
		return nil, nil
	}
