...
func TestManagedHSMDataPlaneVersionlessKeyID(t *testing.T) {
	testData := []struct {
		input string
		valid bool
	}{
		...
		{
			// key vault key id
			input: "https://example.keyvault.azure.net/keys/abc123",
			valid: false,
		},
		{
			// domain but no uri
		...
		},
		{
			...
			input: "https://example.managedhsm.azure.net/numbers/abc123",
			valid: false,
		},
		{
	...
	for _, item := range testData {
		...
		warnings, errs := ManagedHSMDataPlaneVersionlessKeyID(item.input, "some_field")
