...
func (r CloudHardwareSecurityModuleClusterResource) Update() sdk.ResourceFunc {
	return sdk.ResourceFunc{
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			if exists.Model == nil {
				return fmt.Errorf("got nil model for HSM Cluster %s", *id)
			}
			...
			if metadata.ResourceData.HasChange("identity") {
				...
				// below logic needs go-azure-helpers with https://github.com/hashicorp/go-azure-helpers/pull/259
				...
				// always set the tags or API will remove the tags
				update.Tags = exists.Model.Tags
			}

			// the `tags` field should always be set or it will be removed by the API
			if metadata.ResourceData.HasChange("tags") {
				if model.Tags == nil {
					update.Tags = pointer.To(map[string]string{})
				} else {
			...
			if err != nil {
				return fmt.Errorf("updating for %s: %+v", *id, err)
			}
