...
func (r CloudHardwareSecurityModuleClusterResource) Create() sdk.ResourceFunc {
	return sdk.ResourceFunc{
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			if model.AutoGeneratedDomainNameLabelScope != "" {
				parameters.Properties = &cloudhsmclusters.CloudHsmClusterProperties{
	...
	return sdk.ResourceFunc{
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			if m := resp.Model; m != nil {
				...
				model.Tags = tags.Flatten(m.Tags)
				...
				if props := m.Properties; props != nil {
					if props.ActivationState != nil {
						model.ActivationState = string(*props.ActivationState)
					}

					if props.AutoGeneratedDomainNameLabelScope != nil {
						model.AutoGeneratedDomainNameLabelScope = string(*props.AutoGeneratedDomainNameLabelScope)
					}

					if props.StatusMessage != nil {
						model.StatusMessage = *props.StatusMessage
					}
...
func flattenPrivateEndpointConnections(connections *[]cloudhsmclusters.PrivateEndpointConnection) []PrivateEndpointConnectionModel {
	...
	for _, conn := range *connections {
		...
		if conn.Id != nil {
			connModel.Id = *conn.Id
		}
		if conn.Name != nil {
			connModel.Name = *conn.Name
		}
		if conn.Type != nil {
			connModel.Type = *conn.Type
		}
		if conn.Properties != nil {
			if conn.Properties.GroupIds != nil {
				connModel.GroupIds = *conn.Properties.GroupIds
			}
