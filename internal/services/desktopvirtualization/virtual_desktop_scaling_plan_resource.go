...
func expandScalingPlanSchedule(input []interface{}) *[]scalingplan.ScalingSchedule {
	...
	for _, item := range input {
		...
		results = append(results, scalingplan.ScalingSchedule{
			...
			RampUpMinimumHostsPct:          pointer.To(int64(int64(rampUpMinimumHostsPct))),
			RampUpCapacityThresholdPct:     pointer.To(int64(int64(rampUpCapacityThresholdPct))),
			...
			RampDownMinimumHostsPct:        pointer.To(int64(int64(rampDownMinimumHostsPct))),
			RampDownCapacityThresholdPct:   pointer.To(int64(int64(rampDownCapacityThresholdPct))),
			RampDownForceLogoffUsers:       pointer.To(rampDownForceLogoffUsers),
			RampDownStopHostsWhen:          &rampDownStopHostsWhen,
			RampDownWaitTimeMinutes:        pointer.To(int64(int64(rampDownWaitTimeMinutes))),
