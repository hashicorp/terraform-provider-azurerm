...
func expandScalingPlanSchedule(input []interface{}) *[]scalingplan.ScalingSchedule {
	...
	for _, item := range input {
		...
		results = append(results, scalingplan.ScalingSchedule{
			...
			RampUpLoadBalancingAlgorithm:   pointer.To(scalingplan.SessionHostLoadBalancingAlgorithm(v["ramp_up_load_balancing_algorithm"].(string))),
			...
			PeakLoadBalancingAlgorithm:     pointer.To(scalingplan.SessionHostLoadBalancingAlgorithm(v["peak_load_balancing_algorithm"].(string))),
			RampDownStartTime:              expandScalingPlanScheduleTime(rampDownStartTime),
			RampDownLoadBalancingAlgorithm: pointer.To(scalingplan.SessionHostLoadBalancingAlgorithm(v["ramp_down_load_balancing_algorithm"].(string))),
			...
			RampDownStopHostsWhen:          pointer.To(scalingplan.StopHostsWhen(v["ramp_down_stop_hosts_when"].(string))),
			...
			OffPeakLoadBalancingAlgorithm:  pointer.To(scalingplan.SessionHostLoadBalancingAlgorithm(v["off_peak_load_balancing_algorithm"].(string))),
		})
	}
