...
//go:generate go run ../../tools/generator-tests resourceidentity -resource-name api_management_diagnostic -service-package-name apimanagement -properties "name,service_name:api_management_name,resource_group_name" -known-values "subscription_id:data.Subscriptions.Primary" -test-name basic
