...
//go:generate go run ../../tools/generator-tests resourceidentity -resource-name api_management_identity_provider_twitter -service-package-name apimanagement -properties "api_management_name:service_name,resource_group_name" -known-values "subscription_id:data.Subscriptions.Primary" -test-name basic
