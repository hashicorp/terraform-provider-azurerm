...
var _ resourceids.Id = AutomanageConfigurationVersionId{}

func TestAutomanageConfigurationVersionIDFormatter(t *testing.T) {
	actual := NewAutomanageConfigurationVersionID("12345678-1234-9876-4563-123456789012", "resourceGroup1", "configurationProfile1", "version1").ID()
...
func TestAutomanageConfigurationVersionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *AutomanageConfigurationVersionId
	}{
	...
	for _, v := range testData {
		...
		actual, err := AutomanageConfigurationVersionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
