...
func TestAccRoleManagementPolicy_noExpiry(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_role_management_policy", "test")
	r := RoleManagementPolicyResource{}
...
# There is a timing issue where the PIM API cannot find new users created for 5 to 10 seconds
...
resource "azurerm_role_management_policy" "test" {
  ...
  activation {
    ...
    approvers {
      user {
        name = data.azuread_users.users.users[0].display_name
        id   = data.azuread_users.users.users[0].object_id
      }
    }
  }
...
func (RoleManagementPolicyResource) complete() string {
	return `
provider "azurerm" {
  features {}
}
...
resource "azurerm_role_management_policy" "test" {
  ...
  activation {
    ...
    approvers {
      group {
        name = data.azuread_groups.all.display_names[0]
        id   = data.azuread_groups.all.object_ids[0]
      }
      group {
        name = data.azuread_groups.all.display_names[1]
        id   = data.azuread_groups.all.object_ids[1]
      }
      user {
        name = data.azuread_users.users.users[0].display_name
        id   = data.azuread_users.users.users[0].object_id
      ...
      user {
        name = data.azuread_users.users.users[1].display_name
        id   = data.azuread_users.users.users[1].object_id
      }
    }
  }
...
func (RoleManagementPolicyResource) noExpiry() string {
	return `
provider "azurerm" {
  features {}
}
