...
func (p clusterStatePoller) Poll(ctx context.Context) (*pollers.PollResult, error) {
	...
	resourceState := string(*resp.Model.Properties.ResourceState)
	...
	pendingStates := []string{"Creating", "Updating", "Enabling", "Deleting", "Disabling"}
	for _, pendingState := range pendingStates {
		if resourceState == pendingState {
