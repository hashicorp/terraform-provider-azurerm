...
var _ pollers.PollerType = &geoReplicationLinkingPoller{}

type geoReplicationLinkingPoller struct {
	client              DatabasesClientInterface
	id                  databases.DatabaseId
	expectedLinkedDbIds []string
}

func NewGeoReplicationLinkingPoller(client DatabasesClientInterface, id databases.DatabaseId, expectedLinkedDbIds []string) *geoReplicationLinkingPoller {
	return &geoReplicationLinkingPoller{
		client:              client,
		id:                  id,
		expectedLinkedDbIds: expectedLinkedDbIds,
	}
}

func (p geoReplicationLinkingPoller) Poll(ctx context.Context) (*pollers.PollResult, error) {
