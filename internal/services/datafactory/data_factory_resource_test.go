...
func TestAccDataFactory_managedHSMKeyEncryption(t *testing.T) {
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.managedHSMKeyEncryption(data),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
...
func (DataFactoryResource) managedHSMKeyEncryption(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {
    key_vault {
...
resource "azurerm_key_vault_managed_hardware_security_module_role_assignment" "radf" {
  managed_hsm_id     = azurerm_key_vault_managed_hardware_security_module.test.id
  name               = "%[5]s"
  scope              = "/keys"
...
resource "azurerm_data_factory" "test" {
  ...
  customer_managed_hsm_key_id      = azurerm_key_vault_managed_hardware_security_module_key.test.id
  customer_managed_key_identity_id = azurerm_user_assigned_identity.test.id

  depends_on = [azurerm_key_vault_managed_hardware_security_module_role_assignment.radf]
}
`, data.RandomInteger, data.Locations.Primary, uuid.NewString(), uuid.NewString(), uuid.NewString())
}
