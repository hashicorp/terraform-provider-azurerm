...
func expandAzureCredentialReference(input []interface{}) *datafactory.CredentialReference {
	if len(input) == 0 || input[0] == nil {
		return nil
	}
	...
	if v, ok := config["reference_name"].(string); ok {
		referenceName = &v
	}
...
func flattenAzureCredentialReference(input *datafactory.CredentialReference) []map[string]string {
	if input == nil {
		return nil
	}
	...
	if input.ReferenceName != nil {
		parameters["reference_name"] = *input.ReferenceName
	}
