...
func dataSourceStorageQueueRead(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if !features.FivePointOh() {
		if accountName := d.Get("storage_account_name").(string); accountName != "" {
			...
			resourceManagerId := storagequeues.NewQueueID(account.StorageAccountId.SubscriptionId, account.StorageAccountId.ResourceGroupName, account.StorageAccountId.StorageAccountName, queueName)
