...
func flattenAccountAccessKeysAndConnectionStrings(accountName, domainSuffix string, keys []storageaccounts.StorageAccountKey, endpoints accountEndpoints, dnsEndpointType storageaccounts.DnsEndpointType) accountAccessKeysAndConnectionStrings {
	...
	if len(keys) > 0 {
		...
		if output.primaryAccessKey != "" {
			output.primaryConnectionString = connectionString(accountName, output.primaryAccessKey, domainSuffix, dnsEndpointType, endpoints.primaryBlobEndpoint, endpoints.primaryQueueEndpoint, endpoints.primaryTableEndpoint, endpoints.primaryFileEndpoint)
		...
		if output.secondaryAccessKey != "" {
			output.secondaryConnectionString = connectionString(accountName, output.secondaryAccessKey, domainSuffix, dnsEndpointType, endpoints.secondaryBlobEndpoint, endpoints.secondaryQueueEndpoint, endpoints.secondaryTableEndpoint, endpoints.secondaryFileEndpoint)
...
func connectionString(accountName, accessKey, domainSuffix string, dnsEndpointType storageaccounts.DnsEndpointType, blobEndpoint, queueEndpoint, tableEndpoint, fileEndpoint string) string {
	...
	parts := []string{
		...
		fmt.Sprintf("AccountName=%s", accountName),
		fmt.Sprintf("AccountKey=%s", accessKey),
	}
	// Only the service endpoints supported by the account kind are returned by the API,
	// so include them conditionally to mirror the connection string generated by Azure.
	if blobEndpoint != "" {
		parts = append(parts, fmt.Sprintf("BlobEndpoint=%s", blobEndpoint))
	}
	if fileEndpoint != "" {
		parts = append(parts, fmt.Sprintf("FileEndpoint=%s", fileEndpoint))
	}
	if queueEndpoint != "" {
		parts = append(parts, fmt.Sprintf("QueueEndpoint=%s", queueEndpoint))
	}
	if tableEndpoint != "" {
		parts = append(parts, fmt.Sprintf("TableEndpoint=%s", tableEndpoint))
	}
