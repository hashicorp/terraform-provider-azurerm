...
var _ resourceids.Id = LegacyStorageShareResourceManagerId{}

func TestLegacyStorageShareResourceManagerIDFormatter(t *testing.T) {
	actual := NewLegacyStorageShareResourceManagerID("12345678-1234-9876-4563-123456789012", "resGroup1", "storageAccount1", "fileService1", "share1").ID()
...
func TestLegacyStorageShareResourceManagerID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *LegacyStorageShareResourceManagerId
	}{
	...
	for _, v := range testData {
		...
		actual, err := LegacyStorageShareResourceManagerID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
