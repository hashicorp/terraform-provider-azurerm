...
var _ resourceids.Id = StorageShareFileResourceManagerId{}

func TestStorageShareFileResourceManagerIDFormatter(t *testing.T) {
	actual := NewStorageShareFileResourceManagerID("12345678-1234-9876-4563-123456789012", "resGroup1", "storageAccount1", "fileService1", "share1").ID()
...
func TestStorageShareFileResourceManagerID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *StorageShareFileResourceManagerId
	}{
	...
	for _, v := range testData {
		...
		actual, err := StorageShareFileResourceManagerID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
