...
func TestParseScopedAlertID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/some-resource-group/providers/Microsoft.CostManagement/alerts",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseScopedAlertID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseScopedAlertIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ScopedAlertId
	...
	for _, v := range testData {
		...
		actual, err := ParseScopedAlertIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForScopedAlertId(t *testing.T) {
	segments := ScopedAlertId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ScopedAlertId has no segments")
	}
