...
func TestParseScopedExportID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseScopedExportID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseScopedExportIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ScopedExportId
	...
	for _, v := range testData {
		...
		actual, err := ParseScopedExportIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForScopedExportId(t *testing.T) {
	segments := ScopedExportId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ScopedExportId has no segments")
	}
