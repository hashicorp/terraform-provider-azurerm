...
def helm_install_release(chart_path, subscription_id, kubernetes_distro, kubernetes_infra, resource_group_name, cluster_name,
                         ...
                         no_wait, cloud_name, helm_client_location, kube_config, custom_locations_oid="", onboarding_timeout="1800"):
...
def install_agent():
    ...
    private_key_source = parser.add_mutually_exclusive_group(required=True)
    private_key_source.add_argument('--privatePem', type=str)
    private_key_source.add_argument('--privatePemEnvironmentVariable', type=str)
    parser.add_argument('--kubeConfig', type=str, required=True)
    parser.add_argument('--customLocationsOid', type=str, required=False, default='')
    ...
    if args.privatePemEnvironmentVariable:
        privateKey = os.environ[args.privatePemEnvironmentVariable]
    else:
        try:
