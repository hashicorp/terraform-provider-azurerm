...
var _ resourceids.Id = DevTestVirtualNetworkId{}
...
func TestDevTestVirtualNetworkIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *DevTestVirtualNetworkId
	...
	for _, v := range testData {
		...
		actual, err := DevTestVirtualNetworkIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
