...
// ClientID validates the ClientID is valid for an OpenShift Cluster
func ClientID(i interface{}, k string) ([]string, []error) {
	...
	if strings.EqualFold(v, "msi") {
		return nil, []error{
			fmt.Errorf("to define a Red Hat OpenShift cluster with authentication via MSI - remove the `service_principal` block"),
		}
	}
