...
func (r SpringCloudAcceleratorResource) Exists(ctx context.Context, client *clients.Client, state *pluginsdk.InstanceState) (*bool, error) {
	id, err := appplatform.ParseApplicationAcceleratorID(state.ID)
	if err != nil {
		return nil, err
	}
	resp, err := client.AppPlatform.ApplicationAcceleratorClient.Get(ctx, id.ResourceGroupName, id.SpringName, id.ApplicationAcceleratorName)
	if err != nil {
