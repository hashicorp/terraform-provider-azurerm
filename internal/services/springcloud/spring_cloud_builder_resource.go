...
func resourceSpringCloudBuildServiceBuilder() *pluginsdk.Resource {
	return &pluginsdk.Resource{
		...
		Importer: pluginsdk.ImporterValidatingResourceId(func(id string) error {
			...
			_, err := appplatform_rm.ParseBuilderIDInsensitively(id)
			return err
		}),
...
func resourceSpringCloudBuildServiceBuilderCreateUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	// the ID is parsed insensitively to preserve the behaviour of the legacy parser this replaced -
	// we are ok with this remaining insensitive as Azure Spring Apps is deprecated and will be removed
	springId, err := commonids.ParseSpringCloudServiceIDInsensitively(d.Get("spring_cloud_service_id").(string))
	if err != nil {
		return err
	}
