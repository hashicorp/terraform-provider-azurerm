...
func (s SpringCloudAPIPortalResource) Create() sdk.ResourceFunc {
	return sdk.ResourceFunc{
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			id := appplatform.NewApiPortalID(springId.SubscriptionId, springId.ResourceGroupName, springId.SpringName, model.Name)
			...
			if service.Model == nil {
				return fmt.Errorf("checking for presence of existing %s: model was nil", springId)
			}
			...
			resource := appplatform.ApiPortalResource{
			...
			err = client.ApiPortalsCreateOrUpdateThenPoll(ctx, id, resource)
			if err != nil {
