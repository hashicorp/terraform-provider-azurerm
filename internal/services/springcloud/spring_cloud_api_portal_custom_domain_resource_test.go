...
func (r SpringCloudAPIPortalCustomDomainResource) Exists(ctx context.Context, client *clients.Client, state *pluginsdk.InstanceState) (*bool, error) {
	id, err := appplatform.ParseApiPortalDomainID(state.ID)
	if err != nil {
		return nil, err
	}
	resp, err := client.AppPlatform.APIPortalCustomDomainClient.Get(ctx, id.ResourceGroupName, id.SpringName, id.ApiPortalName, id.DomainName)
	if err != nil {
