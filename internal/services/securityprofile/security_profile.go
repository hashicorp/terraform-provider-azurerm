...
// ValuesFromAccessor builds a Values struct from the provided Accessor.
func ValuesFromAccessor(accessor Accessor) Values {
	return Values{
		HostEncryption: *accessor.GetHostEncryption(),
		SecurityType:   accessor.GetSecurityType(),
		SecureBoot:     *accessor.GetSecureBoot(),
		VTPM:           *accessor.GetVTPM(),
	}
}

// ApplyValues writes the provided Values onto the Accessor, omitting unspecified fields.
func ApplyValues(values Values, accessor Accessor) {
	*accessor.GetHostEncryption() = values.HostEncryption
	accessor.SetSecurityType(values.SecurityType)
	*accessor.GetSecureBoot() = values.SecureBoot
	*accessor.GetVTPM() = values.VTPM
}
