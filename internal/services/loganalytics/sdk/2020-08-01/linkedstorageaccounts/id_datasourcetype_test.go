...
func TestParseDataSourceTypeID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.OperationalInsights/workspaces/workspaceValue/linkedStorageAccounts",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseDataSourceTypeID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.DataSourceType != v.Expected.DataSourceType {
			t.Fatalf("Expected %q but got %q for DataSourceType", v.Expected.DataSourceType, actual.DataSourceType)
		}

	}
}

func TestParseDataSourceTypeIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *DataSourceTypeId
	...
	for _, v := range testData {
		...
		actual, err := ParseDataSourceTypeIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForDataSourceTypeId(t *testing.T) {
	segments := DataSourceTypeId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("DataSourceTypeId has no segments")
	}
