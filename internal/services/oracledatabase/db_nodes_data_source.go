...
func (d DBNodesDataSource) ResourceType() string {
	return "azurerm_oracledatabase_db_nodes"
}
...
func (d DBNodesDataSource) Read() sdk.ResourceFunc {
	return sdk.ResourceFunc{
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			client := metadata.Client.OracleDatabase.OracleDatabaseClient.DbNodes
				...
				for _, element := range *model {
					if element.Properties != nil {
						...
						dbNode := DBNodeDataModel{
							...
							BackupVnicId:               pointer.From(properties.BackupVnicId),
							BackupVnic2Id:              pointer.From(properties.BackupVnic2Id),
