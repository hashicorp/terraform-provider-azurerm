...
var _ resourceid.Formatter = ElasticMonitorId{}

func TestElasticMonitorIDFormatter(t *testing.T) {
	actual := NewElasticMonitorID("12345678-1234-9876-4563-123456789012", "resourceGroup1", "monitor1").ID()
	expected := "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/resourceGroup1/providers/Microsoft.Elastic/monitors/monitor1"
	if actual != expected {
		t.Fatalf("Expected %q but got %q", expected, actual)
	}
}

func TestElasticMonitorID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ElasticMonitorId
	}{
	...
	for _, v := range testData {
		...
		actual, err := ElasticMonitorID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
