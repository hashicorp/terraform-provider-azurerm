...
func (c BackupPoliciesClient) CreateOrUpdate(ctx context.Context, id BackupPoliciesId, input BaseBackupPolicyResource) (result CreateOrUpdateOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "backuppolicies.BackupPoliciesClient", "CreateOrUpdate", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForCreateOrUpdate(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "backuppolicies.BackupPoliciesClient", "CreateOrUpdate", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c BackupPoliciesClient) responderForCreateOrUpdate(resp *http.Response) (result CreateOrUpdateOperationResponse, err error) {
