...
func (c AzureBackupJobsClient) JobsList(ctx context.Context, id BackupVaultId) (resp JobsListOperationResponse, err error) {
	req, err := c.preparerForJobsList(ctx, id)
	if err != nil {
		err = autorest.NewErrorWithError(err, "azurebackupjobs.AzureBackupJobsClient", "JobsList", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "azurebackupjobs.AzureBackupJobsClient", "JobsList", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForJobsList(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "azurebackupjobs.AzureBackupJobsClient", "JobsList", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c AzureBackupJobsClient) preparerForJobsList(ctx context.Context, id BackupVaultId) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/backupJobs", id.ID())),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// preparerForJobsListWithNextLink prepares the JobsList request with the given nextLink token.
func (c AzureBackupJobsClient) preparerForJobsListWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c AzureBackupJobsClient) responderForJobsList(resp *http.Response) (result JobsListOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result JobsListOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "azurebackupjobs.AzureBackupJobsClient", "JobsList", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForJobsList(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "azurebackupjobs.AzureBackupJobsClient", "JobsList", result.HttpResponse, "Failure responding to request")
				return
			}
