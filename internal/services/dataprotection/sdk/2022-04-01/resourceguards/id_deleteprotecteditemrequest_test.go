...
func TestParseDeleteProtectedItemRequestID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseDeleteProtectedItemRequestID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseDeleteProtectedItemRequestIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *DeleteProtectedItemRequestId
	...
	for _, v := range testData {
		...
		actual, err := ParseDeleteProtectedItemRequestIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForDeleteProtectedItemRequestId(t *testing.T) {
	segments := DeleteProtectedItemRequestId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("DeleteProtectedItemRequestId has no segments")
	}
