...
func (r LogicAppIntegrationAccountBatchConfigurationResource) Exists(ctx context.Context, client *clients.Client, state *pluginsdk.InstanceState) (*bool, error) {
	id, err := parse.IntegrationAccountBatchConfigurationID(state.ID)
	if err != nil {
		return nil, err
	}
	resp, err := client.Logic.IntegrationAccountBatchConfigurationClient.Get(ctx, id.ResourceGroup, id.IntegrationAccountName, id.BatchConfigurationName)
	if err != nil {
