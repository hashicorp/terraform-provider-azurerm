...
func TestAccKeyVaultCertificate_planDigiCertWithoutType_NoValidationOnUpdate(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_key_vault_certificate", "test")
	r := KeyVaultCertificateResource{}
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		...
		data.ImportStep(),
		{
			Config:             r.planOnlyDigiCertWithoutType(data),
			PlanOnly:           true,
			ExpectNonEmptyPlan: true,
		},
	})
...
func (r KeyVaultCertificateResource) planOnlyDigiCertWithoutType(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
