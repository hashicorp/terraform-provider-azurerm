	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			...
			Check: acceptance.ComposeTestCheckFunc(
				...
				check.That(data.ResourceName).Key("versionless_id").HasValue(fmt.Sprintf("/subscriptions/%s/resourceGroups/acctestRG-%d/providers/Microsoft.KeyVault/vaults/acctestkv-%s/keys/key-%s", data.Subscriptions.Primary, data.RandomInteger, data.RandomString, data.RandomString)),
				check.That(data.ResourceName).Key("versionless_data_plane_id").HasValue(fmt.Sprintf("https://acctestkv-%s.vault.azure.net/keys/key-%s", data.RandomString, data.RandomString)),
			),
		},
		data.ImportStep("key_size", "key_vault_id"),
	})
