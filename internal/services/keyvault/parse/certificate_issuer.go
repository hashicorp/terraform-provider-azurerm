...
func IssuerID(id string) (*IssuerId, error) {
	...
	if len(components) != 3 {
		return nil, fmt.Errorf("keyVault Certificate Issuer ID should have 3 segments, got %d: '%s'", len(components), path)
	}
	if components[0] != "certificates" || components[1] != "issuers" {
		return nil, fmt.Errorf("keyVault Certificate Issuer ID path must begin with %q", "/certificates/issuers")
	}
