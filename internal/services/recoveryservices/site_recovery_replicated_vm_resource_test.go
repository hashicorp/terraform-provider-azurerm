...
func TestAccSiteRecoveryReplicatedVm_premiumV2LRS(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_site_recovery_replicated_vm", "test")
	r := SiteRecoveryReplicatedVmResource{}

	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.premiumV2LRS(data),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
...
func (SiteRecoveryReplicatedVmResource) premiumV2LRS(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {
    resource_group {
