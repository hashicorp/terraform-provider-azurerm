...
func resourceSiteRecoveryNetworkMappingCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	id := replicationnetworkmappings.NewReplicationNetworkMappingID(subscriptionId, resGroup, vaultName, fabricName, sourceNetworkId, name)
	...
	model := resp.Model
	if model == nil {
		return fmt.Errorf("retrieving site recovery network mapping %s (vault %s): model is nil", name, vaultName)
	}
...
func resourceSiteRecoveryNetworkMappingRead(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if err != nil {
		...
		return fmt.Errorf("making Read request on site recovery network mapping %s (vault %s): %+v", id.NetworkMappingName, id.ResourceName, err)
	}
	...
	if model := resp.Model; model != nil {
		if props := model.Properties; props != nil {
			...
			targetFabricId, err := parse.ReplicationFabricID(*props.RecoveryFabricArmId)
			if err != nil {
				return err
			}
...
func resourceSiteRecoveryNetworkMappingDelete(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("deleting site recovery network mapping %s (vault %s): %+v", id.NetworkMappingName, id.ResourceName, err)
	}
