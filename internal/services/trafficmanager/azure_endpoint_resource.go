...
func resourceAzureEndpointUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if d.HasChange("enabled") {
		...
		existing.Model.Properties.EndpointStatus = &status
	}

	if d.HasChange("custom_header") {
		existing.Model.Properties.CustomHeaders = expandEndpointCustomHeaderConfig(d.Get("custom_header").([]interface{}))
	}

	if d.HasChange("target_resource_id") {
		existing.Model.Properties.TargetResourceId = utils.String(d.Get("target_resource_id").(string))
	}

	if d.HasChange("priority") {
		if priority := d.Get("priority").(int); priority != 0 {
			existing.Model.Properties.Priority = utils.Int64(int64(priority))
		}
	}

	if d.HasChange("weight") {
		if weight := d.Get("weight").(int); weight != 0 {
			existing.Model.Properties.Weight = utils.Int64(int64(weight))
		}
	}
