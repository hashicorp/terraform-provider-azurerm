...
func resourceArmTrafficManagerProfileCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if maxReturn, ok := d.GetOk("max_return"); ok {
		profile.Properties.MaxReturn = pointer.To(int64(int64(maxReturn.(int))))
	}
...
func resourceArmTrafficManagerProfileUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if d.HasChange("max_return") {
		if maxReturn, ok := d.GetOk("max_return"); ok {
			update.Properties.MaxReturn = pointer.To(int64(int64(maxReturn.(int))))
		}
	}
...
func expandArmTrafficManagerMonitorConfig(d *pluginsdk.ResourceData) *profiles.MonitorConfig {
	...
	cfg := profiles.MonitorConfig{
		...
		Port:                      pointer.To(int64(int64(monitor["port"].(int)))),
		Path:                      pointer.To(monitor["path"].(string)),
		IntervalInSeconds:         pointer.To(int64(int64(monitor["interval_in_seconds"].(int)))),
		TimeoutInSeconds:          pointer.To(int64(int64(monitor["timeout_in_seconds"].(int)))),
		ToleratedNumberOfFailures: pointer.To(int64(int64(monitor["tolerated_number_of_failures"].(int)))),
	}
	...
	if v, ok := monitor["expected_status_code_ranges"].([]interface{}); ok {
		...
		for _, r := range v {
			...
			ranges = append(ranges, profiles.MonitorConfigExpectedStatusCodeRangesInlined{
				Min: pointer.To(int64(int64(min))),
				Max: pointer.To(int64(int64(max))),
			})
		}
