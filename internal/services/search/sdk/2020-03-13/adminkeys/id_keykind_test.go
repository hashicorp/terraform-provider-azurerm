...
func TestParseKeyKindID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *KeyKindId
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Search/searchServices/searchServiceValue/regenerateAdminKey",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseKeyKindID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.KeyKind != v.Expected.KeyKind {
			t.Fatalf("Expected %q but got %q for KeyKind", v.Expected.KeyKind, actual.KeyKind)
		}

	}
}

func TestParseKeyKindIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *KeyKindId
	...
	for _, v := range testData {
		...
		actual, err := ParseKeyKindIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForKeyKindId(t *testing.T) {
	segments := KeyKindId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("KeyKindId has no segments")
	}
