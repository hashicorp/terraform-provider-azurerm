...
func TestParseCreateQueryKeyID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Search/searchServices/searchServiceValue/createQueryKey",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseCreateQueryKeyID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseCreateQueryKeyIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *CreateQueryKeyId
	...
	for _, v := range testData {
		...
		actual, err := ParseCreateQueryKeyIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForCreateQueryKeyId(t *testing.T) {
	segments := CreateQueryKeyId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("CreateQueryKeyId has no segments")
	}
