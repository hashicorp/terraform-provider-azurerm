...
func TestAccBotChannelDirectLineSpeech_validation(t *testing.T) {
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		...
		},
		{
			...
			ExpectError: regexp.MustCompile("either `cognitive_account_id` must be specified, or both `cognitive_service_access_key` and `cognitive_service_location` must be specified"),
		},
		{
			Config:      r.conflictingFields(data),
			ExpectError: regexp.MustCompile("when `cognitive_account_id` is specified, `cognitive_service_access_key` and `cognitive_service_location` should not be specified"),
		},
	})
}
