...
// TestAccBotWebApp_V0ToV1_501 tests the state migration path from an `id` with lowercased
...
func TestAccBotWebApp_V0ToV1_501(t *testing.T) {
...
func (r BotWebAppResource) basicConfigV0(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
...
resource "azurerm_bot_web_app" "test-import" {
  name                    = "acctestdf%[2]d"
  location                = "global"
...
import {
  to = azurerm_bot_web_app.test-import
...
func (r BotWebAppResource) basicConfigImported(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
