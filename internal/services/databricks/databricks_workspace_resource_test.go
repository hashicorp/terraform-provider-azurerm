...
func TestAccDatabricksWorkspace_serverless(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_databricks_workspace", "test")
	r := DatabricksWorkspaceResource{}
...
func TestAccDatabricksWorkspace_serverlessComplete(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_databricks_workspace", "test")
	databricksPrincipalID := getDatabricksPrincipalId(data.Client().SubscriptionID)
	r := DatabricksWorkspaceResource{}
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.serverlessComplete(data, databricksPrincipalID),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
...
func (DatabricksWorkspaceResource) serverless(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
...
resource "azurerm_key_vault_access_policy" "managed" {
  ...
  */
  key_vault_id = azurerm_key_vault.test.id
  tenant_id    = azurerm_key_vault.test.tenant_id
  object_id    = azurerm_databricks_access_connector.test.identity[0].principal_id
