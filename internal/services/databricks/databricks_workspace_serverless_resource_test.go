...
resource "azurerm_private_endpoint" "test" {
  ...
  custom_network_interface_name = "acctest-nic-%[2]d"

  private_dns_zone_group {
    name                 = "acctest-pdzg-%[2]d"
...
resource "azurerm_private_endpoint" "test2" {
  name                          = "acctest-pe2-%[2]d"
  location                      = azurerm_resource_group.test.location
  resource_group_name           = azurerm_resource_group.test.name
  subnet_id                     = azurerm_subnet.test.id
  custom_network_interface_name = "acctest-nic2-%[2]d"

  private_dns_zone_group {
    name                 = "acctest-pdzg2-%[2]d"
    private_dns_zone_ids = [azurerm_private_dns_zone.test.id]
  }

  private_service_connection {
    name                           = "acctest-psc2-%[2]d"
    is_manual_connection           = false
    private_connection_resource_id = azurerm_databricks_workspace_serverless.test.id
