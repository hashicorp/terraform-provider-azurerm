	...
	data.DataSourceTest(t, []acceptance.TestStep{
		{
			...
			Check: acceptance.ComposeTestCheckFunc(
				...
				check.That(data.ResourceName).Key(`slice_configurations.0.data_network_configurations.0.allowed_services_ids.max_buffered_packets`).HasValue("200"),
				check.That(data.ResourceName).Key(`slice_configurations.0.data_network_configurations.0.allowed_services_ids.session_aggregate_maximum_bit_rate.0.downlink`).HasValue("1 Gbps"),
				check.That(data.ResourceName).Key(`slice_configurations.0.data_network_configurations.0.allowed_services_ids.session_aggregate_maximum_bit_rate.0.uplink`).HasValue("500 Mbps"),
