	...
	data.DataSourceTest(t, []acceptance.TestStep{
		{
			...
			Check: acceptance.ComposeTestCheckFunc(
				...
				check.That(data.ResourceName).Key(`pcc_rules.0.traffic_control_enabled`).HasValue("true"),
				check.That(data.ResourceName).Key(`pcc_rules.0.rule_qos_policy.0.allocation_and_retention_priority_level`).HasValue("9"),
				check.That(data.ResourceName).Key(`pcc_rules.0.rule_qos_policy.0.qos_indicator`).HasValue("9"),
				check.That(data.ResourceName).Key(`pcc_rules.0.rule_qos_policy.0.preemption_capability`).HasValue("NotPreempt"),
				...
				check.That(data.ResourceName).Key(`pcc_rules.0.service_data_flow_templates.0.protocol.0`).HasValue("ip"),
				check.That(data.ResourceName).Key(`pcc_rules.0.service_data_flow_templates.0.remote_ip_list.0`).HasValue("10.3.4.0/24"),
				...
				check.That(data.ResourceName).Key(`service_qos_policy.0.preemption_vulnerability.maximum_bit_rate.0.downlink`).HasValue("1 Gbps"),
				check.That(data.ResourceName).Key(`service_qos_policy.0.preemption_vulnerability.maximum_bit_rate.0.uplink`).HasValue("100 Mbps"),
