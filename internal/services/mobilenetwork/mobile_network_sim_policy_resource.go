...
type SimPolicyModel struct {
	...
	SliceConfigurations          []SliceConfigurationModel `tfschema:"slice_configurations"`
	Tags                         map[string]string         `tfschema:"tags"`
	UeAmbr                       []AmbrModel               `tfschema:"user_equipment_aggregate_maximum_bit_rate"`
}

type SliceConfigurationModel struct {
	DataNetworkConfigurations []DataNetworkConfigurationModel `tfschema:"data_network_configurations"`
	...
	return sdk.ResourceFunc{
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			if metadata.ResourceData.HasChange("default_slice") {
				properties.Properties.DefaultSlice = simpolicy.SliceResourceId{Id: model.DefaultSliceId}

			}

			if metadata.ResourceData.HasChange("registration_timer_in_seconds") {
				properties.Properties.RegistrationTimer = &model.RegistrationTimer
			}
