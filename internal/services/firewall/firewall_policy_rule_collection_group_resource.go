...
func flattenFirewallPolicyRuleApplication(input *[]firewallpolicyrulecollectiongroups.FirewallPolicyRule) ([]interface{}, error) {
	...
	for _, e := range *input {
		...
		output = append(output, map[string]interface{}{
			...
			"source_addresses":      helpers.FlattenSlice(rule.SourceAddresses),
			"source_ip_groups":      helpers.FlattenSlice(rule.SourceIPGroups),
			"destination_addresses": helpers.FlattenSlice(rule.DestinationAddresses),
			"destination_urls":      helpers.FlattenSlice(rule.TargetURLs),
			"destination_fqdns":     helpers.FlattenSlice(rule.TargetFqdns),
			"destination_fqdn_tags": helpers.FlattenSlice(rule.FqdnTags),
			"terminate_tls":         pointer.From(rule.TerminateTLS),
			"web_categories":        helpers.FlattenSlice(rule.WebCategories),
		})
	}
...
func flattenFirewallPolicyRuleNetwork(input *[]firewallpolicyrulecollectiongroups.FirewallPolicyRule) ([]interface{}, error) {
	...
	for _, e := range *input {
		...
		output = append(output, map[string]interface{}{
			...
			"source_ip_groups":      helpers.FlattenSlice(rule.SourceIPGroups),
			"destination_addresses": helpers.FlattenSlice(rule.DestinationAddresses),
			"destination_ip_groups": helpers.FlattenSlice(rule.DestinationIPGroups),
			"destination_fqdns":     helpers.FlattenSlice(rule.DestinationFqdns),
			"destination_ports":     helpers.FlattenSlice(rule.DestinationPorts),
			"description":           pointer.From(rule.Description),
		})
	}
	return output, nil
