...
type DashboardGrafanaModel struct {
	...
	CsrfAlwaysCheck                          bool                                              `tfschema:"csrf_always_check"`
	...
	SnapshotExternalEnabled                  bool                                              `tfschema:"snapshot_external_enabled"`
	Tags                                     map[string]string                                 `tfschema:"tags"`
	UnifiedAlertingScreenshotsCaptureEnabled bool                                              `tfschema:"unified_alerting_screenshots_capture_enabled"`
	UsersConfiguration                       []UsersConfigurationModel                         `tfschema:"users"`
...
type UsersConfigurationModel struct {
	EditorsCanAdmin bool `tfschema:"editors_can_admin"`
	ViewersCanEdit  bool `tfschema:"viewers_can_edit"`
}
...
func (r DashboardGrafanaResource) Arguments() map[string]*pluginsdk.Schema {
	arguments := map[string]*pluginsdk.Schema{
		...
		"unified_alerting_screenshots_capture_enabled": {
			Type:     pluginsdk.TypeBool,
			Optional: true,
			Default:  false,
		},
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			properties := &managedgrafanas.ManagedGrafana{
				...
				Properties: &managedgrafanas.ManagedGrafanaProperties{
					...
					GrafanaConfigurations:             expandGrafanaConfigurationModel(model.SMTP, model.CsrfAlwaysCheck, model.SnapshotExternalEnabled, model.UnifiedAlertingScreenshotsCaptureEnabled, model.UsersConfiguration),
...
func (r DashboardGrafanaResource) Update() sdk.ResourceFunc {
	return sdk.ResourceFunc{
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			if metadata.ResourceData.HasChange("smtp") || metadata.ResourceData.HasChange("csrf_always_check") || metadata.ResourceData.HasChange("snapshot_external_enabled") || metadata.ResourceData.HasChange("unified_alerting_screenshots_capture_enabled") || metadata.ResourceData.HasChange("users") {
				properties.Properties.GrafanaConfigurations = expandGrafanaConfigurationModel(model.SMTP, model.CsrfAlwaysCheck, model.SnapshotExternalEnabled, model.UnifiedAlertingScreenshotsCaptureEnabled, model.UsersConfiguration)
			}
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			if properties := model.Properties; properties != nil {
				...
				if properties.GrafanaConfigurations != nil {
					...
					state.CsrfAlwaysCheck = flattenCsrfAlwaysCheck(properties.GrafanaConfigurations.Security)
					state.SnapshotExternalEnabled = flattenSnapshotExternalEnabled(properties.GrafanaConfigurations.Snapshots)
					state.UnifiedAlertingScreenshotsCaptureEnabled = flattenUnifiedAlertingScreenshotsCaptureEnabled(properties.GrafanaConfigurations.UnifiedAlertingScreenshots)
					state.UsersConfiguration = flattenUsersConfigurationModel(properties.GrafanaConfigurations.Users)
				}
...
func expandGrafanaConfigurationModel(smtpInput []SMTPConfigurationModel, csrfAlwaysCheck bool, snapshotExternalEnabled bool, unifiedAlertingScreenshotsCaptureEnabled bool, usersInput []UsersConfigurationModel) *managedgrafanas.GrafanaConfigurations {
	config := &managedgrafanas.GrafanaConfigurations{}
	...
	if len(smtpInput) > 0 {
		...
		config.Smtp = pointer.To(smtp)
	}

	config.Security = pointer.To(managedgrafanas.Security{
		CsrfAlwaysCheck: pointer.To(csrfAlwaysCheck),
	})

	config.Snapshots = pointer.To(managedgrafanas.Snapshots{
		ExternalEnabled: pointer.To(snapshotExternalEnabled),
	})

	config.UnifiedAlertingScreenshots = pointer.To(managedgrafanas.UnifiedAlertingScreenshots{
		CaptureEnabled: pointer.To(unifiedAlertingScreenshotsCaptureEnabled),
	})

	if len(usersInput) > 0 {
		v := usersInput[0]
		config.Users = pointer.To(managedgrafanas.Users{
			EditorsCanAdmin: pointer.To(v.EditorsCanAdmin),
			ViewersCanEdit:  pointer.To(v.ViewersCanEdit),
		})
	}
...
func flattenSnapshotExternalEnabled(input *managedgrafanas.Snapshots) bool {
	...
	return pointer.From(input.ExternalEnabled)
}

func flattenUnifiedAlertingScreenshotsCaptureEnabled(input *managedgrafanas.UnifiedAlertingScreenshots) bool {
	if input == nil || input.CaptureEnabled == nil {
		return false
	}
	return pointer.From(input.CaptureEnabled)
}

func flattenUsersConfigurationModel(input *managedgrafanas.Users) []UsersConfigurationModel {
	if input == nil {
		return []UsersConfigurationModel{}
	}

	return []UsersConfigurationModel{
		{
			EditorsCanAdmin: pointer.From(input.EditorsCanAdmin),
			ViewersCanEdit:  pointer.From(input.ViewersCanEdit),
		},
	}
}
