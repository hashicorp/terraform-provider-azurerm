...
type DashboardGrafanaModel struct {
	...
	Configuration                     []GrafanaConfigurationModel                       `tfschema:"configuration"`
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			properties := &grafanaresource.ManagedGrafana{
				...
				Properties: &grafanaresource.ManagedGrafanaProperties{
					...
					GrafanaConfigurations:             expandGrafanaConfigurationsModel(model.Configuration),
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			if properties := model.Properties; properties != nil {
				...
				if properties.GrafanaConfigurations != nil {
					state.Configuration = flattenGrafanaConfigurationModel(properties.GrafanaConfigurations, metadata.ResourceData)
				}
...
func expandGrafanaConfigurationsModel(inputList []GrafanaConfigurationModel) *grafanaresource.GrafanaConfigurations {
	if len(inputList) == 0 {
		return nil
	}

	return &grafanaresource.GrafanaConfigurations{
		Smtp: expandSMTPConfigurationModel(inputList[0].SMTP),
	}
}

func expandSMTPConfigurationModel(input []SMTPConfigurationModel) *grafanaresource.Smtp {
	if len(input) == 0 {
		return nil
	}

	smtp := input[0]
	return &grafanaresource.Smtp{
		Enabled:        pointer.To(smtp.SMTPEnabled),
		FromAddress:    pointer.To(smtp.FromAddress),
		FromName:       pointer.To(smtp.FromName),
		Host:           pointer.To(smtp.Host),
		Password:       pointer.To(smtp.Password),
		SkipVerify:     pointer.To(smtp.SkipVerify),
		User:           pointer.To(smtp.User),
		StartTLSPolicy: (*grafanaresource.StartTLSPolicy)(pointer.To(smtp.StartTLSPolicy)),
	}
}
...
func flattenGrafanaConfigurationModel(input *grafanaresource.GrafanaConfigurations, data *schema.ResourceData) []GrafanaConfigurationModel {
	var outputList []GrafanaConfigurationModel
	if input == nil {
		return outputList
	}

	var output GrafanaConfigurationModel

	if input.Smtp != nil {
		output.SMTP = flattenSMTPConfigurationModel(input.Smtp, data)
	}
...
func flattenSMTPConfigurationModel(input *grafanaresource.Smtp, data *schema.ResourceData) []SMTPConfigurationModel {
	...
	output.Password = data.Get("configuration.0.smtp.0.password").(string)
