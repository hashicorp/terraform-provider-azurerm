...
func (r DashboardGrafanaResource) Arguments() map[string]*pluginsdk.Schema {
	return map[string]*pluginsdk.Schema{
		...
		"auto_generated_domain_name_label_scope": {
			...
			ValidateFunc: validation.StringInSlice(managedgrafanas.PossibleValuesForAutoGeneratedDomainNameLabelScope(), false),
		},
			...
			Elem: &pluginsdk.Resource{
				Schema: map[string]*schema.Schema{
					...
					"start_tls_policy": {
						Type:         pluginsdk.TypeString,
						Required:     true,
						ValidateFunc: validation.StringInSlice(managedgrafanas.PossibleValuesForStartTLSPolicy(), false),
					},
				},
			},
		},
...
func expandCreateForDashboardGrafana(model DashboardGrafanaModel) *managedgrafanas.ManagedGrafana {
	...
	properties = managedgrafanas.ManagedGrafana{
		...
		Properties: &managedgrafanas.ManagedGrafanaProperties{
			...
			GrafanaConfigurations:             expandGrafanaConfigurationsModel(model.SMTP),
...
func (r DashboardGrafanaResource) Update() sdk.ResourceFunc {
	return sdk.ResourceFunc{
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			// The SMTP password is never returned by the API, so it must be re-expanded from config on every update,
			// not just when `smtp` changes, otherwise Azure rejects the request with `InvalidConfigurationForSMTP`.
			if properties.Properties.GrafanaConfigurations != nil {
				properties.Properties.GrafanaConfigurations.Smtp = expandSMTPConfigurationModel(model.SMTP)
			} else {
				properties.Properties.GrafanaConfigurations = expandGrafanaConfigurationsModel(model.SMTP)
			}
...
func expandGrafanaConfigurationsModel(input []SMTPConfigurationModel) *managedgrafanas.GrafanaConfigurations {
	smtp := expandSMTPConfigurationModel(input)
	if smtp == nil {
		return nil
	}

	return &managedgrafanas.GrafanaConfigurations{Smtp: smtp}
}

func expandSMTPConfigurationModel(input []SMTPConfigurationModel) *managedgrafanas.Smtp {
	if len(input) == 0 {
		return nil
	}

	v := input[0]
	return &managedgrafanas.Smtp{
