...
// This is fixing issue 17843
func TestAccKubernetesCluster_addonProfileConflictingChanges(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_kubernetes_cluster", "test")
	r := KubernetesClusterResource{}

	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			// Enable policy without OSM addon
...
func (KubernetesClusterResource) addonProfilesConflictingConfig(data acceptance.TestData, policyEnabled bool, osmEnabled bool) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
...
`, data.RandomInteger, data.Locations.Primary, data.RandomInteger, data.RandomInteger, data.RandomInteger, policyEnabled, osmEnabled)
}
