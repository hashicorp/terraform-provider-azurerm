	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			...
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
				check.That(data.ResourceName).Key("confidential_computing_identity.0.client_id").Exists(),
				check.That(data.ResourceName).Key("confidential_computing_identity.0.object_id").Exists(),
				check.That(data.ResourceName).Key("confidential_computing_identity.0.user_assigned_identity_id").Exists(),
			),
		},
		data.ImportStep(),
		{
			...
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
				check.That(data.ResourceName).Key("confidential_computing_identity.0.client_id").DoesNotExist(),
				check.That(data.ResourceName).Key("confidential_computing_identity.0.object_id").DoesNotExist(),
				check.That(data.ResourceName).Key("confidential_computing_identity.0.user_assigned_identity_id").DoesNotExist(),
			),
		},
		data.ImportStep(),
		{
...
resource "azurerm_kubernetes_cluster" "test" {  
  name                = "acctestaks%d"  
  location            = azurerm_resource_group.test.location  resource_group_name = azurerm_resource_group.test.name  dns_prefix          = "acctestaks%d"  
  
  linux_profile {    admin_username = "acctestuser%d"  
  
    ssh_key {      key_data = "ssh-rsa  terraform@demo.tld"    }  }  
  default_node_pool {    name       = "default"    node_count = 1    vm_size    = "Standard_DS2_v2"  }  
  confidential_computing_enabled = %t  
