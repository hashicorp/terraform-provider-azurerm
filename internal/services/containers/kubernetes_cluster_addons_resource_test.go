...
func TestAccKubernetesCluster_addonProfileServiceMeshProfile_certificateAuthority(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_kubernetes_cluster", "test")
	r := KubernetesClusterResource{}
...
func TestAccKubernetesCluster_addonProfileServiceMeshProfile_revisions(t *testing.T) {
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		...
		data.ImportStep(),
		{
			Config: r.addonProfileServiceMeshProfileRevisionsConfig(data, `["asm-1-24"]`),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
		data.ImportStep(),
		{
			Config: r.addonProfileServiceMeshProfileRevisionsConfig(data, `["asm-1-24", "asm-1-25"]`),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
...
func (KubernetesClusterResource) addonProfileServiceMeshProfileRevisionsConfig(data acceptance.TestData, revisions string) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
