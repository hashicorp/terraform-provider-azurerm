...
func resourceKubernetesClusterCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if nodePoolVersion := agentProfile.OrchestratorVersion; nodePoolVersion != nil {
	...
	mcSKUTier := managedclusters.ManagedClusterSKUTier(d.Get("sku_tier").(string))
	mcSKUBasic := managedclusters.ManagedClusterSKUNameBasic
	...
	parameters := managedclusters.ManagedCluster{
		...
		Sku: &managedclusters.ManagedClusterSKU{
			Name: &mcSKUBasic, // the only possible value at this point
			Tier: &mcSKUTier,
		},
	...
	if v := d.Get("automatic_channel_upgrade").(string); v != "" {
		upgradeChannel := managedclusters.UpgradeChannel(v)
		parameters.Properties.AutoUpgradeProfile = &managedclusters.ManagedClusterAutoUpgradeProfile{
			UpgradeChannel: &upgradeChannel,
		}
	} else {
		upgradeChannelNone := managedclusters.UpgradeChannelNone
		parameters.Properties.AutoUpgradeProfile = &managedclusters.ManagedClusterAutoUpgradeProfile{
	...
	if maintenanceConfigRaw, ok := d.GetOk("maintenance_window"); ok {
		...
		maintenanceConfigId := maintenanceconfigurations.NewMaintenanceConfigurationID(id.SubscriptionId, id.ResourceGroupName, id.ResourceName, "default")
		...
		if _, err := client.CreateOrUpdate(ctx, maintenanceConfigId, parameters); err != nil {
...
func resourceKubernetesClusterUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	clusterClient := containersClient.ManagedClustersClient
	...
	if d.HasChange("service_principal") && !d.HasChange("identity") {
		...
		_, err := clusterClient.ResetServicePrincipalProfile(ctx, *id, params)
		if err != nil {
	...
	if d.HasChange("azure_active_directory_role_based_access_control") {
		...
		if props.AadProfile != nil && (props.AadProfile.Managed == nil || !*props.AadProfile.Managed) {
			...
			_, err := clusterClient.ResetAADProfile(ctx, *id, *props.AadProfile)
			if err != nil {
	...
	if d.HasChange("sku_tier") {
		...
		if existing.Model.Sku == nil {
			skuName := managedclusters.ManagedClusterSKUNameBasic
	...
	if d.HasChange("default_node_pool") {
		...
		if nodePoolVersion := agentProfile.Properties.OrchestratorVersion; nodePoolVersion != nil {
			...
			nodePoolMCId := agentpools.NewManagedClusterID(id.SubscriptionId, id.ResourceGroupName, id.ResourceName)
			if err := validateNodePoolSupportsVersion(ctx, containersClient, currentNodePoolVersion, nodePoolMCId, defaultNodePoolId.AgentPoolName, *nodePoolVersion); err != nil {
				return err
			}
		}

		err = nodePoolsClient.CreateOrUpdateThenPoll(ctx, defaultNodePoolId, agentProfile)
		if err != nil {
...
func resourceKubernetesClusterRead(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	identity, err := flattenClusterIdentity(resp.Model.Identity)
	if err != nil {
	...
	maintenanceConfigId := maintenanceconfigurations.NewMaintenanceConfigurationID(id.SubscriptionId, id.ResourceGroupName, id.ResourceName, "default")
	configResp, _ := maintenanceConfigurationsClient.Get(ctx, maintenanceConfigId)
	if props := configResp.Model.Properties; props != nil {
...
func resourceKubernetesClusterDelete(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if _, ok := d.GetOk("maintenance_window"); ok {
		...
		if _, err := client.Delete(ctx, maintenanceConfigId); err != nil {
			return fmt.Errorf("deleting Maintenance Configuration for %s: %+v", *id, err)
		}
	}

	ignorePodDisruptionBudget := managedclusters.DeleteOperationOptions{
...
func flattenKubernetesClusterWindowsProfile(profile *managedclusters.ManagedClusterWindowsProfile, d *pluginsdk.ResourceData) []interface{} {
	...
	var license managedclusters.LicenseType
	if *profile.LicenseType != managedclusters.LicenseTypeNone {
...
func expandKubernetesClusterNetworkProfile(input []interface{}) (*managedclusters.ContainerServiceNetworkProfile, error) {
	...
	networkPlugin := config["network_plugin"].(managedclusters.NetworkPlugin)
	networkMode := config["network_mode"].(managedclusters.NetworkMode)
	if networkPlugin != "azure" && networkMode != "" {
		return nil, fmt.Errorf("`network_mode` cannot be set if `network_plugin` is not `azure`")
	}
	networkPolicy := config["network_policy"].(managedclusters.NetworkPolicy)
	loadBalancerProfileRaw := config["load_balancer_profile"].([]interface{})
	loadBalancerSku := config["load_balancer_sku"].(managedclusters.LoadBalancerSku)
	natGatewayProfileRaw := config["nat_gateway_profile"].([]interface{})
	outboundType := config["outbound_type"].(managedclusters.OutboundType)
	...
	if len(loadBalancerProfileRaw) > 0 {
		if loadBalancerSku != managedclusters.LoadBalancerSkuStandard {
...
func flattenKubernetesClusterNetworkProfile(profile *managedclusters.ContainerServiceNetworkProfile) []interface{} {
	...
	return []interface{}{
		map[string]interface{}{
			...
			"outbound_type":         profile.OutboundType,
		},
	}
}
...
func flattenClusterIdentity(input *identity.SystemOrUserAssignedMap) (*[]interface{}, error) {
...
func expandKubernetesClusterAutoScalerProfile(input []interface{}) *managedclusters.ManagedClusterPropertiesAutoScalerProfile {
	...
	expander := config["expander"].(managedclusters.Expander)
...
func expandKubernetesClusterMaintenanceConfigurationTimeInWeeks(input []interface{}) *[]maintenanceconfigurations.TimeInWeek {
	...
	for _, item := range input {
		v := item.(map[string]interface{})
		day := v["day"].(maintenanceconfigurations.WeekDay)
...
func expandUpgradeSettingsForCluster(input []interface{}) *managedclusters.AgentPoolUpgradeSettings {
...
func flattenUpgradeSettingsFromCluster(input *managedclusters.AgentPoolUpgradeSettings) []interface{} {
	maxSurge := ""
