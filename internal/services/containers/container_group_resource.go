...
func resourceContainerGroupUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	parameters := containerinstance.ContainerGroup{
		...
		Properties: containerinstance.ContainerGroupPropertiesProperties{
			...
			InitContainers:           initContainers,
			Containers:               containers,
			Diagnostics:              expandContainerGroupDiagnostics(d.Get("diagnostics").([]interface{})),
			RestartPolicy:            &restartPolicy,
			OsType:                   containerinstance.OperatingSystemTypes(OSType),
			Volumes:                  &containerGroupVolumes,
			ImageRegistryCredentials: expandContainerImageRegistryCredentials(d),
			DnsConfig:                expandContainerGroupDnsConfig(d.Get("dns_config").([]interface{})),
			SubnetIds:                subnets,
		},
		Zones: &zones,
	}
	...
	if IPAddressType != "None" {
		parameters.Properties.IPAddress = &containerinstance.IPAddress{
			Ports: containerGroupPorts,
			Type:  containerinstance.ContainerGroupIPAddressType(IPAddressType),
		}

		if dnsNameLabel := d.Get("dns_name_label").(string); dnsNameLabel != "" {
			parameters.Properties.IPAddress.DnsNameLabel = &dnsNameLabel
		}
		if dnsNameLabelReusePolicy := d.Get("dns_name_label_reuse_policy").(string); dnsNameLabelReusePolicy != "" {
			parameters.Properties.IPAddress.AutoGeneratedDomainNameLabelScope = (*containerinstance.DnsNameLabelReusePolicy)(&dnsNameLabelReusePolicy)
		}
	}

	if err := client.ContainerGroupsCreateOrUpdateThenPoll(ctx, *id, parameters); err != nil {
	...
	if keyVaultKeyId := d.Get("key_vault_key_id").(string); keyVaultKeyId != "" {
		...
		parameters.Properties.EncryptionProperties = &containerinstance.EncryptionProperties{
			VaultBaseUrl: keyId.KeyVaultBaseUrl,
			KeyName:      keyId.Name,
			KeyVersion:   keyId.Version,
		}
		...
		if keyVaultUAI := d.Get("key_vault_user_assigned_identity_id").(string); keyVaultUAI != "" {
			parameters.Properties.EncryptionProperties.Identity = &keyVaultUAI
		}
	}

	if priority := d.Get("priority").(string); priority != "" {
		parameters.Properties.Priority = pointer.To(containerinstance.ContainerGroupPriority(priority))
	}
