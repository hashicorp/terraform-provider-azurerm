...
func expandAdditionalUnattendContent(input []interface{}) *[]virtualmachines.AdditionalUnattendContent {
	...
	for _, v := range input {
		...
		output = append(output, virtualmachines.AdditionalUnattendContent{
			...
			PassName:      pointer.To(virtualmachines.PassNameOobeSystem),
			ComponentName: pointer.To(virtualmachines.ComponentNameMicrosoftNegativeWindowsNegativeShellNegativeSetup),
		})
	}
