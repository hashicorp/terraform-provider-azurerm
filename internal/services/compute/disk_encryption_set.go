...
func retrieveManagedDiskEncryptionSetEncryptionType(ctx context.Context, client *compute.DiskEncryptionSetsClient, diskEncryptionSetId string) (*managedDisks.EncryptionType, error) {
	...
	var encryptionType *managedDisks.EncryptionType
	if props := resp.EncryptionSetProperties; props != nil && string(props.EncryptionType) != "" {
		v := managedDisks.EncryptionType(props.EncryptionType)
		encryptionType = &v
	}
