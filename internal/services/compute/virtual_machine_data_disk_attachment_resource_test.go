...
func TestAccVirtualMachineDataDiskAttachment_stopVMBeforeDetaching(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_virtual_machine_data_disk_attachment", "test")
	r := VirtualMachineDataDiskAttachmentResource{}
	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.stopVMBeforeDetaching(data),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
				check.That(data.ResourceName).Key("virtual_machine_id").Exists(),
				check.That(data.ResourceName).Key("managed_disk_id").Exists(),
...
func (VirtualMachineDataDiskAttachmentResource) stopVMBeforeDetaching(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {
    managed_disk {
