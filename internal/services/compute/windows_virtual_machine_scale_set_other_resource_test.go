...
func TestAccWindowsVirtualMachineScaleSet_updateHealthProbeSku(t *testing.T) {
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		...
		data.ImportStep("admin_password"),
		// Have to remove VMSS to change the sku of load balancer
		{
			Config: r.updateLoadBalancerHealthProbeSKUBasicVmssRemoved(data),
			Check:  acceptance.ComposeTestCheckFunc(),
		},
		{
...
func (r WindowsVirtualMachineScaleSetResource) updateLoadBalancerHealthProbeSKUBasicVmssRemoved(data acceptance.TestData) string {
	return fmt.Sprintf(`
%[1]s
