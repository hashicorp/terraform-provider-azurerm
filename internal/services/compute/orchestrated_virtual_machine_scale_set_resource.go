...
func resourceOrchestratedVirtualMachineScaleSetCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	props := virtualmachinescalesets.VirtualMachineScaleSet{
		...
		Properties: &virtualmachinescalesets.VirtualMachineScaleSetProperties{
			PlatformFaultDomainCount: utils.Int64(int64(d.Get("platform_fault_domain_count").(int))),
	...
	if !pluginsdk.IsExplicitlyNullInConfig(d, "single_placement_group") {
		props.Properties.SinglePlacementGroup = utils.Bool(d.Get("single_placement_group").(bool))
	}
	...
	if v, ok := d.GetOk("boot_diagnostics"); ok {
		virtualMachineProfile.DiagnosticsProfile = expandVMSSBootDiagnostics(v.([]interface{}))
	}
	...
	} else if pointer.From(virtualMachineProfile.Priority) == virtualmachinescalesets.VirtualMachinePriorityTypesSpot {
	...
	if !isLegacy {
		if v, ok := d.GetOk("plan"); ok {
			props.Plan = expandVMSSPlan(v.([]interface{}))
		}
		...
		if len(osProfileRaw) > 0 {
			...
			if len(winConfigRaw) > 0 {
				...
				if d.HasChange("os_profile.0.windows_configuration.0.secret") {
					vmssOsProfile.Secrets = expandVMSSWindowsSecrets(winConfig["secret"].([]interface{}))
				}
				...
				if d.HasChange("os_profile.0.windows_configuration.0.winrm_listener") {
					...
					vmssOsProfile.WindowsConfiguration.WinRM = expandVMSSWinRMListener(winRmListenersRaw)
				}
			...
			if len(linConfigRaw) > 0 {
				...
				if d.HasChange("os_profile.0.linux_configuration.0.admin_ssh_key") {
					sshPublicKeys := ExpandVMSSSSHKeys(linConfig["admin_ssh_key"].(*pluginsdk.Set).List())
...
func resourceOrchestratedVirtualMachineScaleSetRead(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if model := resp.Model; model != nil {
		...
		if resp.Model.Sku != nil {
			skuName, err = flattenOrchestratedVirtualMachineScaleSetSku(resp.Model.Sku)
...
func flattenOrchestratedVirtualMachineScaleSetPublicIPSku(input *virtualmachinescalesets.PublicIPAddressSku) string {
	var skuName string
	if input != nil {
		if string(pointer.From(input.Name)) != "" && string(pointer.From(input.Tier)) != "" {
			skuName = fmt.Sprintf("%s_%s", string(pointer.From(input.Name)), string(pointer.From(input.Tier)))
		}
	}
