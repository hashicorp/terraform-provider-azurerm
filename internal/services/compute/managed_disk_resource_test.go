...
func TestAccAzureRMManagedDisk_update_withPurchasePlan(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_managed_disk", "test")
	r := ManagedDiskResource{}

	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.create_withPurchasePlan(data),
			Check: resource.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
		data.ImportStep(),
		{
			Config: r.update_withPurchasePlan(data),
			Check: resource.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
...
func (ManagedDiskResource) create_withPurchasePlan(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
...
resource "azurerm_managed_disk" "test" {
  ...
  purchase_plan {
    name      = "testPurchasePlanName"
    product   = "testPurchasePlanProduct"
    publisher = "testPurchasePublisher"
  }
}
`, data.RandomInteger, data.Locations.Primary, data.RandomInteger)
}

func (ManagedDiskResource) update_withPurchasePlan(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
...
resource "azurerm_managed_disk" "test" {
  ...
  purchase_plan {
    name           = "testPurchasePlanNameUpdated"
    product        = "testPurchasePlanProductUpdated"
    publisher      = "testPurchasePublisherUpdated"
    promotion_code = "testPurchasePromotionCodeUpdated"
  }
}
`, data.RandomInteger, data.Locations.Primary, data.RandomInteger)
}
