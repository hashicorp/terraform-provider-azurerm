...
func resourceWindowsVirtualMachineScaleSetCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	additionalUnattendContent := expandVMSSAdditionalUnattendContent(additionalUnattendContentRaw)
	...
	winRmListeners := expandVMSSWinRMListener(winRmListenersRaw)

	secretsRaw := d.Get("secret").([]interface{})
	secrets := expandVMSSWindowsSecrets(secretsRaw)
...
func resourceWindowsVirtualMachineScaleSetUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if existing.Model.Properties.VirtualMachineProfile.StorageProfile == nil {
		return fmt.Errorf("retrieving %s: `properties.virtualMachineProfile,storageProfile` was nil", id)
	}
	...
	if d.HasChange("enable_automatic_updates") ||
		...
		if d.HasChange("secret") {
			secretsRaw := d.Get("secret").([]interface{})
			osProfile.Secrets = expandVMSSWindowsSecrets(secretsRaw)
		}
		...
		if props := model.Properties; props != nil {
			...
			if profile := props.VirtualMachineProfile; profile != nil {
				...
				if osProfile := profile.OsProfile; osProfile != nil {
					...
					if err := d.Set("secret", flattenVMSSWindowsSecrets(osProfile.Secrets)); err != nil {
						return fmt.Errorf("setting `secret`: %+v", err)
					}

					if windows := osProfile.WindowsConfiguration; windows != nil {
						if err := d.Set("additional_unattend_content", flattenVMSSAdditionalUnattendContent(windows.AdditionalUnattendContent, d)); err != nil {
						...
						if err := d.Set("winrm_listener", flattenVMSSWinRMListener(windows.WinRM)); err != nil {
