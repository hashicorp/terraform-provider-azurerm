...
func resourceLinuxVirtualMachineCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	sshKeys := ExpandSSHKeys(d.Get("admin_ssh_key").(*pluginsdk.Set).List())
	...
	params := virtualmachines.VirtualMachine{
		...
		Properties: &virtualmachines.VirtualMachineProperties{
			...
			StorageProfile: &virtualmachines.StorageProfile{
				ImageReference: expandSourceImageReference(d.Get("source_image_reference").([]interface{}), d.Get("source_image_id").(string)),
				OsDisk:         osDisk,
			...
			DiagnosticsProfile:     expandBootDiagnostics(d.Get("boot_diagnostics").([]interface{})),
	...
	if !provisionVMAgent && pointer.From(params.Properties.OsProfile.AllowExtensionOperations) {
	...
	} else if pointer.From(params.Properties.Priority) == virtualmachines.VirtualMachinePriorityTypesSpot {
	...
	if disablePasswordAuthentication && len(sshKeys) == 0 {
		...
		params.Properties.OsProfile.AdminPassword = utils.String(adminPassword)
	}
		...
		if props := model.Properties; props != nil {
			...
			if profile := props.OsProfile; profile != nil {
				...
				if config := profile.LinuxConfiguration; config != nil {
					...
					if patchSettings := config.PatchSettings; patchSettings != nil && pointer.From(patchSettings.PatchMode) != "" {
						patchMode = string(pointer.From(patchSettings.PatchMode))
					}
					d.Set("patch_mode", patchMode)

					assessmentMode := string(virtualmachines.LinuxPatchAssessmentModeImageDefault)
					if patchSettings := config.PatchSettings; patchSettings != nil && pointer.From(patchSettings.AssessmentMode) != "" {
...
func resourceLinuxVirtualMachineUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if d.HasChange("license_type") {
		...
		if v, ok := d.GetOk("license_type"); ok {
			update.Properties.LicenseType = utils.String(v.(string))
		}
	}
	...
	if shouldShutDown {
		...
		forceShutdown := false
		if err = client.PowerOffThenPoll(ctx, *id, virtualmachines.PowerOffOperationOptions{SkipShutdown: utils.Bool(forceShutdown)}); err != nil {
	...
	if shouldDeallocate {
		if !hasEphemeralOSDisk {
			...
			if err := client.DeallocateThenPoll(ctx, *id, virtualmachines.DeallocateOperationOptions{Hibernate: utils.Bool(false)}); err != nil {
...
func resourceLinuxVirtualMachineDelete(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if !meta.(*clients.Client).Features.VirtualMachine.SkipShutdownAndForceDelete {
		...
		if existing.Model != nil && existing.Model.Properties != nil && existing.Model.Properties.ProvisioningState != nil && strings.EqualFold(*existing.Model.Properties.ProvisioningState, "failed") {
			log.Printf("[DEBUG] Powering Off %s was skipped because the VM was in %q state.", id, *existing.Model.Properties.ProvisioningState)
		} else {
			// ISSUE: 4920
			...
			if err = client.PowerOffThenPoll(ctx, *id, virtualmachines.PowerOffOperationOptions{SkipShutdown: utils.Bool(skipShutdown)}); err != nil {
				return fmt.Errorf("powering off %s: %+v", id, err)
			}
	...
	if deleteOSDisk {
		...
		if managedDiskId != "" {
			...
			if err := disksClient.DeleteThenPoll(ctx, *diskId); err != nil {
				return fmt.Errorf("deleting OS Disk %q (Resource Group %q) for %s: %+v", diskId.DiskName, diskId.ResourceGroupName, id, err)
			}
	...
	if !response.WasNotFound(virtualMachine.HttpResponse) {
		log.Printf("[INFO] Linux Virtual Machine still exists, waiting on %s to be deleted", id)
		...
		deleteWait := &pluginsdk.StateChangeConf{
			...
			Refresh: func() (interface{}, string, error) {
				log.Printf("[INFO] checking on state of %q", id)
				resp, err := client.Get(ctx, *id, virtualmachines.DefaultGetOperationOptions())
				if err != nil {
					if response.WasNotFound(resp.HttpResponse) {
