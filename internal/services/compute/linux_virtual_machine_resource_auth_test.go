...
func TestAccLinuxVirtualMachine_authSSHED25519(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_linux_virtual_machine", "test")
	r := LinuxVirtualMachineResource{}

	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.authSSHED25519(data),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
...
func (r LinuxVirtualMachineResource) authSSHED25519(data acceptance.TestData) string {
	return fmt.Sprintf(`
%s
...
resource "azurerm_linux_virtual_machine" "test" {
  ...
  admin_ssh_key {
    username   = "adminuser"
    public_key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAINreJb2zmSALgQUjzq/vKsf05fp5kM0ZGl8YDsP1FdWc you@me.com"
  }
