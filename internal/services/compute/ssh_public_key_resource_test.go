...
func TestAccSshPublicKey_CreateUpdate(t *testing.T) {
	...
	key3 := "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAINreJb2zmSALgQUjzq/vKsf05fp5kM0ZGl8YDsP1FdWc you@me.com"
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		...
		data.ImportStep(),
		{
			Config: r.template(data, key3),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
				check.That(data.ResourceName).Key("public_key").HasValue(key3),
			),
		},
		data.ImportStep(),
	})
