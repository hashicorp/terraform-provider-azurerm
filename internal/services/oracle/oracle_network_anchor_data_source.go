...
type NetworkAnchorDataModel struct {
	...
	DnsForwardingRuleModel             []DnsForwardingRuleModel `tfschema:"dns_forwarding_rule"`
}

type DnsForwardingRuleModel struct {
	DomainNames         string `json:"domain_names"`
	ForwardingIPAddress string `json:"forwarding_ip_address"`
}
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			if model := resp.Model; model != nil {
				...
				if props := model.Properties; props != nil {
					...
					state.DnsListeningEndpointAllowedCidr = pointer.From(props.DnsListeningEndpointAllowedCidrs)
					state.DnsForwardingRuleModel = FlattenDnsForwardingRules(props.DnsForwardingRules)
				}
			}
