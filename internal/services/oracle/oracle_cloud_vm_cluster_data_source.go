...
type CloudVmClusterDataModel struct {
	...
	FileSystemConfigurationDetails []FileSystemConfigurationDetailsModel `tfschema:"file_system_configuration_details"`
...
type FileSystemConfigurationDetailsModel struct {
	MountPoint       string `tfschema:"mount_point"`
	FileSystemSizeGb int64  `tfschema:"file_system_size_gb"`
}
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			if model := resp.Model; model != nil {
				...
				if props := model.Properties; props != nil {
					...
					state.FileSystemConfigurationDetails = FlattenFileSystemConfigurationDetails(props.FileSystemConfigurationDetails)
...
func FlattenFileSystemConfigurationDetails(input *[]cloudvmclusters.FileSystemConfigurationDetails) []FileSystemConfigurationDetailsModel {
	output := make([]FileSystemConfigurationDetailsModel, 0)
	if input != nil {
		for _, item := range *input {
			output = append(output, FileSystemConfigurationDetailsModel{
				MountPoint:       pointer.From(item.MountPoint),
				FileSystemSizeGb: pointer.From(item.FileSystemSizeGb),
			})
		}
	}
	return output
