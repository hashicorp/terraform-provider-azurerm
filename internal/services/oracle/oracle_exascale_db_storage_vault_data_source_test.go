	...
	data.DataSourceTest(t, []acceptance.TestStep{
		{
			...
			Check: acceptance.ComposeTestCheckFunc(
				...
				check.That(data.ResourceName).Key("high_capacity_database_storage").Exists(),
				check.That(data.ResourceName).Key("lifecycle_state").Exists(),
				check.That(data.ResourceName).Key("lifecycle_details").Exists(),
				check.That(data.ResourceName).Key("virtual_machine_cluster_count").Exists(),
