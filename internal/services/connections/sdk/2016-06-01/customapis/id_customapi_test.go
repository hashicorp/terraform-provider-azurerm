...
func TestParseCustomApiID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Web/customApis",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseCustomApiID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseCustomApiIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *CustomApiId
	...
	for _, v := range testData {
		...
		actual, err := ParseCustomApiIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForCustomApiId(t *testing.T) {
	segments := CustomApiId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("CustomApiId has no segments")
	}
