	...
	stream.Results = func(push func(list.ListResult) bool) {
		...
		for _, connection := range connectionsResp.Items {
			...
			if err := r.flatten(meta, connectionId, &connection, nil); err != nil {
				sdk.SetErrorDiagnosticAndPushListResult(result, push, fmt.Sprintf("encoding `%s` resource data", CognitiveAccountConnectionAccountManagedIdentityResource{}.ResourceType()), err)
				return
			}
