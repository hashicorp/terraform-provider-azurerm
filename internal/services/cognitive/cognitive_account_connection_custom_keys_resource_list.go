...
type CognitiveAccountConnectionCustomKeysListResource struct{}

var _ sdk.FrameworkListWrappedResource = new(CognitiveAccountConnectionCustomKeysListResource)

func (CognitiveAccountConnectionCustomKeysListResource) ResourceFunc() *pluginsdk.Resource {
	return sdk.WrappedResource(CognitiveAccountConnectionCustomKeysResource{})
}

func (CognitiveAccountConnectionCustomKeysListResource) Metadata(_ context.Context, _ resource.MetadataRequest, response *resource.MetadataResponse) {
	response.TypeName = CognitiveAccountConnectionCustomKeysResource{}.ResourceType()
}

func (CognitiveAccountConnectionCustomKeysListResource) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, response *list.ListResourceSchemaResponse) {
	response.Schema = cognitiveAccountConnectionListResourceConfigSchema()
}

func (CognitiveAccountConnectionCustomKeysListResource) List(ctx context.Context, request list.ListRequest, stream *list.ListResultsStream, metadata sdk.ResourceMetadata) {
	...
	if err != nil {
		sdk.SetResponseErrorDiagnostic(stream, fmt.Sprintf("listing `%s`", CognitiveAccountConnectionCustomKeysResource{}.ResourceType()), err)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		...
		for _, account := range accounts {
			...
			for _, connection := range connectionsResp.Items {
				...
				rd := sdk.WrappedResource(CognitiveAccountConnectionCustomKeysResource{}).Data(&terraform.InstanceState{})
				...
				_ = rd.Set("custom_keys", map[string]string{})
