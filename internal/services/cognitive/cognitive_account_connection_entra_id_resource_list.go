...
type CognitiveAccountConnectionEntraIDListResource struct{}

var _ sdk.FrameworkListWrappedResource = new(CognitiveAccountConnectionEntraIDListResource)

func (CognitiveAccountConnectionEntraIDListResource) ResourceFunc() *pluginsdk.Resource {
	return sdk.WrappedResource(CognitiveAccountConnectionEntraIDResource{})
}
...
func (CognitiveAccountConnectionEntraIDListResource) List(ctx context.Context, request list.ListRequest, stream *list.ListResultsStream, metadata sdk.ResourceMetadata) {
	...
	stream.Results = func(push func(list.ListResult) bool) {
		...
		for _, connection := range connectionsResp.Items {
			...
			if base.AuthType != accountconnectionresource.ConnectionAuthTypeAAD {
				continue
			}
			...
			if err := r.flatten(meta, connectionId, &connection, nil); err != nil {
				sdk.SetErrorDiagnosticAndPushListResult(result, push, fmt.Sprintf("encoding `%s` resource data", CognitiveAccountConnectionEntraIDResource{}.ResourceType()), err)
				return
			}
