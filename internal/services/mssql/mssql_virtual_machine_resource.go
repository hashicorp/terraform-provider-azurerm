		...
		Properties: &sqlvirtualmachines.SqlVirtualMachineProperties{
			...
			ServerConfigurationsManagementSettings: &sqlvirtualmachines.ServerConfigurationsManagementSettings{
				...
				SqlConnectivityUpdateSettings: &sqlvirtualmachines.SqlConnectivityUpdateSettings{
					...
					Port:                  pointer.To(int64(int64(d.Get("sql_connectivity_port").(int)))),
...
func expandSqlVirtualMachineAutoBackupSettings(input []interface{}) (*sqlvirtualmachines.AutoBackupSettings, error) {
	...
	if len(input) > 0 {
		...
		if v, ok := config["retention_period_in_days"]; ok {
			ret.RetentionPeriod = pointer.To(int64(int64(v.(int))))
		}
		...
		if v, ok := config["manual_schedule"]; ok && len(v.([]interface{})) > 0 {
			...
			ret.FullBackupStartTime = pointer.To(int64(int64(manualSchedule["full_backup_start_hour"].(int))))
			ret.FullBackupWindowHours = pointer.To(int64(int64(manualSchedule["full_backup_window_in_hours"].(int))))
			ret.LogBackupFrequency = pointer.To(int64(int64(manualSchedule["log_backup_frequency_in_minutes"].(int))))
		}
	}
...
func expandSqlVirtualMachineAutoPatchingSettings(input []interface{}) *sqlvirtualmachines.AutoPatchingSettings {
	...
	return &sqlvirtualmachines.AutoPatchingSettings{
		...
		MaintenanceWindowDuration:     pointer.To(int64(int64(autoPatchingSetting["maintenance_window_duration_in_minutes"].(int)))),
		MaintenanceWindowStartingHour: pointer.To(int64(int64(autoPatchingSetting["maintenance_window_starting_hour"].(int)))),
		DayOfWeek:                     &dayOfWeek,
	}
}
...
func expandSqlVirtualMachineAssessmentSettingsSchedule(input []interface{}) *sqlvirtualmachines.Schedule {
	...
	if weeklyInterval := scheduleConfig["weekly_interval"].(int); weeklyInterval != 0 {
		schedule.WeeklyInterval = pointer.To(int64(int64(weeklyInterval)))
	}

	if monthlyOccurrence := scheduleConfig["monthly_occurrence"].(int); monthlyOccurrence != 0 {
		schedule.MonthlyOccurrence = pointer.To(int64(int64(monthlyOccurrence)))
	}
...
func expandSqlVirtualMachineTempDbSettings(input []interface{}) *sqlvirtualmachines.SQLTempDbSettings {
	...
	return &sqlvirtualmachines.SQLTempDbSettings{
		...
		DataFileCount:   pointer.To(int64(int64(tempDbSettings["data_file_count"].(int)))),
		DataFileSize:    pointer.To(int64(int64(tempDbSettings["data_file_size_mb"].(int)))),
		DataGrowth:      pointer.To(int64(int64(tempDbSettings["data_file_growth_in_mb"].(int)))),
		LogFileSize:     pointer.To(int64(int64(tempDbSettings["log_file_size_mb"].(int)))),
		LogGrowth:       pointer.To(int64(int64(tempDbSettings["log_file_growth_mb"].(int)))),
	}
}
...
func expandSqlVirtualMachineSQLInstance(input []interface{}) (*sqlvirtualmachines.SQLInstanceSettings, error) {
	...
	result := sqlvirtualmachines.SQLInstanceSettings{
		...
		MaxDop:                             pointer.To(int64(int64(settings["max_dop"].(int)))),
		MaxServerMemoryMB:                  pointer.To(int64(int64(maxServerMemoryMB))),
		MinServerMemoryMB:                  pointer.To(int64(int64(minServerMemoryMB))),
	}
