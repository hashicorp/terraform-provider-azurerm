...
var _ resourceid.Formatter = AnalyticsAccountId{}

func TestAnalyticsAccountIDFormatter(t *testing.T) {
	actual := NewAnalyticsAccountID("12345678-1234-9876-4563-123456789012", "resGroup1", "account1").ID()
	expected := "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/resGroup1/providers/Microsoft.DataLakeAnalytics/accounts/account1"
	if actual != expected {
		t.Fatalf("Expected %q but got %q", expected, actual)
	}
}

func TestAnalyticsAccountID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *AnalyticsAccountId
	}{
	...
	for _, v := range testData {
		...
		actual, err := AnalyticsAccountID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
