...
func (c ComputePoliciesClient) ListByAccount(ctx context.Context, id AccountId) (resp ListByAccountResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "computepolicies.ComputePoliciesClient", "ListByAccount", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "computepolicies.ComputePoliciesClient", "ListByAccount", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForListByAccount(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "computepolicies.ComputePoliciesClient", "ListByAccount", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c ComputePoliciesClient) ListByAccountComplete(ctx context.Context, id AccountId) (ListByAccountCompleteResult, error) {
	return c.ListByAccountCompleteMatchingPredicate(ctx, id, ComputePolicyPredicate{})
}
...
func (c ComputePoliciesClient) preparerForListByAccount(ctx context.Context, id AccountId) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/computePolicies", id.ID())),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// preparerForListByAccountWithNextLink prepares the ListByAccount request with the given nextLink token.
func (c ComputePoliciesClient) preparerForListByAccountWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c ComputePoliciesClient) responderForListByAccount(resp *http.Response) (result ListByAccountResponse, err error) {
	type page struct {
		Values   []ComputePolicy `json:"value"`
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListByAccountResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "computepolicies.ComputePoliciesClient", "ListByAccount", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForListByAccount(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "computepolicies.ComputePoliciesClient", "ListByAccount", result.HttpResponse, "Failure responding to request")
				return
			}
