...
func (c ComputePoliciesClient) Update(ctx context.Context, id ComputePoliciesId, input UpdateComputePolicyParameters) (result UpdateResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "computepolicies.ComputePoliciesClient", "Update", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForUpdate(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "computepolicies.ComputePoliciesClient", "Update", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c ComputePoliciesClient) responderForUpdate(resp *http.Response) (result UpdateResponse, err error) {
