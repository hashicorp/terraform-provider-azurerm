...
func (c TrustedIdProvidersClient) ListByAccount(ctx context.Context, id AccountId) (resp ListByAccountResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "trustedidproviders.TrustedIdProvidersClient", "ListByAccount", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "trustedidproviders.TrustedIdProvidersClient", "ListByAccount", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForListByAccount(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "trustedidproviders.TrustedIdProvidersClient", "ListByAccount", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c TrustedIdProvidersClient) ListByAccountComplete(ctx context.Context, id AccountId) (ListByAccountCompleteResult, error) {
	return c.ListByAccountCompleteMatchingPredicate(ctx, id, TrustedIdProviderPredicate{})
}
...
func (c TrustedIdProvidersClient) preparerForListByAccount(ctx context.Context, id AccountId) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/trustedIdProviders", id.ID())),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// preparerForListByAccountWithNextLink prepares the ListByAccount request with the given nextLink token.
func (c TrustedIdProvidersClient) preparerForListByAccountWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c TrustedIdProvidersClient) responderForListByAccount(resp *http.Response) (result ListByAccountResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListByAccountResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "trustedidproviders.TrustedIdProvidersClient", "ListByAccount", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForListByAccount(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "trustedidproviders.TrustedIdProvidersClient", "ListByAccount", result.HttpResponse, "Failure responding to request")
				return
			}
