	...
	data.DataSourceTest(t, []acceptance.TestStep{
		{
			...
			Check: acceptance.ComposeTestCheckFunc(
				...
				check.That(data.ResourceName).Key("virtual_machine_scale_set_fabric.0.sku_name").HasValue("Standard_B1s"),
				check.That(data.ResourceName).Key("virtual_machine_scale_set_fabric.0.image.0.well_known_image_name").HasValue("ubuntu-24.04/latest"),
			),
		},
	})
}
