...
func expandStatefulAgentProfileModel(input []StatefulAgentProfileModel) (pools.AgentProfile, error) {
	...
	if len(agentProfile.ManualResourcePredictionsProfile) > 0 {
		...
		resourcePredictions, err := expandResourcePredictionsModel(resourcePredictionsProfile.ResourcePredictions)
		if err != nil {
			return nil, err
		}
...
func expandResourcePredictionsModel(input []ResourcePredictionsModel) (*ResourcePredictionsSdkModel, error) {
	if len(input) == 0 {
		return nil, nil
	}
...
func flattenStatefulAgentProfileToModel(input pools.Stateful) ([]StatefulAgentProfileModel, error) {
	...
	if input.ResourcePredictionsProfile != nil {
		if automatic, ok := input.ResourcePredictionsProfile.(pools.AutomaticResourcePredictionsProfile); ok {
			...
			if input.ResourcePredictions != nil {
				predModel, err := flattenResourcePredictionsModel(pointer.From(input.ResourcePredictions))
				if err != nil {
					return nil, err
				}
	...
	return []StatefulAgentProfileModel{statefulAgentProfileModel}, nil
}

func flattenStatelessAgentProfileToModel(input pools.StatelessAgentProfile) ([]StatelessAgentProfileModel, error) {
	...
	return []StatelessAgentProfileModel{statelessAgentProfileModel}, nil
}

func flattenResourcePredictionsModel(input interface{}) (*ResourcePredictionsModel, error) {
	if input == nil {
		return nil, nil
	}
