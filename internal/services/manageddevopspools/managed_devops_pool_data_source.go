				...
				if props := model.Properties; props != nil {
					...
					if agentProfile := props.AgentProfile; agentProfile != nil {
						if stateful, ok := agentProfile.(pools.Stateful); ok {
							state.StatefulAgentProfile, err = flattenStatefulAgentProfileToModel(stateful)
							if err != nil {
								return fmt.Errorf("flattening stateful agent profile %s: %+v", id, err)
							}
						} else if stateless, ok := agentProfile.(pools.StatelessAgentProfile); ok {
							state.StatelessAgentProfile, err = flattenStatelessAgentProfileToModel(stateless)
							if err != nil {
								return fmt.Errorf("flattening stateless agent profile %s: %+v", id, err)
							}
						}
					}
