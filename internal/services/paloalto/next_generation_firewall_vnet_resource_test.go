...
func TestAccPaloAltoNextGenerationFirewallVNet_basic(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_palo_alto_next_generation_firewall_vnet", "test")
...
func TestAccPaloAltoNextGenerationFirewallVNet_completeRulestack(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_palo_alto_next_generation_firewall_vnet", "test")
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.completeWithRulestack(data),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
...
// TODO - completePanorama
func (r NextGenerationFirewallVnetResource) completeWithRulestack(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
