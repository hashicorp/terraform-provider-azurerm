...
func resourceKustoEventGridDataConnectionCreateUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if df, ok := d.GetOk("data_format"); ok {
		dataConnection.Properties.DataFormat = pointer.To(dataconnections.EventGridDataFormat(df.(string)))
	}

	if databaseRouting, ok := d.GetOk("database_routing_type"); ok {
		dataConnection.Properties.DatabaseRouting = pointer.To(dataconnections.DatabaseRouting(databaseRouting.(string)))
	}
