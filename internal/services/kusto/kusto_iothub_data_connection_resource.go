...
func expandKustoIotHubDataConnectionProperties(d *pluginsdk.ResourceData) *dataconnections.IotHubConnectionProperties {
	...
	if df, ok := d.GetOk("data_format"); ok {
		iotHubDataConnectionProperties.DataFormat = pointer.To(dataconnections.IotHubDataFormat(df.(string)))
	}

	if databaseRouting, ok := d.GetOk("database_routing_type"); ok {
		iotHubDataConnectionProperties.DatabaseRouting = pointer.To(dataconnections.DatabaseRouting(databaseRouting.(string)))
	}
