...
func TestAccKubernetesClusterExtension_hybrid(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_kubernetes_cluster_extension", "test")
	r := KubernetesClusterExtensionResource{}
	data.ResourceTest(t, r, []acceptance.TestStep{
		{
...
`, os.Getenv("ARM_TEST_ARC_K8S_RG"), data.RandomInteger, os.Getenv("ARM_TEST_ARC_K8S_CLUSTER"))
}

func (r KubernetesClusterExtensionResource) hybrid(data acceptance.TestData) string {
	// follow https://learn.microsoft.com/en-us/azure/aks/hybrid/aks-hybrid-preview-azure-vm to create a hybrid Kubernetes cluster
	rgName := "ademo-rg"
	clusterName := "hybridcluster123"
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
...
resource "azurerm_kubernetes_cluster_extension" "test" {
  ...
  resource_group_name   = data.azurerm_resource_group.test.name
  cluster_name          = "%s"
  cluster_resource_name = "provisionedClusters"
  extension_type        = "microsoft.flux"
}
`, rgName, data.RandomInteger, clusterName)
}
