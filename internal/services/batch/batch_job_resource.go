	...
	return sdk.ResourceFunc{
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			params := batchDataplane.JobAddParameter{
				...
				Priority:    pointer.To(int32(int32(model.Priority))),
				Constraints: &batchDataplane.JobConstraints{
					MaxTaskRetryCount: pointer.To(int32(int32(model.TaskRetryMaximum))),
				},
	...
	return sdk.ResourceFunc{
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			if metadata.ResourceData.HasChange("priority") {
				patch.Priority = pointer.To(int32(int32(model.Priority)))
			}
			...
			if metadata.ResourceData.HasChange("task_retry_maximum") {
				...
				patch.Constraints.MaxTaskRetryCount = pointer.To(int32(int32(model.TaskRetryMaximum)))
			}
...
func (r BatchJobResource) addJob(ctx context.Context, client *batchDataplane.JobClient, id parse.JobId, job batchDataplane.JobAddParameter) error {
	...
	_, err := client.Add(ctx, job, pointer.To(int32(int32(timeout.Seconds()))), nil, nil, &date.TimeRFC1123{Time: now})
	if err != nil {
...
func (r BatchJobResource) getJob(ctx context.Context, client *batchDataplane.JobClient, id parse.JobId) (batchDataplane.CloudJob, error) {
	...
	now := time.Now()
	timeout := deadline.Sub(now)
	return client.Get(ctx, id.Name, "", "", pointer.To(int32(int32(timeout.Seconds()))), nil, nil, &date.TimeRFC1123{Time: now}, "", "", nil, nil)
}
...
func (r BatchJobResource) patchJob(ctx context.Context, client *batchDataplane.JobClient, id parse.JobId, job batchDataplane.JobPatchParameter) error {
	...
	now := time.Now()
	timeout := deadline.Sub(now)
	_, err := client.Patch(ctx, id.Name, job, pointer.To(int32(int32(timeout.Seconds()))), nil, nil, &date.TimeRFC1123{Time: now}, "", "", nil, nil)
	return err
}
...
func (r BatchJobResource) deleteJob(ctx context.Context, client *batchDataplane.JobClient, id parse.JobId) error {
	...
	now := time.Now()
	timeout := deadline.Sub(now)
	_, err := client.Delete(ctx, id.Name, pointer.To(int32(int32(timeout.Seconds()))), nil, nil, &date.TimeRFC1123{Time: now}, "", "", nil, nil)
	return err
}
