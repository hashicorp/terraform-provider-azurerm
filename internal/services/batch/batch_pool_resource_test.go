	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			...
			Check: acceptance.ComposeTestCheckFunc(
				...
				check.That(data.ResourceName).Key("start_task.0.container.0.host_batch_bind_mounts.0.source").HasValue("Startup"),
				check.That(data.ResourceName).Key("start_task.0.container.0.host_batch_bind_mounts.0.read_only_enabled").HasValue("false"),
			),
		},
		data.ImportStep("stop_pending_resize_operation",
