...
func resourceIotCentralADGroupUser() *pluginsdk.Resource {
	return &pluginsdk.Resource{
		Create: resourceIotCentralADGroupUserCreate,
		Read:   resourceIotCentralADGroupUserRead,
		Update: resourceIotCentralADGroupUserUpdate,
		Delete: resourceIotCentralADGroupUserDelete,
...
func resourceIotCentralADGroupUserCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	model := iotcentralDataplane.ADGroupUser{
		ObjectID: &objectId,
		TenantID: &tenantId,
	...
	user, isADGroupUser := resp.Value.AsADGroupUser()
	if !isADGroupUser {
		return fmt.Errorf("expected %s to be an AD group user", id)
	}
	...
	d.SetId(userId.ID())
	return resourceIotCentralADGroupUserRead(d, meta)
}

func resourceIotCentralADGroupUserRead(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).IoTCentral
...
func resourceIotCentralADGroupUserUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).IoTCentral
	ctx, cancel := timeouts.ForCreate(meta.(*clients.Client).StopContext, d)
	defer cancel()
	...
	existing, isADGroupUser := resp.Value.AsADGroupUser()
	if !isADGroupUser {
		return fmt.Errorf("expected %s to be an AD group user", id)
	}
...
func resourceIotCentralADGroupUserDelete(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).IoTCentral
