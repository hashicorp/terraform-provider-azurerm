...
func TestAccIoTCentralGroupUser_basic(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_iotcentral_group_user", "test")
	r := IotCentralGroupUserResource{}
...
func TestAccIoTCentralGroupUser_complete(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_iotcentral_group_user", "test")
	r := IotCentralGroupUserResource{}
...
func TestAccIoTCentralGroupUser_basicUpdateRole(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_iotcentral_group_user", "test")
	r := IotCentralGroupUserResource{}
...
func (IotCentralGroupUserResource) Exists(ctx context.Context, clients *clients.Client, state *pluginsdk.InstanceState) (*bool, error) {
	...
	_, isValid := iotcentral.TryValidateGroupUserExistence(resp.Value)

	return &isValid, nil
}

func (r IotCentralGroupUserResource) basic(data acceptance.TestData, roleId string) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
...
resource "azurerm_iotcentral_group_user" "test" {
...
func (r IotCentralGroupUserResource) complete(data acceptance.TestData, roleId string) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
...
func (IotCentralGroupUserResource) templateBasic(data acceptance.TestData) string {
	return fmt.Sprintf(`
...
func (IotCentralGroupUserResource) templateComplete(data acceptance.TestData) string {
	return fmt.Sprintf(`
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-%[1]d"
  location = "%[2]s"
