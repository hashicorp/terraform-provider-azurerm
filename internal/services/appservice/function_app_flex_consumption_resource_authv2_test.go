...
func TestAccFunctionAppFlexConsumption_FivePointOhAuthV2Update(t *testing.T) {
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		...
		{
			Config: r.authV2FacebookFivePointOh(data),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
				check.That(data.ResourceName).Key("kind").HasValue("functionapp,linux"),
				check.That(data.ResourceName).Key("auth_settings_v2.0.auth_enabled").HasValue("true"),
		...
		data.ImportStep("site_credential.0.password"),
		{
			Config: r.authV2RemovedFivePointOh(data),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
				check.That(data.ResourceName).Key("kind").HasValue("functionapp,linux"),
...
func (r FunctionAppFlexConsumptionResource) authV2AppleFivePointOh(data acceptance.TestData) string {
...
func (r FunctionAppFlexConsumptionResource) authV2FacebookFivePointOh(data acceptance.TestData) string {
...
func (r FunctionAppFlexConsumptionResource) authV2RemovedFivePointOh(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
...
func TestAccFunctionAppFlexConsumption_authV2Update(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_function_app_flex_consumption", "test")
	r := FunctionAppFlexConsumptionResource{}
