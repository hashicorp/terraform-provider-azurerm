...
var _ resourceids.Id = AppHybridConnectionId{}

func TestAppHybridConnectionIDFormatter(t *testing.T) {
	actual := NewAppHybridConnectionID("12345678-1234-9876-4563-123456789012", "resGroup1", "site1", "hybridConnectionNamespace1", "relay1").ID()
...
func TestAppHybridConnectionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *AppHybridConnectionId
	}{
	...
	for _, v := range testData {
		...
		actual, err := AppHybridConnectionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
