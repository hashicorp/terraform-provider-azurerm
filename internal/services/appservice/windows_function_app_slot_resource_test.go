...
func TestAccWindowsFunctionAppSlot_withPushStandardPlan(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_windows_function_app_slot", "test")
	r := WindowsFunctionAppSlotResource{}
...
func (r WindowsFunctionAppSlotResource) push(data acceptance.TestData, planSku string) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
...
`, r.storageContainerTemplate(data, planSku), data.RandomInteger, data.RandomInteger, data.RandomInteger)
}
