...
func TestAccFunctionAppFlexConsumption_vNetRoutingUpdate(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_function_app_flex_consumption", "test")
	r := FunctionAppFlexConsumptionResource{}
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		...
		data.ImportStep("site_credential.0.password"),
		{
			Config: r.vNetRoutingEnabled(data),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
...
func TestAccFunctionAppFlexConsumption_siteStrategyUpdate(t *testing.T) {
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		...
		data.ImportStep("site_credential.0.password"),
		{
			Config: r.siteUpdateStrategyRollingUpdate(data),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
...
func (r FunctionAppFlexConsumptionResource) vNetRoutingEnabled(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
...
func (r FunctionAppFlexConsumptionResource) siteUpdateStrategyRollingUpdate(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
...
resource "azurerm_function_app_flex_consumption" "test" {
  ...
  site_update_strategy = "RollingUpdate"
