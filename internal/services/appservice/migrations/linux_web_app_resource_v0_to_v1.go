											...
											"status_code": {
												...
												Elem: &pluginsdk.Resource{
													Schema: map[string]*pluginsdk.Schema{
														...
														"win32_status": { // TODO - this is going away!
															Type:     pluginsdk.TypeInt,
															Optional: true,
														},
...
func (l LinuxWebAppV0ToV1) UpgradeFunc() pluginsdk.StateUpgraderFunc {
	return func(ctx context.Context, rawState map[string]interface{}, meta interface{}) (map[string]interface{}, error) {
		if sc, ok := rawState["site_config"].([]interface{}); ok {
			if len(sc) > 0 {
				if siteConfig, ok := sc[0].(map[string]interface{}); ok {
					if ah, ok := siteConfig["auto_heal_setting"].([]interface{}); ok {
						if len(ah) > 0 {
							if autoHeal, ok := ah[0].(map[string]interface{}); ok {
								delete(autoHeal, "win32_status")
								ah[0] = autoHeal
							}
							siteConfig["auto_heal_setting"] = ah
						}
					}
					sc[0] = siteConfig
				}
			}
			rawState["site_config"] = sc
		}
