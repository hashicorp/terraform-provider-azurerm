	...
	data.ResourceTest(t, r, []resource.TestStep{
		{
			...
			Check: resource.ComposeTestCheckFunc(
				...
				check.That(data.ResourceName).Key("identity.0.identity_ids").IsEmpty(),
	...
	data.ResourceTest(t, r, []resource.TestStep{
		{
			...
			Check: resource.ComposeTestCheckFunc(
				...
				check.That(data.ResourceName).Key("identity.0.identity_ids").IsSet(),
...
func TestAccEventGridDomain_basicWithSystemAndUserAssignedManagedIdentity(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_eventgrid_domain", "test")
	r := EventGridDomainResource{}

	data.ResourceTest(t, r, []resource.TestStep{
		{
			Config: r.basicWithSystemAndUserAssignedManagedIdentity(data),
			Check: resource.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
				check.That(data.ResourceName).Key("identity.#").HasValue("1"),
...
func (EventGridDomainResource) basicWithSystemAndUserAssignedManagedIdentity(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
