	...
	data.DataSourceTest(t, []acceptance.TestStep{
		{
			...
			Check: acceptance.ComposeTestCheckFunc(
				...
				check.That(data.ResourceName).Key("partner_topic.0.inline_event_type.#").HasValue("1"),
				check.That(data.ResourceName).Key("partner_topic.0.inline_event_type.0.name").HasValue("SampleEventType"),
				check.That(data.ResourceName).Key("partner_topic.0.inline_event_type.0.display_name").HasValue("Sample Event Type"),
				check.That(data.ResourceName).Key("partner_topic.0.inline_event_type.0.data_schema_url").HasValue("https://example.com/sample-event-type-schema"),
				check.That(data.ResourceName).Key("partner_topic.0.inline_event_type.0.description").HasValue("This is a sample event type"),
				check.That(data.ResourceName).Key("partner_topic.0.inline_event_type.0.documentation_url").HasValue("https://example.com/sample-event-type-docs"),
				check.That(data.ResourceName).Key("readiness_state").Exists(),
			),
		},
	})
