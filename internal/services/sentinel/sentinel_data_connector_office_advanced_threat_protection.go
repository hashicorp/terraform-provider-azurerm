...
func resourceSentinelDataConnectorOfficeATPCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if d.IsNewResource() {
		...
		if !utils.ResponseWasNotFound(resp.Response) {
			return tf.ImportAsExistsError("azurerm_sentinel_data_connector_office_advanced_threat_protection", id.ID())
		}
	}
...
func resourceSentinelDataConnectorOfficeATPRead(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if !ok {
		return fmt.Errorf("%s was not an Office Advanced Threat Protection Data Connector", id)
	}
