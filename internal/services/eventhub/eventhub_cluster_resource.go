...
func resourceEventHubClusterDelete(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	return pluginsdk.Retry(d.Timeout(pluginsdk.TimeoutDelete), func() *pluginsdk.RetryError {
		...
		if err != nil {
			if strings.Contains(err.Error(), "cannot be deleted until at least 4 hours after its created time") || response.WasStatusCode(future.HttpResponse, http.StatusTooManyRequests) {
