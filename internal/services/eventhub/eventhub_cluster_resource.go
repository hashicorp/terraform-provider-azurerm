...
func resourceEventHubClusterDelete(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	return pluginsdk.Retry(d.Timeout(pluginsdk.TimeoutDelete), func() *pluginsdk.RetryError {
		...
		if err != nil {
			if strings.Contains(err.Error(), "at least 4 hours after its created time") || response.WasBadRequest(future.HttpResponse) || response.WasStatusCode(future.HttpResponse, http.StatusTooManyRequests) {
