...
func resourceFrontdoorProfileEndpoint() *pluginsdk.Resource {
	return &pluginsdk.Resource{
		Create: resourceFrontdoorProfileEndpointCreate,
		Read:   resourceFrontdoorProfileEndpointRead,
		Update: resourceFrontdoorProfileEndpointUpdate,
		Delete: resourceFrontdoorProfileEndpointDelete,
		...
		Importer: pluginsdk.ImporterValidatingResourceId(func(id string) error {
			_, err := parse.FrontdoorProfileEndpointID(id)
			return err
		}),
...
func resourceFrontdoorProfileEndpointCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Cdn.FrontDoorProfileEndpointsClient
	...
	id := parse.NewFrontdoorProfileEndpointID(profileId.SubscriptionId, profileId.ResourceGroupName, profileId.ProfileName, d.Get("name").(string))
	...
	if d.IsNewResource() {
		...
		if !response.WasNotFound(existing.HttpResponse) {
			return tf.ImportAsExistsError("azurerm_frontdoor_profile_endpoint", id.ID())
		}
	}
	...
	d.SetId(id.ID())
	return resourceFrontdoorProfileEndpointRead(d, meta)
}

func resourceFrontdoorProfileEndpointRead(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Cdn.FrontDoorProfileEndpointsClient
	...
	id, err := parse.FrontdoorProfileEndpointID(d.Id())
	if err != nil {
		return err
	}
...
func resourceFrontdoorProfileEndpointUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Cdn.FrontDoorProfileEndpointsClient
...
func resourceFrontdoorProfileEndpointDelete(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Cdn.FrontDoorProfileEndpointsClient
