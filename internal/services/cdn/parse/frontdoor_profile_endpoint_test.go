...
var _ resourceids.Id = FrontdoorProfileEndpointId{}

func TestFrontdoorProfileEndpointIDFormatter(t *testing.T) {
	actual := NewFrontdoorProfileEndpointID("12345678-1234-9876-4563-123456789012", "resourceGroup1", "profile1", "endpoint1").ID()
...
func TestFrontdoorProfileEndpointID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *FrontdoorProfileEndpointId
	}{
	...
	for _, v := range testData {
		...
		actual, err := FrontdoorProfileEndpointID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
