...
func TestFrontDoorProfileIDFormatter(t *testing.T) {
	actual := NewFrontDoorProfileID("12345678-1234-9876-4563-123456789012", "resGroup1", "profile1").ID()
...
func TestFrontDoorProfileIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *FrontDoorProfileId
	...
	for _, v := range testData {
		...
		actual, err := FrontDoorProfileIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
