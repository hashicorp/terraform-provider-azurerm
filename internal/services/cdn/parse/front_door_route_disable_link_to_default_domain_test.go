...
func TestFrontDoorRouteDisableLinkToDefaultDomainIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *FrontDoorRouteDisableLinkToDefaultDomainId
	...
	for _, v := range testData {
		...
		actual, err := FrontDoorRouteDisableLinkToDefaultDomainIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
