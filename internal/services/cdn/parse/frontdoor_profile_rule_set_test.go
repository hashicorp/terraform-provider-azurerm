...
var _ resourceids.Id = FrontdoorProfileRuleSetId{}

func TestFrontdoorProfileRuleSetIDFormatter(t *testing.T) {
	actual := NewFrontdoorProfileRuleSetID("12345678-1234-9876-4563-123456789012", "resourceGroup1", "profile1", "ruleSet1").ID()
...
func TestFrontdoorProfileRuleSetID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *FrontdoorProfileRuleSetId
	}{
	...
	for _, v := range testData {
		...
		actual, err := FrontdoorProfileRuleSetID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
