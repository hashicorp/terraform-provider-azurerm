...
var _ resourceids.Id = FrontdoorProfileOriginGroupId{}

func TestFrontdoorProfileOriginGroupIDFormatter(t *testing.T) {
	actual := NewFrontdoorProfileOriginGroupID("12345678-1234-9876-4563-123456789012", "resourceGroup1", "profile1", "originGroup1").ID()
...
func TestFrontdoorProfileOriginGroupID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *FrontdoorProfileOriginGroupId
	}{
	...
	for _, v := range testData {
		...
		actual, err := FrontdoorProfileOriginGroupID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
