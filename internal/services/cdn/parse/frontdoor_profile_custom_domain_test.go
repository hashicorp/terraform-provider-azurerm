...
var _ resourceids.Id = FrontdoorProfileCustomDomainId{}

func TestFrontdoorProfileCustomDomainIDFormatter(t *testing.T) {
	actual := NewFrontdoorProfileCustomDomainID("12345678-1234-9876-4563-123456789012", "resourceGroup1", "profile1", "customDomain1").ID()
...
func TestFrontdoorProfileCustomDomainID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *FrontdoorProfileCustomDomainId
	}{
	...
	for _, v := range testData {
		...
		actual, err := FrontdoorProfileCustomDomainID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
