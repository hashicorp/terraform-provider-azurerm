...
var _ resourceids.Id = FrontDoorCustomDomainTxtId{}

func TestFrontDoorCustomDomainTxtIDFormatter(t *testing.T) {
	actual := NewFrontDoorCustomDomainTxtID("12345678-1234-9876-4563-123456789012", "resGroup1", "profile1", "customDomain1", "txt1").ID()
	expected := "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/resGroup1/providers/Microsoft.Cdn/profiles/profile1/customDomains/customDomain1/dnsTxt/txt1"
	if actual != expected {
...
func TestFrontDoorCustomDomainTxtID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *FrontDoorCustomDomainTxtId
	}{
	...
	for _, v := range testData {
		...
		actual, err := FrontDoorCustomDomainTxtID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestFrontDoorCustomDomainTxtIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *FrontDoorCustomDomainTxtId
	...
	for _, v := range testData {
		...
		actual, err := FrontDoorCustomDomainTxtIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
