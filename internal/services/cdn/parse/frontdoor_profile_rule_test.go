...
var _ resourceids.Id = FrontdoorProfileRuleId{}

func TestFrontdoorProfileRuleIDFormatter(t *testing.T) {
	actual := NewFrontdoorProfileRuleID("12345678-1234-9876-4563-123456789012", "resourceGroup1", "profile1", "ruleSet1", "rule1").ID()
...
func TestFrontdoorProfileRuleID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *FrontdoorProfileRuleId
	}{
	...
	for _, v := range testData {
		...
		actual, err := FrontdoorProfileRuleID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
