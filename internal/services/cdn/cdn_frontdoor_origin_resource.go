...
func resourceCdnFrontDoorOriginCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	props := &cdn.AFDOriginProperties{
		...
		HTTPPort:                    pointer.To(int32(int32(d.Get("http_port").(int)))),
		HTTPSPort:                   pointer.To(int32(int32(d.Get("https_port").(int)))),
		Priority:                    pointer.To(int32(int32(d.Get("priority").(int)))),
		Weight:                      pointer.To(int32(int32(d.Get("weight").(int)))),
	}
...
func resourceCdnFrontDoorOriginUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if d.HasChange("http_port") {
		params.HTTPPort = pointer.To(int32(int32(d.Get("http_port").(int))))
	}

	if d.HasChange("https_port") {
		params.HTTPSPort = pointer.To(int32(int32(d.Get("https_port").(int))))
	}
	...
	if d.HasChange("priority") {
		params.Priority = pointer.To(int32(int32(d.Get("priority").(int))))
	}

	if d.HasChange("weight") {
		params.Weight = pointer.To(int32(int32(d.Get("weight").(int))))
	}
