...
func CdnFrontDoorValidateConditionMatchValues(configName, operator string, matchValues []string) error {
	if operator == "" {
		return fmt.Errorf("%q is invalid: no 'operator' value has been set, got %q", configName, operator)
	}
	...
	if operator == "Any" && len(matchValues) > 0 {
		return fmt.Errorf("%q is invalid: the 'match_values' field must not be set if the conditions 'operator' is set to 'Any'", configName)
	}

	if operator != "Any" && len(matchValues) == 0 {
		return fmt.Errorf("%q is invalid: the 'match_values' field must be set if the conditions 'operator' is not set to 'Any'", configName)
	}
...
func CdnFrontDoorValidateGeoMatchCountryCodes(configName string, matchValues []string) error {
	for _, matchValue := range matchValues {
		if ok, _ := azureValidate.RegExHelper(matchValue, "match_values", `^[A-Z]{2}$`); !ok {
			return fmt.Errorf("%q is invalid: when the 'operator' is set to 'GeoMatch' the value must be a valid country code consisting of 2 uppercase characters, got %q", configName, matchValue)
		}
	}
...
func CdnFrontDoorValidateCIDRMatchValues(configName string, matchValues []string) error {
	...
	for _, matchValue := range matchValues {
		...
		if _, err := FrontDoorRuleCidrIsValid(matchValue, "match_values"); err != nil {
			return fmt.Errorf("%q is invalid: when the 'operator' is set to 'IPMatch' the 'match_values' must be a valid IPv4 or IPv6 CIDR, got %q", configName, matchValue)
		}
	}

	if _, err := FrontDoorRuleCidrOverlap(rawMatchValues, "match_values"); err != nil {
		return fmt.Errorf("%q is invalid: %+v", configName, err)
	}
...
func CdnFrontDoorValidateRouteConfigurationOverrideAction(input CdnFrontDoorRouteConfigurationOverrideInput) error {
	...
	if input.CacheBehavior == string(cdnrules.RuleIsCompressionEnabledDisabled) {
		if input.QueryStringCachingBehavior != "" {
			return fmt.Errorf("the 'route_configuration_override_action' block is not valid, if the 'cache_behavior' is set to 'Disabled' you cannot define the 'query_string_caching_behavior', got %q", input.QueryStringCachingBehavior)
		}
		...
		if input.CacheDuration != "" {
			return fmt.Errorf("the 'route_configuration_override_action' block is not valid, if the 'cache_behavior' is set to 'Disabled' you cannot define the 'cache_duration', got %q", input.CacheDuration)
		}
