...
func resourceCdnOriginGroup() *pluginsdk.Resource {
	return &pluginsdk.Resource{
		Create: resourceCdnOriginGroupCreate,
		Read:   resourceCdnOriginGroupRead,
		Update: resourceCdnOriginGroupUpdate,
		Delete: resourceCdnOriginGroupDelete,
...
func resourceCdnOriginGroupCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if !utils.ResponseWasNotFound(existing.Response) {
		return tf.ImportAsExistsError("azurerm_cdn_origin_group", id.ID())
	}

	props := cdn.OriginGroup{
		OriginGroupProperties: &cdn.OriginGroupProperties{
			HealthProbeSettings: expandCdnOriginGroupHealthProbeParameters(d.Get("health_probe").([]interface{})),
			Origins:             expandCdnOrigins(d.Get("origins").([]interface{})),
	...
	d.SetId(id.ID())
	return resourceCdnOriginGroupRead(d, meta)
}

func resourceCdnOriginGroupRead(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Cdn.CdnOriginGroupsClient
	...
	if props := resp.AFDOriginGroupProperties; props != nil {
		...
		if err := d.Set("load_balancing", flattenCdnOriginGroupLoadBalancingSettingsParameters(props.LoadBalancingSettings)); err != nil {
...
func resourceCdnOriginGroupUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Cdn.CdnOriginGroupsClient
	workaroundClient := azuresdkhacks.NewCdnOriginGroupsWorkaroundClient(client)
	...
	if d.HasChange("health_probe") {
		params.HealthProbeSettings = expandCdnOriginGroupHealthProbeParameters(d.Get("health_probe").([]interface{}))
	}

	if d.HasChange("load_balancing") {
		params.LoadBalancingSettings = expandCdnOriginGroupLoadBalancingSettingsParameters(d.Get("load_balancing").([]interface{}))
	}
...
func resourceCdnOriginGroupDelete(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Cdn.CdnOriginGroupsClient
...
func expandCdnOriginGroupHealthProbeParameters(input []interface{}) *cdn.HealthProbeParameters {
...
func expandCdnOriginGroupLoadBalancingSettingsParameters(input []interface{}) *cdn.LoadBalancingSettingsParameters {
	if len(input) == 0 || input[0] == nil {
		return nil
	}
...
func flattenCdnOriginGroupLoadBalancingSettingsParameters(input *cdn.LoadBalancingSettingsParameters) []interface{} {
	if input == nil {
		return []interface{}{}
	}
...
func flattenCdnOriginGroupHealthProbeParameters(input *cdn.HealthProbeParameters) []interface{} {
	if input == nil {
		return []interface{}{}
	}
