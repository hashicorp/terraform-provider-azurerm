...
func resourceFrontdoorProfileSecret() *pluginsdk.Resource {
	return &pluginsdk.Resource{
		Create: resourceFrontdoorProfileSecretCreate,
		Read:   resourceFrontdoorProfileSecretRead,
		Delete: resourceFrontdoorProfileSecretDelete,
...
func resourceFrontdoorProfileSecretCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Cdn.FrontdoorProfileSecretsClient
	...
	if d.IsNewResource() {
		...
		if !response.WasNotFound(existing.HttpResponse) {
			return tf.ImportAsExistsError("azurerm_cdn_secret", id.ID())
		}
	}
	...
	d.SetId(id.ID())
	return resourceFrontdoorProfileSecretRead(d, meta)
}

func resourceFrontdoorProfileSecretRead(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Cdn.FrontdoorProfileSecretsClient
...
func resourceFrontdoorProfileSecretDelete(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Cdn.FrontdoorProfileSecretsClient
