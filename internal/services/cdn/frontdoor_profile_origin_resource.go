...
func resourceFrontdoorProfileOrigin() *pluginsdk.Resource {
	return &pluginsdk.Resource{
		Create: resourceFrontdoorProfileOriginCreate,
		Read:   resourceFrontdoorProfileOriginRead,
		Update: resourceFrontdoorProfileOriginUpdate,
		Delete: resourceFrontdoorProfileOriginDelete,
		...
		Importer: pluginsdk.ImporterValidatingResourceId(func(id string) error {
			_, err := parse.FrontdoorProfileOriginID(id)
			return err
		}),
...
func resourceFrontdoorProfileOriginCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Cdn.FrontDoorProfileOriginsClient
	ctx, cancel := timeouts.ForCreate(meta.(*clients.Client).StopContext, d)
	defer cancel()

	originGroupId, err := parse.FrontdoorProfileOriginGroupID(d.Get("cdn_origin_group_id").(string))
	if err != nil {
		return err
	}
	...
	id := parse.NewFrontdoorProfileOriginID(originGroupId.SubscriptionId, originGroupId.ResourceGroup, originGroupId.ProfileName, originGroupId.OriginGroupName, d.Get("name").(string))
	...
	if d.IsNewResource() {
		...
		if !response.WasNotFound(existing.HttpResponse) {
			return tf.ImportAsExistsError("azurerm_frontdoor_profile_origin", id.ID())
		}
	}
	...
	d.SetId(id.ID())
	return resourceFrontdoorProfileOriginRead(d, meta)
}

func resourceFrontdoorProfileOriginRead(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Cdn.FrontDoorProfileOriginsClient
	...
	id, err := parse.FrontdoorProfileOriginID(d.Id())
	if err != nil {
		return err
	}
	...
	d.Set("cdn_origin_group_id", afdorigingroups.NewOriginGroupID(id.SubscriptionId, id.ResourceGroup, id.ProfileName, id.OriginGroupName).ID())
...
func resourceFrontdoorProfileOriginUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Cdn.FrontDoorProfileOriginsClient
...
func resourceFrontdoorProfileOriginDelete(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Cdn.FrontDoorProfileOriginsClient
