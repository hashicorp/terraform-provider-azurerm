...
type AfdOriginResource struct{}

func TestAccCdnAfdOrigin_basic(t *testing.T) {
...
func TestAccCdnAfdOrigin_disappears(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_cdn_frontdoor_origin", "test")
	r := AfdOriginResource{}
...
func (r AfdOriginResource) Exists(ctx context.Context, client *clients.Client, state *pluginsdk.InstanceState) (*bool, error) {
	id, err := parse.AfdOriginsID(state.ID)
	if err != nil {
		return nil, err
	}
	resp, err := client.Cdn.AFDOriginsClient.Get(ctx, id.ResourceGroup, id.ProfileName, id.OriginGroupName, id.OriginName)
	if err != nil {
...
func (r AfdOriginResource) Destroy(ctx context.Context, client *clients.Client, state *pluginsdk.InstanceState) (*bool, error) {
	id, err := parse.AfdOriginsID(state.ID)
	if err != nil {
		return nil, err
	}

	originsClient := client.Cdn.AFDOriginsClient
	future, err := originsClient.Delete(ctx, id.ResourceGroup, id.ProfileName, id.OriginGroupName, id.OriginName)
	if err != nil {
...
func (r AfdOriginResource) basic(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
