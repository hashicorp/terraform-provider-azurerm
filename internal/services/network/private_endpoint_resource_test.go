...
func TestAccPrivateEndpoint_multipleIpConfigurations(t *testing.T) {
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.recoveryServiceVaultWithStaticIpConfig(data),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
...
func (r PrivateEndpointResource) recoveryServiceVaultWithStaticIpConfig(data acceptance.TestData) string {
	return fmt.Sprintf(`
%s
