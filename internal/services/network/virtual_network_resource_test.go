	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			...
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
				check.That(data.ResourceName).Key("ip_address_pool.0.allocated_ip_address_prefixes").Exists(),
			),
		},
		data.ImportStep(),
	})
...
func TestAccVirtualNetwork_ipAddressPoolUpdate(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_virtual_network", "test")
	r := VirtualNetworkResource{}
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		...
		data.ImportStep(),
		{
			Config: r.ipAddressPoolIdUpdated(data),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
				check.That(data.ResourceName).Key("ip_address_pool.0.allocated_ip_address_prefixes").Exists(),
			),
...
func (VirtualNetworkResource) ipAddressPoolIdUpdated(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
