...
func (r RouteTableListResource) List(ctx context.Context, request list.ListRequest, stream *list.ListResultsStream, metadata sdk.ResourceMetadata) {
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, table := range results {
			...
			if err != nil {
				sdk.SetErrorDiagnosticAndYieldListResult(result, push, "parsing Route Table ID", err)
				return
			}
			...
			if err := resourceRouteTableFlatten(rd, id, &table); err != nil {
				sdk.SetErrorDiagnosticAndYieldListResult(result, push, fmt.Sprintf("encoding `%s` resource data", routeTableResourceName), err)
				return
			}
