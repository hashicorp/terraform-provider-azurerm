...
func ListenerID(input string) (*ListenerId, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("parsing %q as an Listener ID: %+v", input, err)
	}
...
// ListenerIDInsensitively parses an Listener ID into an ListenerId struct, insensitively
// This should only be used to parse an ID for rewriting, the ListenerID
// method should be used instead for validation etc.
//
// Whilst this may seem strange, this enables Terraform have consistent casing
// which works around issues in Core, whilst handling broken API responses.
func ListenerIDInsensitively(input string) (*ListenerId, error) {
	id, err := resourceids.ParseAzureResourceID(input)
	if err != nil {
		return nil, err
	}
	...
	// find the correct casing for the 'listeners' segment
	listenersKey := "listeners"
	for key := range id.Path {
		if strings.EqualFold(key, listenersKey) {
			listenersKey = key
			break
		}
	}
	if resourceId.Name, err = id.PopSegment(listenersKey); err != nil {
		return nil, err
	}
