...
func resourceSubnetCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	existing, err := client.Get(ctx, id, subnets.GetOperationOptions{})
	if err != nil {
	...
	if value, ok := d.GetOk("address_prefixes"); ok {
		...
		properties.AddressPrefixes = pointer.To(addressPrefixes)
	}
	if properties.AddressPrefixes != nil && len(*properties.AddressPrefixes) == 1 {
		properties.AddressPrefix = pointer.To((*properties.AddressPrefixes)[0])
	...
	if value, ok := d.GetOk("default_outbound_access_enabled"); ok {
		if !value.(bool) && len(delegationsRaw) > 0 {
			return fmt.Errorf("`default_outbound_access_enabled` cannot be disabled if delegations are present")
		}
		properties.DefaultOutboundAccess = pointer.To(value.(bool))
	}
	...
	err = client.CreateOrUpdateThenPoll(ctx, id, subnet)
	if err != nil {
...
func SubnetProvisioningStateRefreshFunc(ctx context.Context, client *subnets.SubnetsClient, id commonids.SubnetId) pluginsdk.StateRefreshFunc {
	return func() (interface{}, string, error) {
		res, err := client.Get(ctx, id, subnets.GetOperationOptions{})
		if err != nil {
