...
func resourceNetworkInterfaceNatRuleAssociationCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("updating NAT Rule Association for Network Interface %s: %+v", id, err)
	}

	if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
		return fmt.Errorf("waiting for completion of NAT Rule Association for NIC %s: %+v", id, err)
	}
