...
// todo remove in 4.0
func TestAccApplicationGateway_rewriteRuleSets_rewriteUrlThreePointOh(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_application_gateway", "test")
	r := ApplicationGatewayResource{}

	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.rewriteRuleSets_rewriteUrlThreePointOh(data),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
				check.That(data.ResourceName).Key("rewrite_rule_set.0.name").Exists(),
...
func TestAccApplicationGateway_rewriteRuleSets_rewriteUrl(t *testing.T) {
	if !features.FourPointOhBeta() {
		t.Skip("Skipping as this is test should only be run in version 4.0")
	}
	data := acceptance.BuildTestData(t, "azurerm_application_gateway", "test")
	r := ApplicationGatewayResource{}
...
func (r ApplicationGatewayResource) rewriteRuleSets_rewriteUrlThreePointOh(data acceptance.TestData) string {
	return fmt.Sprintf(`
%s
