...
func TestAccApplicationGateway_v1SkuNotSupported(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_application_gateway", "test")
	r := ApplicationGatewayResource{}

	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config:      r.v1SkuNotSupported(data),
			ExpectError: regexp.MustCompile(`the creation of new "[^"]+" V1 SKU is no longer supported, please use V2 SKU with \"Basic\", \"Standard_v2\" or \"WAF_v2\" tier instead`),
		},
	})
}
...
func (r ApplicationGatewayResource) v1SkuNotSupported(data acceptance.TestData) string {
	return fmt.Sprintf(`
%s
