...
type NetworkDDoSCustomPolicyResourceModel struct {
	...
	LoadBalancerFrontendIPConfigurationIds []string `tfschema:"load_balancer_frontend_ip_configuration_ids"`
...
func (r NetworkDDoSCustomPolicyResource) flatten(metadata sdk.ResourceMetaData, id *ddoscustompolicies.DdosCustomPolicyId, model *ddoscustompolicies.DdosCustomPolicy) error {
	...
	if props := model.Properties; props != nil {
		...
		state.LoadBalancerFrontendIPConfigurationIds = flattenNetworkDDoSCustomPolicySubResourceIds(props.FrontEndIPConfiguration)
		state.PublicIPAddressIds = flattenNetworkDDoSCustomPolicySubResourceIds(props.PublicIPAddresses)
	}
...
func flattenNetworkDDoSCustomPolicySubResourceIds(input *[]ddoscustompolicies.SubResource) []string {
	...
	for _, v := range *input {
		output = append(output, pointer.From(v.Id))
	}
