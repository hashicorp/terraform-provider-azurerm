...
func resourceVirtualNetworkCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	for _, subnet := range *vnet.Properties.Subnets {
		if subnet.Properties != nil && subnet.Properties.NetworkSecurityGroup != nil {
			...
			if !utils.SliceContainsValue(networkSecurityGroupIDs, parsedNsgID.ID()) {
	...
	if v, ok := d.GetOk("subnet"); ok {
		...
		for _, subnet := range subnets {
			...
			if networkSecurityGroupId != "" {
				...
				if !utils.SliceContainsValue(nsgIDs, parsedNsgID.ID()) {
			...
			if routeTableId != "" {
				...
				if !utils.SliceContainsValue(routeTableIDs, parsedRouteTableID.ID()) {
