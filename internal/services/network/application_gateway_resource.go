...
func applicationGatewayCustomizeDiff(ctx context.Context, d *pluginsdk.ResourceDiff, _ interface{}) error {
	...
	// When `match` property is set as default by Azure while the property is not configured by user in Terraform script, set new value of `match` property (newProbe) be same as default generated by Azure (oldProbe) to ignore the change during plan time. Only deal with probe TypeSet size equal to 1 as there will be ambiguity in order when size is more than 1
