...
func testAccSystemCenterVirtualMachineManagerVirtualNetwork_list_basic(t *testing.T) {
	...
	data := acceptance.BuildTestData(t, "azurerm_system_center_virtual_machine_manager_virtual_network", "testlist")
...
func (r SystemCenterVirtualMachineManagerVirtualNetworkResource) basicList(data acceptance.TestData) string {
	return fmt.Sprintf(`
%s
...
resource "azurerm_system_center_virtual_machine_manager_virtual_network" "test" {
  ...
  name                                                           = "acctest-scvmmvnet-${count.index}-%d"
...
func (r SystemCenterVirtualMachineManagerVirtualNetworkResource) basicQuery() string {
	return `
list "azurerm_system_center_virtual_machine_manager_virtual_network" "list" {
  provider = azurerm
  config {}
}
`
...
func (r SystemCenterVirtualMachineManagerVirtualNetworkResource) basicQueryByResourceGroupName(data acceptance.TestData) string {
	return fmt.Sprintf(`
list "azurerm_system_center_virtual_machine_manager_virtual_network" "list" {
  provider = azurerm
  config {
    resource_group_name = "acctestrg-scvmmvnet-%d"
  }
}
`, data.RandomInteger)
}
