...
func TestSystemCenterVirtualMachineManagerVirtualMachineTemplateName(t *testing.T) {
	testCases := []struct {
		Input    string
		Expected bool
	}{
	...
	for _, v := range testCases {
		_, errors := SystemCenterVirtualMachineManagerVirtualMachineTemplateName(v.Input, "name")
		result := len(errors) == 0
