	...
	return sdk.ResourceFunc{
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			if v := model.NodeStorageQuotaInMb; v != 0 {
				parameters.Properties.NodeStorageQuotaInMb = pointer.To(int64(model.NodeStorageQuotaInMb))
			}

			if v := model.NodeVCores; v != 0 {
				parameters.Properties.NodeVCores = pointer.To(int64(model.NodeVCores))
			}
