...
func validateAndGetBasePath(t *testing.T) string {
	...
	if basePath == "" {
		t.Fatalf("%s must be set", EnvVCRPath)
	}
	...
	if !strings.Contains(basePath, "/terraform-provider-azurerm/.local/") {
		t.Fatalf("%s must match path pattern %%/terraform-provider-azurerm/.local/%%, got: %s", EnvVCRPath, basePath)
	}
...
func GetHTTPClient(t *testing.T) *http.Client {
	...
	if debugEnabled {
		...
		client.Transport = &debugTransport{t: t, base: baseTransport, mode: vcrMode}
	}
...
func (d *debugTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	start := time.Now()
	d.t.Logf("%v VCR debug [%s] request: %s %s ", start, d.mode, req.Method, req.URL.String())
	...
	d.t.Logf("%v VCR debug [%s] response after %s: %s %s", time.Now(), d.mode, time.Since(start), req.URL.String(), resp.Status)
	return resp, nil
}
