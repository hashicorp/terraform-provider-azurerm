...
- ⚠️ Replay is still failing for `TestAccApiManagementLogger_basicEventHub` (step check path)
...
Observed during replay of `TestAccApiManagementLogger_basicEventHub`:

- VCR debug log shows `GET .../loggers/...` returning `200 OK`
- Step check fails with `404 ResourceGroupNotFound` from `ExistsInAzure`

This indicates replay behavior is not yet consistent across all call paths for this test. Work remains to align client lifecycle and call-path behavior for provider operations and check helpers in replay mode.
