...
1. **Discovery**: Calls the provider functions (e.g. `provider.SupportedServices()`) to find resources/data sources
2. **Parsing**: Parses the abstract syntax tree (AST) generated by the source code to retrieve method calls on `go-azure-sdk` clients, then uses combination of static single assignment (SSA) and AST to retrieve the API version and provider (i.e. Microsoft.<something>)
