...
func EnsureRegistered(ctx context.Context, client *providers.ProvidersClient, subscriptionId commonids.SubscriptionId, requiredRPs ResourceProviders, enhancedValidationEnabled bool) error {
	// Cache supported resource providers if RP registration and enhanced validation are not both disabled
	if len(requiredRPs) == 0 && !enhancedValidationEnabled {
