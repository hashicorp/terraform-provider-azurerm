...
(
  set -x
  go run internal/tools/schema-api/main.go -export .release/azurermProviderSchema.json
)

echo "Committing changelog and provider schema..."
(
  set -x
  git commit CHANGELOG.md .release/azurermProviderSchema.json -m v"${RELEASE}"
