...
.EXAMPLE
    .\install-copilot-setup.ps1 -Auto-Approve
...
$PowerShellModulesDir = Join-Path $ModulesDir "powershell"

try {
    # Import clean architecture modules only - completely self-contained
    Import-Module (Join-Path $PowerShellModulesDir "core.psm1") -Force -ErrorAction Stop
    Import-Module (Join-Path $PowerShellModulesDir "validation.psm1") -Force -ErrorAction Stop
    Import-Module (Join-Path $PowerShellModulesDir "installation.psm1") -Force -ErrorAction Stop
    Import-Module (Join-Path $PowerShellModulesDir "ui.psm1") -Force -ErrorAction Stop
